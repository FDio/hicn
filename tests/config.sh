...
while (("${#}")); do
  ...
  'ping')
    ...
    ;;
  'getlogs')
    docker cp hicn-light-client:/tmp/lite_client.log /tmp/lite_client.log
    docker cp hicn-light-server:/tmp/lite_server.log /tmp/lite_server.log
    docker cp hicn-light-server:/tmp/hiperf.log /tmp/hiperf.log
    break
    ;;
  *)
    exit 1
