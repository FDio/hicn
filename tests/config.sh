...
function setchannel() {
  ...
  DOCKER_COMMAND="docker-compose -f "${topology}".yml -f "${topology}-${configuration}".yml"
  ${DOCKER_COMMAND} exec "${service}" bash -c "/workspace/tests/config.sh link set ${device} ${link_model[${3}]}"
}
# test-name client/server link-model
function changechannel() {
  ...
  DOCKER_COMMAND="docker-compose -f "${topology}".yml -f "${topology}-${configuration}".yml"
  ${DOCKER_COMMAND} exec "${service}" bash -c "/workspace/tests/config.sh link change ${device} ${link_model[${3}]}"
}
...
function start() {
  ...
  DOCKER_COMMAND="docker-compose -f ${topology}.yml -f ${topology}-${conf}.yml"

  ${DOCKER_COMMAND} exec client bash /workspace/tests/config.sh runtest "${tests[${TESTNAME}]}"

  # Print also forwader log
  echo "Forwarder Log - CLIENT"
  ${DOCKER_COMMAND} exec client cat "${FORWARDER_LOG_PATH}"

  echo

  echo "Forwarder Log - SERVER"
  ${DOCKER_COMMAND} exec server cat "${FORWARDER_LOG_PATH}"
}
...
#---------------------------------------------------------------
 ...
 DOCKER_COMMAND="docker-compose -f 1-node.yaml exec client"
