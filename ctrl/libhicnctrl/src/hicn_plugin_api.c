...
int fill_face_with_info(hc_face_t * face, vapi_type_fib_path *path, hc_sock_t *s) {
  switch(path->type){
    ...
    case FIB_API_PATH_TYPE_UDP_ENCAP:
    {
      ...
      face->face.netdevice.index = path->nh.obj_id;
...
vapi_error_e parse_route_list( vapi_ctx_t ctx,
  ...
  if (empty_spots < reply->route.n_paths) {
    int new_size = data->size + (reply->route.n_paths - empty_spots);
  ...
  for (int i = 0; i < reply->route.n_paths; i++) {
    hc_route_t * route = &((hc_route_t*)(data->buffer))[data->current];
    //route->face_id = reply->faceids[i];
    route->cost = 1;
    route->len = reply->route.prefix.len;
    if (reply->route.prefix.address.af == ADDRESS_IP6)
      {
        memcpy(route->remote_addr.v6.as_u8, reply->route.prefix.address.un.ip6, 16);
      }
      else
      {
        memcpy(route->remote_addr.v4.as_u8, reply->route.prefix.address.un.ip4, 4);
      }
    route->family = reply->route.prefix.address.af == ADDRESS_IP6? AF_INET6 : AF_INET;
    /* Fill up the face information */
    if (fill_face_with_info(&(route->face), &reply->route.paths[i], rs->s) == 0)
      data->current++;
  }
...
int _hc_route_list(hc_sock_t *s, hc_data_t **pdata, bool async) {
  ...
  hicnp_msg->payload.table.table_id = 10;
