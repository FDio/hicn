...
vapi_error_e vapi_cb(vapi_ctx_t ctx, void *callback_ctx, void *payload) {
  ...
  if (hc_sock_map_get(s->map, ((vapi_type_msg_header2_t *)payload)->context, &request) <
      0) {
  ...
  s->buffer[s->woff % RECV_BUFLEN] = ((vapi_type_msg_header2_t *)payload)->context;
  s->woff++;
...
int hc_sock_send(hc_sock_t *s, hc_msg_t *msg, size_t msglen, int seq) {
  (void)msglen;
  (vapi_type_msg_header2_t*)msg->context = htons(seq);
