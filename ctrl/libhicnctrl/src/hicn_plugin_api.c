...
int hc_sock_get_next_seq(hc_sock_t *s, void *ctx) {
  return ((vapi_type_msg_header2_t *)ctx)->_vl_msg_id;
}
...
vapi_error_e vapi_cb(vapi_ctx_t ctx, void *callback_ctx, void *payload) {
  hc_sock_t *s = (hc_sock_t *)callback_ctx;
  ...
  if (hc_sock_map_get(s->map, ((vapi_type_msg_header2_t *)payload)->_vl_msg_id, &request) <
      0) {
  ...
  if (request->data->current == request->data->size) {
    ...
    memcpy(request->data->buffer, tmp,
  ...
  s->buffer[s->woff % RECV_BUFLEN] = ((vapi_type_msg_header2_t *)payload)->_vl_msg_id;
  s->woff++;
...
int hc_sock_send(hc_sock_t *s, hc_msg_t *msg, size_t msglen, int seq) {
  (void)msglen;
  (void)seq;
  int rc = vapi_send(s->g_vapi_ctx_instance, msg);
  if (rc != VAPI_OK) {
    perror("hc_sock_send");
    return -1;
  }
...
int hc_sock_recv(hc_sock_t *s) {
  return vapi_dispatch_one(s->g_vapi_ctx_instance);
}
...
int hc_execute_command(hc_sock_t *s, hc_msg_t *msg, size_t msg_len,
  ...
  int seq = hc_sock_get_next_seq(s, msg);
  if (seq < 0) {
...
int parse_route_create(uint8_t *src, uint8_t *dst) {
  ...
  vapi_msg_hicn_api_route_nhops_add_reply *reply =
      (vapi_msg_hicn_api_route_nhops_add_reply *)request->data->buffer;

  int retval = reply->payload.retval;
  free(reply);
  return retval;
}
...
int _hc_route_create(hc_sock_t *s, hc_route_t *route, bool async) {
  ...
  msg = vapi_alloc_hicn_api_route_nhops_add(s->g_vapi_ctx_instance);

  memcpy(&msg->payload.prefix.address.un.ip6[0], &route->remote_addr, 16);
  msg->payload.prefix.address.af =
      route->family == AF_INET ? IPV4_ADDR_LEN_BITS : IPV6_ADDR_LEN_BITS;
  msg->payload.prefix.len = route->len;
  msg->payload.face_ids[0] = route->face_id;
  ...
  vapi_set_event_cb(
      s->g_vapi_ctx_instance,
      vapi_lookup_vapi_msg_id_t(s->g_vapi_ctx_instance, msg->header._vl_msg_id),
      &vapi_cb, s);
...
int parse_route_delete(uint8_t *src, uint8_t *dst) {
  ...
  hc_sock_request_t *request = (hc_sock_request_t *)src;
  vapi_msg_hicn_api_route_del_reply *reply =
      (vapi_msg_hicn_api_route_del_reply *)request->data->buffer;
...
int _hc_route_delete(hc_sock_t *s, hc_route_t *route, bool async) {
  ...
  msg = vapi_alloc_hicn_api_route_del(s->g_vapi_ctx_instance);
  ...
  hc_command_params_t params = {
      ...
      .cmd_id = REMOVE_ROUTE,
      .size_in =
          sizeof(vapi_msg_hicn_api_route_del),  // useless for hicn-plugin
      .size_out = 0,
      .parse = NULL,
  };
...
int parse_route_list(uint8_t *src, uint8_t *dst) {
  ...
  hc_sock_request_t *request = (hc_sock_request_t *)src;
  vapi_msg_hicn_api_routes_details *reply =
      (vapi_msg_hicn_api_routes_details *)request->data->buffer;

  int size = 0;
  for (int i = 0; i < request->data->current; i++) {
    vapi_msg_hicn_api_routes_details *reply =
        (vapi_msg_hicn_api_routes_details
             *)(request->data->buffer + i * request->data->in_element_size);
    if (reply->payload.retval == 0) size += reply->payload.nfaces;
  }
  ...
  for (int j = 0; j < request->data->current; j++) {
    ...
    if (reply->payload.retval == 0) {
      for (int i = 0; i < reply->payload.nfaces; i++) {
        output[cur].face_id = reply->payload.faceids[i];
        output[cur].cost = 1;
        output[cur].len = reply->payload.prefix.len;
        memcpy(output[cur].remote_addr.v6.as_u8, reply->payload.prefix.address.un.ip6, 16);
        output[cur].family = reply->payload.prefix.address.af;
        cur++;
      }
    }
  }
  ...
  request->data->size = sizeof(hc_route_t) * reply->payload.nfaces;
...
int _hc_route_list(hc_sock_t *s, hc_data_t **pdata, bool async) {
  ...
  vapi_msg_hicn_api_routes_dump_hton(msg);
