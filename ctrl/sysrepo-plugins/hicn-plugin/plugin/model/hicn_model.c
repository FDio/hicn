 ...
 for(int count=0; count<nleaves; count++){
  ...
  sr_val_build_xpath(&vals[route], "%s[routeid='%d']/prefix", "/hicn:hicn-state/routes/route",
   ...
   if (temp->route.prefix.address.af==ADDRESS_IP4){
     ...
     memcpy(&sa.sin_addr.s_addr,temp->route.prefix.address.un.ip4,4);
     inet_ntop(AF_INET, &sa.sin_addr, buf, 4);
     vals[route].data.string_val =  temp->route.prefix.address.un.ip4;
   }else{
     struct sockaddr_in sa;
     memcpy(&sa.sin_addr.s_addr,temp->route.prefix.address.un.ip6,16);
     inet_ntop(AF_INET, &sa.sin_addr, buf, 16);
     vals[route].data.string_val =  temp->route.prefix.address.un.ip6;
   }


   memset(buf, 0x00, 20);
   /*  convert to string*/
   *parent = lyd_new_path(NULL, sr_get_context(sr_session_get_connection(session)), vals[route].xpath, buf, 0, 0);
...
static inline int  faces_update(sr_val_t * vals, uint32_t nleaves, struct lyd_node **parent, sr_session_ctx_t *session){
 ...
 for(int count=0; count<nleaves; count++){
   ...
   *parent = lyd_new_path(NULL, sr_get_context(sr_session_get_connection(session)), vals[face].xpath, buf, 0, 0);
