...
void usage() {
  LOG(INFO) << "HIPERF - Instrumentation tool for performing active network"
               "measurements with hICN";
  LOG(INFO) << "usage: hiperf [-S|-C] [options] [prefix|name]";
  LOG(INFO);
  LOG(INFO) << "SERVER OR CLIENT:";
#ifndef _WIN32
  LOG(INFO) << "-D\t\t\t\t\t"
            << "Run as a daemon";
  LOG(INFO) << "-R\t\t\t\t\t"
...
#endif
  LOG(INFO);
  LOG(INFO) << "CLIENT SPECIFIC:";
  LOG(INFO) << "-b\t<beta_parameter>\t\t"
            << "RAAQM beta parameter";
  LOG(INFO) << "-d\t<drop_factor_parameter>\t\t"
            << "RAAQM drop factor "
               "parameter";
  LOG(INFO) << "-L\t<interest lifetime>\t\t"
  ...
  LOG(INFO) << "\t\t\t\t\t\t1: unreliable ";
  LOG(INFO) << "\t\t\t\t\t\t2: rtx only ";
  LOG(INFO) << "\t\t\t\t\t\t3: fec only ";
  LOG(INFO) << "\t\t\t\t\t\t4: delay based ";
  LOG(INFO) << "\t\t\t\t\t\t5: low rate ";
  LOG(INFO) << "\t\t\t\t\t\t6: low rate and best path ";
  LOG(INFO) << "\t\t\t\t\t\t7: low rate and replication";
  LOG(INFO) << "\t\t\t\t\t\t8: low rate and best path/replication ";
  LOG(INFO) << "\t\t\t\t\t\t9: only fec low residual losses ";
  LOG(INFO) << "\t\t\t\t\t\t10: delay and best path ";
  LOG(INFO) << "\t\t\t\t\t\t11: delay and replication ";
  LOG(INFO) << "\t\t\t\t\t\t(default: 2 = rtx only) ";
  LOG(INFO) << "-H\t\t\t\t\t"
            << "Disable periodic print headers in stats report.";
  LOG(INFO) << "-n\t<nb_iterations>\t\t\t"
...
int main(int argc, char *argv[]) {
    ...
    switch (opt) {
      ...
      case 'y': {
        ...
        if (strncasecmp(optarg, "sha256", 6) == 0) {
          ...
          LOG(WARNING) << "Unknown hash algorithm. Using SHA 256.";
        }
  ...
  if (options > 0 && role < 0) {
    LOG(ERROR) << "Client options cannot be used when using the "
                  "software in server mode";
    usage();
    return EXIT_FAILURE;
  } else if (options < 0 && role > 0) {
    LOG(ERROR) << "Server options cannot be used when using the "
                  "software in client mode";
    usage();
    return EXIT_FAILURE;
  } else if (!role) {
    LOG(ERROR) << "Please specify if running hiperf as client "
                  "or server.";
    usage();
    return EXIT_FAILURE;
  }
  ...
  if (argv[optind] == 0) {
    LOG(ERROR) << "Please specify the name/prefix to use.";
    usage();
    return EXIT_FAILURE;
  } else {
    if (role > 0) {
