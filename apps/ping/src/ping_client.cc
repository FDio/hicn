...
class Client : private interface::Portal::TransportCallback {
 public:
  ...
  void onContentObject(Interest &interest, ContentObject &object) override {
    ...
    if (VLOG_IS_ON(2)) {
      LOG(INFO) << "<<< recevied object. ";
      LOG(INFO) << "<<< interest name: " << interest.getName().getPrefix()
      ...
      LOG(INFO) << "<<< round trip: " << rtt << " [us]";
      LOG(INFO) << "<<< interest name: " << interest.getName().getPrefix();

      LOG(INFO) << "<<< object name: " << object.getName();
      LOG(INFO) << "<<< content object size: "
  ...
  void onError(const std::error_code &ec) override {
    LOG(ERROR) << "Aborting ping due to internal error: " << ec.message();
    afterSignal();
  }
  ...
  void doPing() {
    ...
    if (VLOG_IS_ON(2)) {
      LOG(INFO) << ">>> send interest " << interest->getName()
                << " src port: " << interest->getSrcPort()
                << " dst port: " << interest->getDstPort()
  ...
  void afterSignal() {
    LOG(INFO) << "Stopping ping...";
    LOG(INFO) << "Sent: " << traffic_generator_->getSentCount()
...
void help() {
  LOG(INFO) << "usage: hicn-consumer-ping [options]";
  LOG(INFO) << "PING options";
  LOG(INFO) << "-i <val>          ping interval in microseconds (default "
               "1000000ms)";
  LOG(INFO) << "-m <val>          maximum number of pings to send (default 10)";
  LOG(INFO) << "-s <val>          sorce port (default 9695)";
  LOG(INFO) << "-d <val>          destination port (default 8080)";
  LOG(INFO) << "-t <val>          set packet ttl (default 64)";
  LOG(INFO) << "-a <val> <pass>   set the passphrase and the number of "
               "suffixes in interest manifest (default 0);";
  LOG(INFO) << "                  e.g. '-m 6 -a -2' sends two interest (0 and "
               "3) with 2 suffixes each (1,2 and 4,5 respectively)";
  LOG(INFO) << "HICN options";
  LOG(INFO) << "-n <val>          hicn name (default b001::1)";
  LOG(INFO) << "-l <val>          interest lifetime in milliseconds (default "
               "500ms)";
  ...
  LOG(INFO) << "-F <conf_file>    Path to optional configuration file for "
               "libtransport";
  LOG(INFO) << "-b <type>         Traffic generator type. Use 'RANDOM' for "
