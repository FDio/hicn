  ...
  asio::async_write(socket_, asio::buffer(buffer->data(), buffer->length()),
                    [this](std::error_code ec, std::size_t length) {
                      if (TRANSPORT_EXPECT_FALSE(!ec)) {
                        TRANSPORT_LOGI("Content successfully sent!");
                        ...
                        TRANSPORT_LOGI("Content NOOOT sent!");
                      }
                    });
}  // namespace transport
...
void ATSConnector::handleRead(std::error_code ec, std::size_t length,
  ...
  if (TRANSPORT_EXPECT_TRUE(!ec)) {
    ...
    receive_callback_(input_buffer_, bytes_in_buffer, 0, !size, false);
  ...
  asio::async_read_until(
      ...
      [this](std::error_code ec, std::size_t length) {
        ...
        if (TRANSPORT_EXPECT_TRUE(!ec)) {
          ...
          receive_callback_(buffer, header_input_buffer_.size(), !size, false,
                            true);
...
void ATSConnector::tryReconnection() {
  TRANSPORT_LOGI("Connection lost. Trying to reconnect...\n");
  if (state_ == ConnectorState::CONNECTED) {
    state_ = ConnectorState::CONNECTING;
    is_reconnection_ = true;
    io_service_.post([this]() {
      if (socket_.is_open()) {
        TRANSPORT_LOGI("Ok socket is open");
        socket_.shutdown(asio::ip::tcp::socket::shutdown_type::shutdown_both);
        socket_.close();
      }
