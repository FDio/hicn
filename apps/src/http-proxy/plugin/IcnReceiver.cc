...
void AsyncConsumerProducer::manageIncomingInterest(
  ...
  connector_.send(payload, [this, packet = std::move(packet)]() {
    TRANSPORT_LOGI("Insertion!! Size %lu",
                   (unsigned long)response_name_queue_.size());
  });
}

void AsyncConsumerProducer::publishContent(const uint8_t* data,
                                           std::size_t size,
                                           const int response_id, bool is_last,
                                           bool headers) {
  uint32_t start_suffix = 0;
  TRANSPORT_LOGI("Queue size %zu", (std::size_t)response_name_queue_.size());
  ...
  if (is_last) {
    TRANSPORT_LOGI("Deleting %s Queue size %zu", name.toString().c_str(),
                   (std::size_t)response_name_queue_.size());
