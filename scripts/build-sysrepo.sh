...
build_package() {
    ...
    rm -rf *
    cmake -DCMAKE_INSTALL_PREFIX=/usr -DBUILD_LIBYANG=ON  ${SCRIPT_PATH}/../extras/libyang
    make install 
    make package
    ...
    cmake -DCMAKE_INSTALL_PREFIX=/usr -DBUILD_HICNPLUGIN=ON -DBUILD_LIBMEMIF=OFF \
          -DBUILD_SYSREPOPLUGIN=ON  -DBUILD_SYSREPO=ON -DBUILD_HICNLIGHT=OFF \
          -DBUILD_LIBTRANSPORT=OFF -DBUILD_CTRL=OFF -DBUILD_UTILS=OFF ${SCRIPT_PATH}
    make package
