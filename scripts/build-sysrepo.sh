...
setup_fdio_repo() {
    ...
    if [ ${DISTRIB_ID} == "ubuntu" ]; then
        ...
        curl -O https://github.com/muscariello/build-scripts/raw/master/deb/libyang_0.16-r2_amd64.deb
        curl -O https://github.com/muscariello/build-scripts/raw/master/deb/sysrepo_0.7.7_amd64.deb
        sudo ${apt_get} clean && sudo ${apt_get} update
        sudo /usr/bin/dpkg -i ./libyang_0.16-r2_amd64.deb ./sysrepo_0.7.7_amd64.deb
        sudo ${apt_get} install -f -y --allow-unauthenticated --no-install-recommends
