...
install_collectd_headers() {
    ...
    if [ "${DISTRIB_ID}" == "centos" ]; then
        ...
        if [ "${VERSION_CODENAME}" == "xenial" ]; then
            awk '/config.h/ { print; print "#include \"collectd/liboconfig/oconfig.h\""; next }1' /usr/include/collectd/core/daemon/configfile.h | tee /usr/include/collectd/core/daemon/configfile.h
        fi            
    fi
}
