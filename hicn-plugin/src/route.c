...
walk_rc_t enable_data_receiving (vnet_main_t * vnm,
                                 vnet_sw_interface_t * si,
                                 void *ctx)
{
  fib_protocol_t *proto = (fib_protocol_t *) ctx;
  if (*proto == FIB_PROTOCOL_IP4 && si->sw_if_index != ~0)
    vnet_feature_enable_disable ("ip4-local", "hicn-data-input-ip4",
                                 si->sw_if_index, 1, 0, 0);
  else if (*proto == FIB_PROTOCOL_IP6 && si->sw_if_index != ~0)
    vnet_feature_enable_disable ("ip6-local", "hicn-data-input-ip6",
                                 si->sw_if_index, 1, 0, 0);
...
walk_rc_t disable_data_receiving (vnet_main_t * vnm,
                                  vnet_sw_interface_t * si,
                                  void *ctx)
{
  fib_protocol_t *proto = (fib_protocol_t *) ctx;
  ...
  if (fib_hicn_entry_index == FIB_NODE_INDEX_INVALID)
    {
      ...
      vnet_sw_interface_walk(vnm, enable_data_receiving, &(prefix->fp_proto));
...
hicn_route_disable (fib_prefix_t *prefix) {
  ...
  if (fib_hicn_entry_index == FIB_NODE_INDEX_INVALID)
      ...
      else
	{
      ...
      vnet_main_t * vnm = vnet_get_main ();
      vnet_sw_interface_walk(vnm, disable_data_receiving, &(prefix->fp_proto));
    }
