    ...
    if (dpo_is_adj(dpo))
      {
        ...
        u32 sw_if = adj->rewrite_header.sw_if_index;
        ip46_address_t * nh = &(adj->sub_type.nbr.next_hop);
        hicn_face_id_t face_id = HICN_FACE_NULL;

        hicn_face_t * face = hicn_face_get_with_dpo(nh, sw_if, dpo, &hicn_face_hashtb);

        /* If found the face is full, otherwise we look among the ifaces */
        if (face == NULL)
          {
            face = hicn_face_get(nh, sw_if, &hicn_face_hashtb);
          }
  ...
  for (int i = 0; i < fib_entry->entry_count; i++)
    {
      ...
      if (idx_nh == ~0)
      ...
      else
        {
         ...
         if (fib_entry->proto == FIB_PROTOCOL_IP4)
           vnet_feature_enable_disable ("ip4-local", "hicn-data-input-ip4",
                                        face->sw_if, 1, 0, 0);
         else if (fib_entry->proto == FIB_PROTOCOL_IP6)
           vnet_feature_enable_disable ("ip6-local", "hicn-data-input-ip6",
                                        face->sw_if, 1, 0, 0);
...
hicn_route_init ()
{
  ...
  ip_table_create(FIB_PROTOCOL_IP4, HICN_FIB_TABLE, 1, "hicn4");
  ip_table_create(FIB_PROTOCOL_IP6, HICN_FIB_TABLE, 1, "hicn6");
}
