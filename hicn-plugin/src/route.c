  ...
  if (ret == HICN_ERROR_NONE)
      ...
      for (int i = 0; i < n_face_dpo && (ret == HICN_ERROR_NONE); i++)
	{
	  ...
	  dpo_vft = hicn_dpo_get_vft (vft_id);
          ...
          hicn_face_t * face = hicn_dpoi_get_from_idx (faces_dpo_tmp[i].dpoi_index);
          //Disable feature on the interface
          if(prefix->fp_proto == FIB_PROTOCOL_IP4)
            vnet_feature_enable_disable("ip4-local", "hicn-data-input-ip4", face->shared.sw_if, 1, 0, 0);
          else if (prefix->fp_proto == FIB_PROTOCOL_IP6)
            vnet_feature_enable_disable("ip6-local", "hicn-data-input-ip6", face->shared.sw_if, 1, 0, 0);
  ...
  if (ret == HICN_ERROR_ROUTE_NOT_FOUND)
      ...
      for (int i = 0; i < n_face_dpo; i++)
	{
	  clib_memcpy (&nhops[i], &face_dpo_tmp[i], sizeof (dpo_id_t));
          hicn_face_t * face = hicn_dpoi_get_from_idx (face_dpo_tmp[i].dpoi_index);
          //Disable feature on the interface
          if(prefix->fp_proto == FIB_PROTOCOL_IP4)
            vnet_feature_enable_disable("ip4-local", "hicn-data-input-ip4", face->shared.sw_if, 1, 0, 0);
          else if (prefix->fp_proto == FIB_PROTOCOL_IP6)
  ...
  if (ret == HICN_ERROR_NONE)
    {
      ...
      //Disable feature on the interface
      if(prefix->fp_proto == FIB_PROTOCOL_IP4)
        vnet_feature_enable_disable("ip4-local", "hicn-data-input-ip4", face->shared.sw_if, 0, 0, 0);
      else if (prefix->fp_proto == FIB_PROTOCOL_IP6)
        vnet_feature_enable_disable("ip6-local", "hicn-data-input-ip6", face->shared.sw_if, 0, 0, 0);
