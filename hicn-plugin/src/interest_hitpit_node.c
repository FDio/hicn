 ...
 */
static uword hicn_interest_hitpit_node_fn(vlib_main_t *vm,
  ...
  while (n_left_from > 0) {
    ...
    while (n_left_from > 0 && n_left_to_next > 0) {
      ...
      dpo_id_t hicn_dpo_id0 = {.dpoi_type = dpo_vft0->hicn_dpo_get_type(),
                               .dpoi_proto = 0,
                               .dpoi_next_node = 0,
      ...
      if (tnow > pitp->shared.expire_time) {
        ...
        hicn_pcs_delete(rt->pitcs, &pitp, &node0, vm, hash_entry0, dpo_vft0,
                        &hicn_dpo_id0);
        ...
        if ((hash_entry0->he_flags & HICN_HASH_ENTRY_FLAG_CS_ENTRY)) {
          ...
          found = hicn_face_search(hicnb0->face_id, &(pitp->u.pit.faces));

          if (found) {
            strategy_vft0->hicn_select_next_hop(dpo_ctx_id0, &nh_idx, &outface);
            /* Retransmission */
            /*
  ...
  vlib_node_increment_counter(vm, hicn_interest_hitpit_node.index,
                              ...
                              stats.interests_aggregated);
  vlib_node_increment_counter(vm, hicn_interest_hitpit_node.index,
                              HICNFWD_ERROR_INT_RETRANS, stats.interests_retx);
  vlib_node_increment_counter(vm, hicn_interest_hitpit_node.index,
...
void drop_packet(u32 *next0) { *next0 = HICN_INTEREST_HITPIT_NEXT_ERROR_DROP; }
