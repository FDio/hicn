...
always_inline void clone_from_cs(vlib_main_t *vm, u32 *bi0_cs,
                                 vlib_buffer_t *dest, u8 isv6) {
  ...
  if (hicnb->flags & HICN_BUFFER_FLAGS_PKT_LESS_TWO_CL) {
    ...
    clib_memcpy_fast(dest->opaque2, cs_buf->opaque2, sizeof(cs_buf->opaque2));
...
static uword hicn_interest_hitcs_node_fn(vlib_main_t *vm,
...
static u8 *hicn_interest_hitcs_format_trace(u8 *s, va_list *args) {
  ...
  hicn_interest_hitcs_trace_t *t = va_arg(*args, hicn_interest_hitcs_trace_t *);
