...
static fib_table_walk_rc_t ip4_fib_show_walk_cb(
    fib_node_index_t fib_entry_index, void *arg) {
...
static fib_table_walk_rc_t ip6_fib_table_show_walk(
    fib_node_index_t fib_entry_index, void *arg) {
...
void hicn_mapme_process_fib_entry(vlib_main_t *vm, hicn_face_id_t face,
  ...
  if (load_balance_dpo_id->dpoi_type != DPO_LOAD_BALANCE) return;
...
void hicn_mapme_process_ip4_fib(vlib_main_t *vm, hicn_face_id_t face) {
  ...
  pool_foreach(fib_table, im4->fibs) {
    ...
    if (table_id >= 0 && table_id != (int)fib->table_id) continue;
    if (fib_index != ~0 && fib_index != (int)fib->index) continue;
...
void hicn_mapme_process_ip6_fib(vlib_main_t *vm, hicn_face_id_t face) {
  /* Walk IPv6 FIB */
 ...
 */
void hicn_mapme_on_face_added(vlib_main_t *vm, hicn_face_id_t face) {
...
static_always_inline void *get_packet_buffer(vlib_main_t *vm, u32 node_index,
                                             u32 dpoi_index,
                                             ip46_address_t *addr,
                                             hicn_type_t type) {
  vlib_frame_t *f;
...
static_always_inline bool hicn_mapme_send_message(vlib_main_t *vm,
  ...
  u8 *buffer = get_packet_buffer(vm, node_index, face, (ip46_address_t *)prefix,
...
static_always_inline void hicn_mapme_send_updates(vlib_main_t *vm,
...
static uword hicn_mapme_eventmgr_process(vlib_main_t *vm,
  ...
  for (;;) {
    ...
    if (timeout != 0) {
       ...
       */
      timeout =
          (due_time > current_time) ? due_time - current_time : DEFAULT_TIMEOUT;
    ...
    switch (event_type) {
      ...
      case HICN_MAPME_EVENT_FACE_NH_SET: {
        ...
        for (u8 i = 0; i < vec_len(retx_events); i++) {
          ...
          u8 j;
          for (j = 0; j < NXTLEN; j++)
            if (!dpo_cmp(&NXT[j].dpo, &retx->dpo)) break;
      ...
      case ~0:
        ...
        for (u8 pos = 0; pos < CURLEN; pos++) {
          ...
          if (!tfib) {
            DEBUG("NULL TFIB entry for dpoi_index=%d", retx->dpo.dpoi_index);
            continue;
          }
    ...
    if (event_data) vlib_process_put_event_data(vm, event_data);
