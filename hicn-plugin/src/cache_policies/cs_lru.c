...
int hicn_cs_lru_flush (vlib_main_t * vm, struct hicn_pit_cs_s * pitcs,  hicn_cs_policy_t * state)
{
  ...
  while (idx != 0)
    {
      ...
      u64 hashval = 0;
      hicn_hashtb_fullhash ((u8 *)&(lrunode->hn_key.ks.key), lrunode->hn_keysize,
					  &hashval);
      hicn_hash_bucket_t * bucket = NULL;
      if ((hashval & (pitcs->pcs_table->ht_bucket_count - 1)) == lrunode->bucket_id)
	{
	  //The bucket is in the non overflown
	  bucket = pool_elt_at_index(pitcs->pcs_table->ht_buckets, lrunode->bucket_id);
	}
      else
	{
	  bucket = pool_elt_at_index(pitcs->pcs_table->ht_overflow_buckets, lrunode->bucket_id);
	}
      hicn_hash_entry_t * hash_entry = &(bucket->hb_entries[lrunode->entry_idx]);
      hash_entry->locks++;
      hicn_pcs_cs_delete(vm, pitcs, &lrupcs, &lrunode, hash_entry, NULL, NULL);
      idx = state->tail;
      i++;
    }
