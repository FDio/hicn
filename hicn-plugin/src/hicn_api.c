...
static void vl_api_hicn_api_node_params_set_t_handler(
  ...
  rv = hicn_infra_plugin_enable_disable((int)(mp->enable_disable), pit_max_size,
                                        pit_max_lifetime_sec, cs_max_size, ~0);

  REPLY_MACRO(VL_API_HICN_API_NODE_PARAMS_SET_REPLY /* , rmp, mp, rv */);
}

static void vl_api_hicn_api_node_params_get_t_handler(
...
static void vl_api_hicn_api_node_stats_get_t_handler(
...
static void vl_api_hicn_api_face_params_get_t_handler(
...
static void send_face_details(hicn_face_t *face, vl_api_hicn_face_t *mp) {
  ...
  if (sw_interface != NULL) {
    sbuf = format(0, "%U", format_vnet_sw_interface_name, vnm, sw_interface);
    strcpy((char *)(mp->if_name), (char *)sbuf);
  }
}

static void send_faces_details(vl_api_registration_t *reg, hicn_face_t *face,
                               u32 context) {
...
static void vl_api_hicn_api_faces_dump_t_handler(
...
static void vl_api_hicn_api_face_get_t_handler(vl_api_hicn_api_face_get_t *mp) {
...
static void send_face_stats_details(vl_api_registration_t *reg,
  ...
  mp->_vl_msg_id = htons(VL_API_HICN_API_FACE_STATS_DETAILS + hm->msg_id_base);
  mp->context = context;
...
static void vl_api_hicn_api_face_stats_dump_t_handler(
...
static void send_route_details(vl_api_registration_t *reg,
...
static fib_table_walk_rc_t vl_api_hicn_api_route_dump_walk(fib_node_index_t fei,
                                                           void *arg) {
  ...
  const dpo_id_t *load_balance_dpo_id = fib_entry_contribute_ip_forwarding(fei);
...
static void vl_api_hicn_api_routes_dump_t_handler(
...
static void vl_api_hicn_api_face_prod_del_t_handler(
...
static void vl_api_hicn_api_face_cons_del_t_handler(
...
static void vl_api_hicn_api_udp_tunnel_add_del_t_handler(
  ...
  if (mp->is_add) {
    ...
    udp_tunnel_del(proto, fib_index, &src_addr, &dst_addr, src_port, dst_port,
 ...
 */
vnet_api_error_t hicn_face_api_entry_params_serialize(
    hicn_face_id_t faceid, vl_api_hicn_api_face_params_get_reply_t *reply) {
  int rv = HICN_ERROR_NONE;
