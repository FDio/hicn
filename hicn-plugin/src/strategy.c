...
hicn_strategy_format_trace (u8 * s, va_list * args)
{
  ...
  hicn_strategy_vft_t * vft = hicn_dpo_get_strategy_vft(t->dpo_type);

  return vft->hicn_strategy_format(s);
}
                  ...
                  /* vlib_node_registration_t * hicn_strategy_node) */
{
  ...
  hicn_strategy_vft_t * strategy = hicn_mw_strategy_get_vft();
  f64 tnow;

  from = vlib_frame_vector_args (frame);
  n_left_from = frame->n_vectors;
  next_index = (hicn_strategy_next_t) node->cached_next_index;
  rt = vlib_node_get_runtime_data (vm, node->index);
