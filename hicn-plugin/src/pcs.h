...
always_inline int hicn_pcs_cs_insert(
...
always_inline int hicn_pcs_pit_insert(
    hicn_pit_cs_t *pitcs, hicn_pcs_entry_t *entry, hicn_hash_node_t *node,
    hicn_hash_entry_t **hash_entry, u64 hashval, u32 *node_id,
    index_t *dpo_ctx_id, u8 *vft_id, u8 *is_cs, u8 *hash_entry_id,
    u32 *bucket_id, u8 *bucket_is_overflow);

always_inline void hicn_pcs_pit_delete(hicn_pit_cs_t *pitcs,
...
always_inline void hicn_pcs_remove_lock(
    hicn_pit_cs_t *pitcs, hicn_pcs_entry_t **pcs_entryp,
    hicn_hash_node_t **node, vlib_main_t *vm, hicn_hash_entry_t *hash_entry,
...
static inline hicn_pcs_entry_t *hicn_pit_get_data(hicn_hash_node_t *node) {
  return (hicn_pcs_entry_t *)(hicn_hashtb_node_data(node));
}

/* Init pit/cs data block (usually inside hash table node) */
static inline void hicn_pit_init_data(hicn_pcs_entry_t *p) {
...
/* Init pit/cs data block (usually inside hash table node) */
static inline void hicn_cs_init_data(hicn_pcs_entry_t *p) {
  p->shared.entry_flags = 0;
  p->u.pit.faces.n_faces = 0;
  p->u.pit.faces.is_overflow = 0;
}

static inline f64 hicn_pcs_get_exp_time(f64 cur_time_sec, u64 lifetime_msec) {
 ...
 * good choice.
 */
static inline void hicn_pit_set_lru_max(hicn_pit_cs_t *p, u32 limit) {
 ...
 */
static inline u32 hicn_pit_get_int_count(const hicn_pit_cs_t *pitcs) {
 ...
 */
static inline u32 hicn_pit_get_cs_count(const hicn_pit_cs_t *pitcs) {
  return (pitcs->pcs_cs_count);
}

static inline u32 hicn_pcs_get_ntw_count(const hicn_pit_cs_t *pitcs) {
  return (pitcs->policy_state.count);
}

static inline u32 hicn_pit_get_htb_bucket_count(const hicn_pit_cs_t *pitcs) {
  return (pitcs->pcs_table->ht_overflow_buckets_used);
}

static inline int hicn_cs_enabled(hicn_pit_cs_t *pit) {
 ...
 */
always_inline void hicn_pcs_delete_internal(
    hicn_pit_cs_t *pitcs, hicn_pcs_entry_t **pcs_entryp,
    hicn_hash_entry_t *hash_entry, hicn_hash_node_t **node, vlib_main_t *vm,
    const hicn_dpo_vft_t *dpo_vft, dpo_id_t *hicn_dpo_id) {
...
always_inline void hicn_pcs_cs_delete(vlib_main_t *vm, hicn_pit_cs_t *pitcs,
  ...
  if (!(hash_entry->he_flags & HICN_HASH_ENTRY_FLAG_DELETED)) {
    ...
    policy_vft->hicn_cs_dequeue(pitcs, (*nodep), (*pcs_entryp), policy_state);
  ...
  if (hash_entry->locks == 0) {
    hicn_pcs_delete_internal(pitcs, pcs_entryp, hash_entry, nodep, vm, dpo_vft,
                             hicn_dpo_id);
  } else {
...
always_inline int hicn_pcs_cs_insert_update(
  ...
  ret = hicn_pcs_cs_insert(vm, pitcs, entry, node, hash_entry, hashval, node_id,
...
always_inline int hicn_pcs_pit_insert(
  ...
  if (PREDICT_TRUE(ret == HICN_ERROR_NONE)) pitcs->pcs_pit_count++;
...
always_inline void hicn_pcs_pit_delete(hicn_pit_cs_t *pitcs,
  ...
  if (hash_entry->locks == 0) {
    pitcs->pcs_pit_count--;
    hicn_pcs_delete_internal(pitcs, pcs_entryp, hash_entry, node, vm, dpo_vft,
                             hicn_dpo_id);
  } else {
    hash_entry->he_flags |= HICN_HASH_ENTRY_FLAG_DELETED;
  }
...
always_inline int hicn_pcs_insert(vlib_main_t *vm, hicn_pit_cs_t *pitcs,
  ...
  if ((*hash_entry)->he_flags & HICN_HASH_ENTRY_FLAG_CS_ENTRY) {
    ...
    ret = hicn_pcs_pit_insert(pitcs, entry, node, hash_entry, hashval, node_id,
...
always_inline u16 hicn_infra_ms2clicks(u64 time_ms, u64 ms_per_click) {
  f64 time_clicks = ((f64)(time_ms + ms_per_click - 1)) / ((f64)ms_per_click);
