...
hicn_select_next_hop_rr (index_t dpo_idx, int *nh_idx, dpo_id_t ** outface)
{
  ...
  if (dpo_id_is_valid (&hicn_strategy_rr_ctx->default_ctx.next_hops[hicn_strategy_rr_ctx->current_nhop]))
    {
      *outface =
        (dpo_id_t *) & hicn_strategy_rr_ctx->
        default_ctx.next_hops[hicn_strategy_rr_ctx->current_nhop];

    }
  else
    return HICN_ERROR_STRATEGY_NH_NOT_FOUND;

  hicn_strategy_rr_ctx->current_nhop = (hicn_strategy_rr_ctx->current_nhop +1) % hicn_strategy_rr_ctx->default_ctx.entry_count;
