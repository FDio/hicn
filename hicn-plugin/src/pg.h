 ...
 *   generator stream then specifies it as next node (see documentation)
 * - hicnpg-data is added as next hop of the ip4/6-local node exploiting the corresponding
 *   feature. The node hicnpg-data runs after the node ip4/6-local and before
 *   ip4/6-local-end-of-arc.
...
typedef struct hicnpg_main_s
{
  ...
  ip46_address_t pgen_clt_hicn_name; //hICN name to put in the destiantion addess of an interest
