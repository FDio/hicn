                        ...
                        u8 isv6, vlib_buffer_t * b)
{
  if (PREDICT_FALSE ((node->flags & VLIB_NODE_FLAG_TRACE) &&
	...
	vlib_add_trace (vm, node, b, sizeof (*t));
      ...
      if (isv6)
        {
          clib_memcpy_fast(&(t->udp6.udp), vlib_buffer_get_current(b) + sizeof(ip6_header_t), sizeof(udp_header_t));
          clib_memcpy_fast(&(t->udp6.ip), vlib_buffer_get_current(b), sizeof(ip6_header_t));
          t->ishicn = hb->flags & HICN_BUFFER_FLAGS_FROM_UDP_TUNNEL;
        }
      else
        {
          clib_memcpy_fast(&(t->udp4.udp), vlib_buffer_get_current(b) + sizeof(ip4_header_t), sizeof(udp_header_t));
          clib_memcpy_fast(&(t->udp4.ip), vlib_buffer_get_current(b), sizeof(ip4_header_t));
          t->ishicn = hb->flags & HICN_BUFFER_FLAGS_FROM_UDP_TUNNEL;
        }
    }
}
