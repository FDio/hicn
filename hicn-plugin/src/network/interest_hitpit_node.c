			     ...
			     vlib_frame_t *frame)
{
	...
	while (n_left_from > 0)
	{
	   ...
	   */
			if (PREDICT_FALSE(ret != HICN_ERROR_NONE || !hicn_node_compare(nameptr, namelen, node0)))
			{
	   ...
	   */
		   ...
		   */
					...
					found =
					    hicn_face_search(rt->pitcs->hicn_face_bucket_pool, hicnb0->face_id,
							     &(pitp->u.pit.faces));
		       ...
		       */
					...
					{
						hicn_face_db_add_face(rt->pitcs->hicn_face_bucket_pool, hicnb0->face_id,
								      &pitp->u.pit.faces);
