...
hicn_mapme_nh_set (hicn_mapme_tfib_t *tfib, hicn_face_id_t in_face_id)
{
  ...
  for (int i = 0; i < n_entries; i++)
	...
	{
	  ip_nh_udp_tunnel_add_del_helper (
	    prefix->fp_proto, prefix, face->dpo.dpoi_index,
	    face->dpo.dpoi_proto, &fib_node_index, 0);
	}
      else
	{
	  continue;
  ...
  if (dpo_is_udp_encap (&face->dpo))
    {
      ip_nh_udp_tunnel_add_del_helper (
	prefix->fp_proto, prefix, face->dpo.dpoi_index, face->dpo.dpoi_proto,
	&fib_node_index, 1);
    }
  else if (dpo_is_adj (&face->dpo))
    {
      ip_nh_adj_add_del_helper (prefix->fp_proto, prefix, &face->nat_addr,
				face->sw_if, &fib_node_index, 1);
    }
  else
    {
...
hicn_mapme_nh_add (hicn_mapme_tfib_t *tfib, hicn_face_id_t face_id)
{
  ...
  if (dpo_is_udp_encap (&face->dpo))
    {
      ip_nh_udp_tunnel_add_del_helper (
	(fib_protocol_t) face->dpo.dpoi_proto, prefix, face->dpo.dpoi_index,
	face->dpo.dpoi_proto, &fib_node_index, 1);
    }
  else
    {
      ip_nh_adj_add_del_helper ((fib_protocol_t) face->dpo.dpoi_proto, prefix,
				&face->nat_addr, face->sw_if, &fib_node_index,
				1);
    }
			  ...
			  fib_node_index_t *fib_node_index)
{
  ...
  if (face->sw_if == ~0)
      ...
      else
	{
      ...
      ret = ip_nh_udp_tunnel_add_del_helper (prefix->fp_proto, prefix,
					     face->dpo.dpoi_index, dpo_proto,
					     fib_node_index, 1);
    }
  else
    {
