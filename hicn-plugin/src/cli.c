				     ...
				     vlib_cli_command_t * cmd)
{
  ...
  if (ip46_address_is_ip4(&prefix->fp_addr))
  ...
  else
    {
      ...
      prefix->fp_addr.ip6.as_u64[0] = prefix->fp_addr.ip6.as_u64[0] & mask.as_u64[0];
      prefix->fp_addr.ip6.as_u64[1] = prefix->fp_addr.ip6.as_u64[1] & mask.as_u64[1];
				     ...
				     vlib_cli_command_t * cmd)
{
  ...
  if (unformat_user (main_input, unformat_line_input, line_input))
      ...
      while (unformat_check_input (line_input) != UNFORMAT_END_OF_INPUT)
	{
	    ...
	    {;
            ...
            if (unformat (line_input, "size %d", &payload_size))
	    {
  ...
  if ((prefix->fp_addr.ip6.as_u64[0] == (u64)0 && prefix->fp_addr.ip6.as_u64[1] == 0) || payload_size == 0 || sw_if_index == ~0)
    {
      return clib_error_return (0,
