...
vlib_node_registration_t hicn_interest_pcslookup_node;
{
  ...
  if (!interest_manifest_is_valid (int_manifest_header, int_manifest_size))
    return;

  // Iterate over interest manifest entries
  interest_manifest_foreach_suffix (int_manifest_header, name.suffix)
  {
    // Lookup name
    ret = hicn_pcs_lookup_one (pitcs, &name, &pcs_entry);

    if (ret == HICN_ERROR_NONE)
      {
	// Match in PCS. We need to clone a packet for to the
	// interest_hic{pit,cs} nodes.
	vlib_buffer_clone (vm, bi0, clone, 1, 0);

	// Get clone and set the PCS result
	b0 = vlib_get_buffer (vm, bi0);
	hicn_get_buffer (b0)->pcs_entry_id =
	  hicn_pcs_entry_get_index (pitcs, pcs_entry);
 ...
 */
static uword
hicn_interest_manifest_pcslookup_node_fn (vlib_main_t *vm,
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  if (interest_manifest_is_valid (int_manifest_header, payload_size))
	    {
	      interest_manifest_foreach_suffix (int_manifest_header,
						name.suffix, pos)
	      {
		ret = hicn_pcs_lookup_one (rt->pitcs, &name, &pcs_entry);
		...
		if (ret == HICN_ERROR_NONE)
		  {
		    ...
		    ret = hicn_store_internal_state (
		      clone, hicn_pcs_entry_get_index (rt->pitcs, pcs_entry),
		      vnet_buffer (b0)->ip.adj_index[VLIB_TX]);
		  ...
		  {
		    // No match. Forward interest to strategy node.
		    forward = 1;
		  }
	      }
	    }
...
{
  return hicn_interest_pcslookup_node_fn (vm, node, frame);
}

VLIB_NODE_FN (hicn_interest_manifest_pcslookup_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
  return hicn_interest_manifest_pcslookup_node_fn (vm, node, frame);
}
