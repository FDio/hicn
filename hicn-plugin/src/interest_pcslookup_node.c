					      ...
					      vlib_frame_t *frame)
{
  ...
  while (n_left_from > 0)
      ...
      while (n_left_from > 0 && n_left_to_next > 0)
	{
	  ...
	  hicn_name_suffix_t *suffix = &name.suffix;
		...
		if (ret == HICN_ERROR_NONE)
		  {
		    ...
		    cloneb = vlib_get_buffer (vm, clone[0]);

		    ret = hicn_store_internal_state (
		      cloneb, hicn_pcs_entry_get_index (rt->pitcs, pcs_entry),
		      vnet_buffer (b0)->ip.adj_index[VLIB_TX]);
			...
			bitmap_unset_no_check (
		      ...
		      {
			// Interest must be forwarded as it contains
			// retransmissions
			forward = 1;
		      }
		...
		else
		  {
		    ...
		    // This cannot fail as we just checked if PCS contains this
		    // entry
