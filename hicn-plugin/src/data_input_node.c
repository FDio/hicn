...
format_hicn_data_input_trace (u8 * s, va_list * args)
{
    ...
    hicn_data_input_trace_t *t =
        va_arg (*args, hicn_data_input_trace_t *);
    u32 indent = format_get_indent (s);
    u8 isv6 = (u8) va_arg (*args, int);

    s = format (s, "%U hicn_data_input%s: sw_if_index %d next-index %d", format_white_space, indent,
                isv6 ? "_ip6" : "_ip4", t->sw_if_index, t->next_index);
    return s;
}
