...
static clib_error_t *hicn_face_cli_show_command_fn(vlib_main_t *vm,
  ...
  if (unformat_user(main_input, unformat_line_input, line_input)) {
    ...
    if (face_type_name != NULL) {
      ...
      vec_foreach_index(idx, face_type_names_vec) {
        if (!strcmp(face_type_names_vec[idx], face_type_name)) found = idx;
      }
      if (found == ~0) return (clib_error_return(0, "Face type unknown"));
    }
  }
  ...
  if (face_id != HICN_FACE_NULL) {
    if (!hicn_dpoi_idx_is_valid(face_id))
      return clib_error_return(0, "%s",
                               get_error_string(HICN_ERROR_FACE_NOT_FOUND));
    ...
    for (int i = 0; i < HICN_N_COUNTER; i++) {
      ...
      _vec_len(n) = 0;
      n = format(n, "bytes");
      s = format(s, "\n%U%-16v%16Ld\n", format_white_space, indent + 30, n,
                 v.bytes);
    }
