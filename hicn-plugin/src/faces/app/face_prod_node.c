...
static_always_inline u32
hicn_reply_probe_v4 (vlib_buffer_t * b)
{
...
static_always_inline u32
hicn_reply_probe_v6 (vlib_buffer_t * b)
{
  hicn_header_t *h0 = vlib_buffer_get_current (b);
...
static_always_inline u32
hicn_face_prod_match_and_fix (vlib_node_runtime_t * node,
				   vlib_buffer_t * b, fib_prefix_t * prefix)
{
  u8 *ptr = vlib_buffer_get_current (b);
  u8 v = *ptr & 0xf0;
  ...
  if ( v == 0x40 )
  ...
  else if ( v == 0x60 )
    {
      ...
      match_res = (*dst_port == clib_net_to_host_u16(DEFAULT_PROBING_PORT));
      hicn_reply_probe_v6(b);
    }
...
static uword
hicn_face_prod_output_node_fn (vlib_main_t * vm,
			      vlib_node_runtime_t * node,
			      vlib_frame_t * frame)
{
  u32 n_left_from, *from, *to_next;
  hicn_face_prod_out_next_t next_index;
  vl_api_hicn_api_node_stats_get_reply_t stats = { 0 };
...
/* *INDENT-OFF* */
VLIB_REGISTER_NODE(hicn_face_prod_output_node) =
  {
   .function = hicn_face_prod_output_node_fn,
   .name = "hicn-face-prod-output",
   .vector_size = sizeof(u32),
   .format_trace = format_face_prod_output_trace,
   .type = VLIB_NODE_TYPE_INTERNAL,
   .n_errors = ARRAY_LEN(face_prod_output_error_strings),
   .error_strings = face_prod_output_error_strings,
   .n_next_nodes = HICN_FACE_PROD_OUTPUT_N_NEXT,
   .next_nodes =
   {
    [HICN_FACE_PROD_OUT_NEXT_OUT_IP4] = "hicn-face-ip4-output",
    [HICN_FACE_PROD_OUT_NEXT_OUT_IP6] = "hicn-face-ip6-output",
    [HICN_FACE_PROD_OUT_NEXT_IN_IP4] = "hicn-face-ip4-input",
    [HICN_FACE_PROD_OUT_NEXT_IN_IP6] = "hicn-face-ip6-input",
    [HICN_FACE_PROD_OUT_NEXT_ERROR_DROP] = "error-drop",
   },
  };
/* *INDENT-ON* */
