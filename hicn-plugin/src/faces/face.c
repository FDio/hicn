...
u8 *face_show(u8 *s, int face_id, u32 indent) {
...
static fib_node_t *hicn_face_node_get(fib_node_index_t index) {
  hicn_face_t *face;
...
static void hicn_face_last_lock_gone(fib_node_t *node) {}

static hicn_face_t *hicn_face_from_fib_node(fib_node_t *node) {
  return ((hicn_face_t *)(((char *)node) -
                          STRUCT_OFFSET_OF(hicn_face_t, fib_node)));
}

static fib_node_back_walk_rc_t hicn_face_back_walk_notify(
...
static void hicn_face_show_memory(void) {}
...
void hicn_face_module_init(vlib_main_t *vm) {
  ...
  mhash_init(&hicn_face_vec_hashtb, sizeof(hicn_face_input_faces_t) /* value */,
...
u8 *format_hicn_face(u8 *s, va_list *args) {
  ...
  if (face->flags & HICN_FACE_FLAGS_FACE) {
    ...
    s = format(s, "nat address %U locks %u, path_label %u", format_ip46_address,
               &face->nat_addr, IP46_TYPE_ANY, face->locks, face->pl_id);
    ...
    if ((face->flags & HICN_FACE_FLAGS_DELETED)) s = format(s, " (deleted)");
...
static void hicn_iface_to_face(hicn_face_t *face, const dpo_id_t *dpo) {
  ...
  if (dpo_is_adj(dpo)) {
    ...
    if (dpo->dpoi_type != DPO_ADJACENCY_MIDCHAIN ||
      ...
      if (!ip46_address_is_zero(nh)) {
        ...
        face->fib_entry_index =
            fib_entry_track(fib_index, &prefix, hicn_face_fib_node_type,
...
int hicn_face_add(const dpo_id_t *dpo_nh, ip46_address_t *nat_address,
  ...
  face = hicn_face_get_with_dpo(nat_address, sw_if, dpo_nh, &hicn_face_hashtb);
  ...
  if (in_faces == NULL) {
    ...
    *vec = vec_new(hicn_face_id_t, 0);
    ...
    hicn_face_vec_t *vec = pool_elt_at_index(hicn_vec_pool, in_faces->vec_id);
    ...
    /* If the face is an application producer face, we set it as the preferred
     * incoming face. */
