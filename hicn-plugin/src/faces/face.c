...
clib_bihash_8_8_t face_dpo_hash;
...
hicn_face_module_init (vlib_main_t * vm)
{
  ...
  clib_bihash_init_8_8(&face_dpo_hash, "Mapping faces-dpos", 1024, 16 << 20 /* sized to contain 2M elements of size 8 bytes */);
}
               ...
               int sw_if, hicn_face_id_t * pfaceid, u8 is_app_prod)
{
  ...
  if (face == NULL || dpo_cmp(dpo_nh, &face->dpo))
    {
      /*
       * if dpos are different it means we haev different next hops.
       * This happens when we have multiple next hops with the same nat_address
       * and sw_if. For example the case in which we are connected to a switch and
       * we serve multiple hosts (belonging to the same network) from the same
       * interface. The dpo will be different since it uniquely identify a next hop.
       */
  ...
  if (!(face->flags & HICN_FACE_FLAGS_IFACE))
