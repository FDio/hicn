 ...
 */
static uword hicn_data_pcslookup_node_fn(vlib_main_t *vm,
    ...
    while (n_left_from > 0 && n_left_to_next > 0) {
      ...
      if (PREDICT_TRUE(ret0 == HICN_ERROR_NONE &&
        ...
        int res = hicn_hashtb_lookup_node(
            ...
            0 /*! take lock if hit CS */, &node_id0, &dpo_ctx_id0, &vft_id0,
            &is_cs0, &hash_entry_id, &bucket_id, &bucket_is_overflown);
      ...
      hicn_store_internal_state(b0, name_hash, node_id0, dpo_ctx_id0, vft_id0,
                                hash_entry_id, bucket_id, bucket_is_overflown);
...
/* packet trace format function */
static u8 *hicn_data_pcslookup_format_trace(u8 *s, va_list *args) {
  CLIB_UNUSED(vlib_main_t * vm) = va_arg(*args, vlib_main_t *);
  CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
  hicn_data_pcslookup_trace_t *t = va_arg(*args, hicn_data_pcslookup_trace_t *);
