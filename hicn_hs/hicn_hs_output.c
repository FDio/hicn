...
#include <hicn_hs/hicn_hs_inlines.h>
...
typedef enum _hicn_hs_output_next
{
  ...
  HICN_HS_OUTPUT_NEXT_HICN_INPUT,
...
#define foreach_hicn_hs_output4_next		\
  ...
  _ (HICN_INPUT, HICN_HS_INPUT4_NODE_NAME)	\
  _ (DROP, "error-drop")


#define foreach_hicn_hs_output6_next		\
  _ (IP_LOOKUP, "ip6-lookup")			\
  _ (HICN_INPUT, HICN_HS_INPUT6_NODE_NAME)	\
  _ (DROP, "error-drop")
...
{
  u32 n_left_from, *from, *bi, thread_index = vm->thread_index;
  u32 to_flush[VLIB_FRAME_SIZE], *tf, count;
  ...
  while (n_left_from >= 4)
	...
	CLIB_PREFETCH (b[3]->data, 2 * CLIB_CACHE_LINE_BYTES, STORE);
	  ...
	  vnet_buffer (b[1])->sw_if_index[VLIB_TX] = hc1->c_fib_index;
          vnet_buffer (b[1])->sw_if_index[VLIB_RX] = 0;
          next[0] = !hicn_hs_buffer(b[1])->is_interest;
	  tf[0] = bi[1];
  ...
  vlib_buffer_enqueue_to_next (vm, node, to_flush, nexts, count);
  vlib_node_increment_counter (vm, node_index, HICN_HS_ERROR_PACKETS_SENT, count);
