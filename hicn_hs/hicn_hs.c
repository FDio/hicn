		       ...
		       hicn_format_t format, hicn_name_t *name)
{
  ...
  if (PREDICT_FALSE(hicn_packet_init_header(format, header) < 0))
  {
    HICN_HS_DBG (1, "hicn header initialization failed");
    return 0;
  }

  if (PREDICT_FALSE(hicn_interest_set_name(format, header, name) < 0))
  {
    HICN_HS_DBG (1, "Failed to copy name into interest packet");
    return 0;
  }

  if (ctx->cfg_flags & HICN_HS_CFG_F_NO_CSUM_OFFLOAD)
    header->v6.tcp.csum = hicn_hs_compute_checksum (ctx, b);
  else {
    header->v6.tcp.csum = 0;
...
always_inline u32
hicn_hs_send_interests_i (vlib_main_t *vm, hicn_hs_ctx_t *ctx, vlib_buffer_t **b, u32 *bi, u32 count)
{
  int ret;
  ...
  while (count >= 8)
	...
	CLIB_PREFETCH (b[7]->data, 2 * CLIB_CACHE_LINE_BYTES, STORE);
      ...
      ret += hicn_hs_make_interest (ctx, b[0], format, name);
      ...
      ret += hicn_hs_make_interest (ctx, b[1], format, name);
      ...
      ret += hicn_hs_make_interest (ctx, b[2], format, name);
      ...
      ret += hicn_hs_make_interest (ctx, b[3], format, name);
...
hicn_hs_send_interests (hicn_hs_ctx_t * ctx, u32 start_offset, u32 n_interest)
{
  ...
  return hicn_hs_send_interests_i (vm, ctx, b, bi, n_interest);
}
					...
					session_t * s, session_error_t code)
{
  ...
  /**
   * Notify APP about connection completed.
   */
