		       ...
		       hicn_name_t* name, u32 seq_number, u8 is_interest)
{
  ...
  if (PREDICT_FALSE (ctx->current_bytes_produced == ctx->current_content_size && !is_interest))
    {
      // Last packet
		    ...
		    vlib_buffer_t * b)
{
  ...
  hicn_ops_vft[buffer->type.l1]->get_header_length(buffer->type, &hicn_header->protocol, (size_t *)(&skip));
...
void hicn_hs_process_incoming_interest (hicn_hs_ctx_t *ctx, vlib_buffer_t* interest)
{
  ...
  else
  {
	...
	ctx->current_production_name.ip6.suffix = 0;
