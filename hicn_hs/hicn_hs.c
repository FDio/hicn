		       ...
		       hicn_name_t* name, u32 seq_number, u8 is_interest)
{
  ...
  u16 payload_len = sizeof (tcp_header_t) + b->current_length;
...
{
  hicn_hs_ctx_t *ctx = (hicn_hs_ctx_t *) (tconn);
  u32 seq_number = ctx->hs_proto->next_seq_number (ctx);
  hicn_name_t *name = &ctx->current_production_name;
  vlib_buffer_push_hicn(ctx, b, HF_INET6_TCP, name, seq_number, 0);
