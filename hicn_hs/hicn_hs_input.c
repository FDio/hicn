			     ...
			     u8 is_ip4)
{
  ...
  if (buffer->is_interest)
    ...
    {
      ctx_index = vnet_buffer (b)->ip.adj_index[VLIB_TX];
      hicn_ops_vft[buffer->type.l2]->get_data_name_suffix (buffer->type, &hicn->protocol, &buffer->seq_number);
    }
  ...
  while (n_left_from > 0)
    {
      ...
      /**
       * Get information from tcp packet.
       */

      hicn_hs_process_incoming_data(ctx, b);
    }
