...
// static void
// send_data_chunk (hicn_client_main_t * hcm, eclient_session_t * s)
// {
//   u8 *test_data = hcm->connect_test_data;
...
hicn_client_init (vlib_main_t * vm)
{
  ...
  vec_validate (hcm->quic_session_index_by_thread, vtm->n_vlib_mains);
...
static int
hicn_client_session_connected_callback (u32 app_index, u32 api_context,
		     ...
		     unformat_input_t * input, vlib_cli_command_t * cmd)
{
  ...
  hcm->expected_connections = n_clients * hcm->quic_streams;
  hcm->rx_total = 0;
  hcm->tx_total = 0;
  ...
  switch (event_type)
	...
	ec_cli_output ("%d three-way handshakes in %.2f seconds %.2f/s",
      ...
      ec_cli_output ("Test started at %.6f", hcm->test_start_time);
      break;
  ...
  for (i = 0; i < vec_len (hcm->connection_index_by_thread); i++)
    {
      ...
      vec_reset_length (hcm->quic_session_index_by_thread[i]);
    }
...
VLIB_CLI_COMMAND (hicn_client_command, static) =
{
  ...
  .short_help = "test hicn hs client [uri <hicn://ip6_address>]",
