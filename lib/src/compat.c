...
int hicn_packet_get_format(const hicn_header_t *h, hicn_format_t *format) {
  *format = HF_UNSPEC;
 ...
 */
hicn_type_t hicn_format_to_type(hicn_format_t format) {
  switch (format) {
    case HF_INET_TCP:
...
int hicn_packet_compute_checksum(hicn_format_t format, hicn_header_t *h) {
  hicn_type_t type = hicn_format_to_type(format);
  return hicn_ops_vft[type.l1]->update_checksums(type, &h->protocol, 0, 0);
}

int hicn_packet_compute_header_checksum(hicn_format_t format, hicn_header_t *h,
                                        u16 init_sum) {
  hicn_type_t type = hicn_format_to_type(format);
...
int hicn_packet_check_integrity(hicn_format_t format, hicn_header_t *h) {
  hicn_type_t type = hicn_format_to_type(format);
...
int hicn_packet_get_header_length(hicn_format_t format, const hicn_header_t *h,
                                  size_t *header_length) {
...
int hicn_packet_get_payload_length(hicn_format_t format, const hicn_header_t *h,
...
int hicn_packet_set_payload_length(hicn_format_t format, hicn_header_t *h,
                                   size_t payload_length) {
  hicn_type_t type = hicn_format_to_type(format);
...
int hicn_packet_compare(const hicn_header_t *packet1,
  ...
  if (type1.as_u32 != type2.as_u32) return HICN_LIB_ERROR_UNEXPECTED;

  rc = hicn_ops_vft[type1.l1]->get_length(type1, &packet1->protocol, &len1);
  if (PREDICT_FALSE(rc < 0)) return HICN_LIB_ERROR_UNEXPECTED;

  rc = hicn_ops_vft[type2.l1]->get_length(type2, &packet2->protocol, &len2);
  if (PREDICT_FALSE(rc < 0)) return HICN_LIB_ERROR_UNEXPECTED;

  if (len1 != len2) return HICN_LIB_ERROR_UNEXPECTED;

  return memcmp((u8 *)packet1, (u8 *)packet2, len1);
}

int hicn_packet_get_name(hicn_format_t format, const hicn_header_t *h,
...
int hicn_packet_set_name(hicn_format_t format, hicn_header_t *h,
                         const hicn_name_t *name, u8 is_interest) {
  hicn_type_t type = hicn_format_to_type(format);

#ifndef HICN_VPP_PLUGIN
  if (name->type & HNT_IOV) return HICN_LIB_ERROR_NOT_IMPLEMENTED;
...
int hicn_packet_set_payload(hicn_format_t format, hicn_header_t *h,
...
int hicn_packet_get_payload(hicn_format_t format, const hicn_header_t *h,
...
int hicn_packet_get_locator(hicn_format_t format, const hicn_header_t *h,
...
int hicn_packet_set_locator(hicn_format_t format, hicn_header_t *h,
...
int hicn_packet_get_signature_size(hicn_format_t format, const hicn_header_t *h,
                                   size_t *bytes) {
  hicn_type_t type = hicn_format_to_type(format);
  return hicn_ops_vft[type.l1]->get_signature_size(type, &h->protocol, bytes);
}

int hicn_packet_set_signature_size(hicn_format_t format, hicn_header_t *h,
                                   size_t bytes) {
  hicn_type_t type = hicn_format_to_type(format);
...
int hicn_packet_get_hoplimit(const hicn_header_t *h, u8 *hops) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
      *hops = h->v6.ip.hlim;
      break;
...
int hicn_packet_set_hoplimit(hicn_header_t *h, u8 hops) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
      h->v6.ip.hlim = hops;
      break;
...
int hicn_packet_get_lifetime(const hicn_header_t *h, u32 *lifetime) {
  hicn_type_t type = hicn_header_to_type(h);
  return hicn_ops_vft[type.l1]->get_lifetime(type, &h->protocol,
                                             (hicn_lifetime_t *)lifetime);
}

int hicn_packet_set_lifetime(hicn_header_t *h, u32 lifetime) {
  hicn_type_t type = hicn_header_to_type(h);
  return hicn_ops_vft[type.l1]->set_lifetime(type, &h->protocol,
                                             (hicn_lifetime_t)lifetime);
}

int hicn_packet_get_reserved_bits(const hicn_header_t *h, u8 *reserved_bits) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_set_reserved_bits(hicn_header_t *h, u8 reserved_bits) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_get_payload_type(const hicn_header_t *h,
...
int hicn_packet_set_payload_type(hicn_header_t *h,
                                 hicn_payload_type_t payload_type) {
...
int hicn_packet_set_syn(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_reset_syn(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
      h->v6.tcp.flags = h->v6.tcp.flags & ~TCP_FLAG_SYN;
      break;
...
int hicn_packet_test_syn(const hicn_header_t *h, bool *flag) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_set_ack(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_reset_ack(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
      h->v6.tcp.flags = h->v6.tcp.flags & ~TCP_FLAG_ACK;
      break;
...
int hicn_packet_test_ack(const hicn_header_t *h, bool *flag) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_set_rst(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_reset_rst(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
      h->v6.tcp.flags = h->v6.tcp.flags & ~TCP_FLAG_RST;
      break;
...
int hicn_packet_test_rst(const hicn_header_t *h, bool *flag) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_set_fin(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_reset_fin(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
      h->v6.tcp.flags = h->v6.tcp.flags & ~TCP_FLAG_FIN;
      break;
...
int hicn_packet_test_fin(const hicn_header_t *h, bool *flag) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_set_ece(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_reset_ece(hicn_header_t *h) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
      h->v6.tcp.flags = h->v6.tcp.flags & ~TCP_FLAG_ECE;
      break;
...
int hicn_packet_test_ece(const hicn_header_t *h, bool *flag) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_set_src_port(hicn_header_t *h, u16 src_port) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_get_src_port(const hicn_header_t *h, u16 *src_port) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_set_dst_port(hicn_header_t *h, u16 dst_port) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_packet_get_dst_port(const hicn_header_t *h, u16 *dst_port) {
  switch (HICN_IP_VERSION(h)) {
    case 6:
...
int hicn_interest_get_name(hicn_format_t format, const hicn_header_t *interest,
                           hicn_name_t *name) {
...
int hicn_interest_set_name(hicn_format_t format, hicn_header_t *interest,
  ...
  int ret_err =
      hicn_packet_reset_ece(interest);  // interest packet -> ece flag unset
  if (ret_err < 0) return HICN_LIB_ERROR_UNEXPECTED;
