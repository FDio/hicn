...
int ipv4_init_packet_header(hicn_type_t type, hicn_protocol_t *h) {
...
int ipv4_get_interest_locator(hicn_type_t type, const hicn_protocol_t *h,
                              ip46_address_t *ip_address) {
  ip_address->ip4 = h->ipv4.saddr;
  return HICN_LIB_ERROR_NONE;
}

int ipv4_set_interest_locator(hicn_type_t type, hicn_protocol_t *h,
                              const ip46_address_t *ip_address) {
  h->ipv4.saddr = ip_address->ip4;
  return HICN_LIB_ERROR_NONE;
}

int ipv4_get_interest_name(hicn_type_t type, const hicn_protocol_t *h,
                           hicn_name_t *name) {
...
int ipv4_set_interest_name(hicn_type_t type, hicn_protocol_t *h,
                           const hicn_name_t *name) {
  h->ipv4.daddr = name->ip4.prefix_as_ip4;
  return CHILD_OPS(set_interest_name_suffix, type, h, &(name->ip4.suffix));
}

int ipv4_get_interest_name_suffix(hicn_type_t type, const hicn_protocol_t *h,
                                  hicn_name_suffix_t *suffix) {
  return CHILD_OPS(set_interest_name_suffix, type, h, suffix);
}

int ipv4_set_interest_name_suffix(hicn_type_t type, hicn_protocol_t *h,
                                  const hicn_name_suffix_t *suffix) {
  return CHILD_OPS(set_interest_name_suffix, type, h, suffix);
}

int ipv4_reset_interest_for_hash(hicn_type_t type, hicn_protocol_t *h) {
...
int ipv4_get_data_locator(hicn_type_t type, const hicn_protocol_t *h,
                          ip46_address_t *ip_address) {
  ip_address->ip4 = h->ipv4.daddr;
  return HICN_LIB_ERROR_NONE;
}

int ipv4_set_data_locator(hicn_type_t type, hicn_protocol_t *h,
                          const ip46_address_t *ip_address) {
  h->ipv4.daddr = ip_address->ip4;
  return HICN_LIB_ERROR_NONE;
}

int ipv4_get_data_name(hicn_type_t type, const hicn_protocol_t *h,
                       hicn_name_t *name) {
...
int ipv4_set_data_name(hicn_type_t type, hicn_protocol_t *h,
                       const hicn_name_t *name) {
  h->ipv4.saddr = name->ip4.prefix_as_ip4;
  return CHILD_OPS(set_data_name_suffix, type, h, &(name->ip4.suffix));
}

int ipv4_get_data_name_suffix(hicn_type_t type, const hicn_protocol_t *h,
                              hicn_name_suffix_t *suffix) {
  return CHILD_OPS(get_data_name_suffix, type, h, suffix);
}

int ipv4_set_data_name_suffix(hicn_type_t type, hicn_protocol_t *h,
                              const hicn_name_suffix_t *suffix) {
  return CHILD_OPS(set_data_name_suffix, type, h, suffix);
}

int ipv4_get_data_pathlabel(hicn_type_t type, const hicn_protocol_t *h,
                            u32 *pathlabel) {
  return CHILD_OPS(get_data_pathlabel, type, h, pathlabel);
}

int ipv4_set_data_pathlabel(hicn_type_t type, hicn_protocol_t *h,
                            const u32 pathlabel) {
  return CHILD_OPS(set_data_pathlabel, type, h, pathlabel);
}

int ipv4_update_data_pathlabel(hicn_type_t type, hicn_protocol_t *h,
                               const hicn_faceid_t face_id) {
  return CHILD_OPS(update_data_pathlabel, type, h, face_id);
}

int ipv4_reset_data_for_hash(hicn_type_t type, hicn_protocol_t *h) {
...
int ipv4_get_lifetime(hicn_type_t type, const hicn_protocol_t *h,
                      hicn_lifetime_t *lifetime) {
  return CHILD_OPS(get_lifetime, type, h, lifetime);
}

int ipv4_set_lifetime(hicn_type_t type, hicn_protocol_t *h,
                      const hicn_lifetime_t lifetime) {
  return CHILD_OPS(set_lifetime, type, h, lifetime);
}

int ipv4_update_checksums(hicn_type_t type, hicn_protocol_t *h,
...
int ipv4_verify_checksums(hicn_type_t type, hicn_protocol_t *h,
                          u16 partial_csum, size_t payload_length) {
  /*
   * Checksum field is not accounted for in lower layers, so we can compute
   * them in any order. Note that it is only a header checksum.
   */
  if (csum(h, IPV4_HDRLEN, 0) != 0) return HICN_LIB_ERROR_CORRUPTED_PACKET;
...
int ipv4_rewrite_interest(hicn_type_t type, hicn_protocol_t *h,
...
int ipv4_rewrite_data(hicn_type_t type, hicn_protocol_t *h,
...
int ipv4_get_current_length(hicn_type_t type, const hicn_protocol_t *h,
                            size_t *header_length) {
  *header_length = IPV4_HDRLEN;
  return HICN_LIB_ERROR_NONE;
}

int ipv4_get_length(hicn_type_t type, const hicn_protocol_t *h,
                    size_t *header_length) {
  *header_length = h->ipv4.len;
  return HICN_LIB_ERROR_NONE;
}

int ipv4_get_current_header_length(hicn_type_t type, const hicn_protocol_t *h,
                                   size_t *header_length) {
  *header_length = IPV4_HDRLEN;
  return HICN_LIB_ERROR_NONE;
}

int ipv4_get_header_length(hicn_type_t type, const hicn_protocol_t *h,
                           size_t *header_length) {
...
int ipv4_set_payload_length(hicn_type_t type, hicn_protocol_t *h,
                            size_t payload_length) {
  size_t child_header_length;
  int rc = CHILD_OPS(get_header_length, type, h, &child_header_length);
  if (rc < 0) return rc;
...
int ipv4_get_signature_size(hicn_type_t type, const hicn_protocol_t *h,
                            size_t *signature_size) {
  return CHILD_OPS(get_signature_size, type, h, signature_size);
}

int ipv4_set_signature_size(hicn_type_t type, hicn_protocol_t *h,
                            size_t signature_size) {
  return CHILD_OPS(set_signature_size, type, h, signature_size);
}

int ipv4_set_signature_timestamp(hicn_type_t type, hicn_protocol_t *h,
                                 uint64_t signature_timestamp) {
  return CHILD_OPS(set_signature_timestamp, type, h, signature_timestamp);
}

int ipv4_get_signature_timestamp(hicn_type_t type, const hicn_protocol_t *h,
                                 uint64_t *signature_timestamp) {
  return CHILD_OPS(get_signature_timestamp, type, h, signature_timestamp);
}

int ipv4_set_validation_algorithm(hicn_type_t type, hicn_protocol_t *h,
                                  uint8_t validation_algorithm) {
  return CHILD_OPS(set_validation_algorithm, type, h, validation_algorithm);
}

int ipv4_get_validation_algorithm(hicn_type_t type, const hicn_protocol_t *h,
                                  uint8_t *validation_algorithm) {
  return CHILD_OPS(get_validation_algorithm, type, h, validation_algorithm);
}

int ipv4_set_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t *key_id) {
  return CHILD_OPS(set_key_id, type, h, key_id);
}

int ipv4_get_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t **key_id,
                    uint8_t *key_id_size) {
