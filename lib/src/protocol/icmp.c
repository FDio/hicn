...
DECLARE_get_interest_locator(icmp, UNEXPECTED) DECLARE_set_interest_locator(
    icmp, UNEXPECTED) DECLARE_get_interest_name(icmp, UNEXPECTED)
    DECLARE_set_interest_name(icmp, UNEXPECTED) DECLARE_get_interest_name_suffix(
        icmp, UNEXPECTED) DECLARE_set_interest_name_suffix(icmp, UNEXPECTED)
        DECLARE_get_data_locator(icmp, UNEXPECTED) DECLARE_set_data_locator(
            icmp, UNEXPECTED) DECLARE_get_data_name(icmp, UNEXPECTED)
            DECLARE_set_data_name(icmp, UNEXPECTED) DECLARE_get_data_name_suffix(
                icmp, UNEXPECTED) DECLARE_set_data_name_suffix(icmp, UNEXPECTED)
                DECLARE_get_data_pathlabel(
                    icmp, UNEXPECTED) DECLARE_set_data_pathlabel(icmp,
                                                                 UNEXPECTED)
                    DECLARE_update_data_pathlabel(
                        icmp, UNEXPECTED) DECLARE_get_lifetime(icmp, UNEXPECTED)
                        DECLARE_set_lifetime(icmp, UNEXPECTED) DECLARE_get_length(
                            icmp,
                            UNEXPECTED) DECLARE_get_payload_length(icmp,
                                                                   UNEXPECTED)
                            DECLARE_set_payload_length(
                                icmp,
                                UNEXPECTED) int icmp_init_packet_header(hicn_type_t
                                                                            type,
                                                                        hicn_protocol_t
                                                                            *h) {
...
int icmp_reset_interest_for_hash(hicn_type_t type, hicn_protocol_t *h) {
  h->icmp.csum = 0;

  return CHILD_OPS(reset_interest_for_hash, type, h);
}

int icmp_reset_data_for_hash(hicn_type_t type, hicn_protocol_t *h) {
  h->icmp.csum = 0;

  return CHILD_OPS(reset_data_for_hash, type, h);
}

int icmp_update_checksums(hicn_type_t type, hicn_protocol_t *h,
...
int icmp_verify_checksums(hicn_type_t type, hicn_protocol_t *h,
                          u16 partial_csum, size_t payload_length) {
  return HICN_LIB_ERROR_NOT_IMPLEMENTED;
...
int icmp_rewrite_interest(hicn_type_t type, hicn_protocol_t *h,
...
int icmp_rewrite_data(hicn_type_t type, hicn_protocol_t *h,
...
int icmp_get_current_header_length(hicn_type_t type, const hicn_protocol_t *h,
                                   size_t *header_length) {
  *header_length = ICMP_HDRLEN;
  return HICN_LIB_ERROR_NONE;
}

int icmp_get_header_length(hicn_type_t type, const hicn_protocol_t *h,
                           size_t *header_length) {
...
int icmp_get_signature_size(hicn_type_t type, const hicn_protocol_t *h,
                            size_t *signature_size) {
  return CHILD_OPS(get_signature_size, type, h, signature_size);
}

int icmp_set_signature_size(hicn_type_t type, hicn_protocol_t *h,
                            size_t signature_size) {
  return CHILD_OPS(set_signature_size, type, h, signature_size);
}

int icmp_set_signature_timestamp(hicn_type_t type, hicn_protocol_t *h,
                                 uint64_t signature_timestamp) {
  return CHILD_OPS(set_signature_timestamp, type, h, signature_timestamp);
}

int icmp_get_signature_timestamp(hicn_type_t type, const hicn_protocol_t *h,
                                 uint64_t *signature_timestamp) {
  return CHILD_OPS(get_signature_timestamp, type, h, signature_timestamp);
}

int icmp_set_validation_algorithm(hicn_type_t type, hicn_protocol_t *h,
                                  uint8_t validation_algorithm) {
  return CHILD_OPS(set_validation_algorithm, type, h, validation_algorithm);
}

int icmp_get_validation_algorithm(hicn_type_t type, const hicn_protocol_t *h,
                                  uint8_t *validation_algorithm) {
  return CHILD_OPS(get_validation_algorithm, type, h, validation_algorithm);
}

int icmp_set_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t *key_id) {
  return CHILD_OPS(set_key_id, type, h, key_id);
}

int icmp_get_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t **key_id,
                    uint8_t *key_id_size) {
