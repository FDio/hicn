...
int ipv6_init_packet_header(hicn_type_t type, hicn_protocol_t *h) {
...
int ipv6_get_interest_locator(hicn_type_t type, const hicn_protocol_t *h,
                              ip46_address_t *ip_address) {
  ip_address->ip6 = h->ipv6.saddr;
  return HICN_LIB_ERROR_NONE;
}

int ipv6_set_interest_locator(hicn_type_t type, hicn_protocol_t *h,
                              const ip46_address_t *ip_address) {
  h->ipv6.saddr = ip_address->ip6;
  return HICN_LIB_ERROR_NONE;
}

int ipv6_get_interest_name(hicn_type_t type, const hicn_protocol_t *h,
                           hicn_name_t *name) {
...
int ipv6_set_interest_name(hicn_type_t type, hicn_protocol_t *h,
                           const hicn_name_t *name) {
  h->ipv6.daddr = name->ip6.prefix_as_ip6;
  return CHILD_OPS(set_interest_name_suffix, type, h, &(name->ip6.suffix));
}

int ipv6_get_interest_name_suffix(hicn_type_t type, const hicn_protocol_t *h,
                                  hicn_name_suffix_t *suffix) {
  return CHILD_OPS(get_interest_name_suffix, type, h, suffix);
}

int ipv6_set_interest_name_suffix(hicn_type_t type, hicn_protocol_t *h,
                                  const hicn_name_suffix_t *suffix) {
  return CHILD_OPS(set_interest_name_suffix, type, h, suffix);
}

int ipv6_reset_interest_for_hash(hicn_type_t type, hicn_protocol_t *h) {
...
int ipv6_get_data_locator(hicn_type_t type, const hicn_protocol_t *h,
                          ip46_address_t *ip_address) {
  ip_address->ip6 = h->ipv6.daddr;
  return HICN_LIB_ERROR_NONE;
}

int ipv6_set_data_locator(hicn_type_t type, hicn_protocol_t *h,
                          const ip46_address_t *ip_address) {
  h->ipv6.daddr = ip_address->ip6;
  return HICN_LIB_ERROR_NONE;
}

int ipv6_get_data_name(hicn_type_t type, const hicn_protocol_t *h,
                       hicn_name_t *name) {
...
int ipv6_set_data_name(hicn_type_t type, hicn_protocol_t *h,
                       const hicn_name_t *name) {
  h->ipv6.saddr = name->ip6.prefix_as_ip6;
  return CHILD_OPS(set_data_name_suffix, type, h, &(name->ip6.suffix));
}

int ipv6_get_data_name_suffix(hicn_type_t type, const hicn_protocol_t *h,
                              hicn_name_suffix_t *suffix) {
  return CHILD_OPS(get_data_name_suffix, type, h, suffix);
}

int ipv6_set_data_name_suffix(hicn_type_t type, hicn_protocol_t *h,
                              const hicn_name_suffix_t *suffix) {
  return CHILD_OPS(set_data_name_suffix, type, h, suffix);
}

int ipv6_get_data_pathlabel(hicn_type_t type, const hicn_protocol_t *h,
                            u32 *pathlabel) {
  return CHILD_OPS(get_data_pathlabel, type, h, pathlabel);
}

int ipv6_set_data_pathlabel(hicn_type_t type, hicn_protocol_t *h,
                            const u32 pathlabel) {
  return CHILD_OPS(set_data_pathlabel, type, h, pathlabel);
}

int ipv6_update_data_pathlabel(hicn_type_t type, hicn_protocol_t *h,
                               const hicn_faceid_t face_id) {
  return CHILD_OPS(update_data_pathlabel, type, h, face_id);
}

int ipv6_reset_data_for_hash(hicn_type_t type, hicn_protocol_t *h) {
...
int ipv6_get_lifetime(hicn_type_t type, const hicn_protocol_t *h,
                      hicn_lifetime_t *lifetime) {
  return CHILD_OPS(get_lifetime, type, h, lifetime);
}

int ipv6_set_lifetime(hicn_type_t type, hicn_protocol_t *h,
                      const hicn_lifetime_t lifetime) {
  return CHILD_OPS(set_lifetime, type, h, lifetime);
}

int ipv6_update_checksums(hicn_type_t type, hicn_protocol_t *h,
...
int ipv6_verify_checksums(hicn_type_t type, hicn_protocol_t *h,
                          u16 partial_csum, size_t payload_length) {
  /* Retrieve payload length if not specified */
  if (payload_length == 0) {
    int rc = ipv6_get_payload_length(type, h, &payload_length);
...
int ipv6_rewrite_interest(hicn_type_t type, hicn_protocol_t *h,
...
int ipv6_rewrite_data(hicn_type_t type, hicn_protocol_t *h,
...
int ipv6_get_length(hicn_type_t type, const hicn_protocol_t *h,
                    size_t *header_length) {
  *header_length = IPV6_HDRLEN + ntohs(h->ipv6.len);
  return HICN_LIB_ERROR_NONE;
}

int ipv6_get_current_header_length(hicn_type_t type, const hicn_protocol_t *h,
                                   size_t *header_length) {
  *header_length = IPV6_HDRLEN;
  return HICN_LIB_ERROR_NONE;
}

int ipv6_get_header_length(hicn_type_t type, const hicn_protocol_t *h,
                           size_t *header_length) {
...
int ipv6_set_payload_length(hicn_type_t type, hicn_protocol_t *h,
                            size_t payload_length) {
  size_t child_header_length;
  int rc = CHILD_OPS(get_header_length, type, h, &child_header_length);
  if (rc < 0) return rc;
...
int ipv6_get_signature_size(hicn_type_t type, const hicn_protocol_t *h,
                            size_t *signature_size) {
  return CHILD_OPS(get_signature_size, type, h, signature_size);
}

int ipv6_set_signature_size(hicn_type_t type, hicn_protocol_t *h,
                            size_t signature_size) {
  return CHILD_OPS(set_signature_size, type, h, signature_size);
}

int ipv6_set_signature_timestamp(hicn_type_t type, hicn_protocol_t *h,
                                 uint64_t signature_timestamp) {
  return CHILD_OPS(set_signature_timestamp, type, h, signature_timestamp);
}

int ipv6_get_signature_timestamp(hicn_type_t type, const hicn_protocol_t *h,
                                 uint64_t *signature_timestamp) {
  return CHILD_OPS(get_signature_timestamp, type, h, signature_timestamp);
}

int ipv6_set_validation_algorithm(hicn_type_t type, hicn_protocol_t *h,
                                  uint8_t validation_algorithm) {
  return CHILD_OPS(set_validation_algorithm, type, h, validation_algorithm);
}

int ipv6_get_validation_algorithm(hicn_type_t type, const hicn_protocol_t *h,
                                  uint8_t *validation_algorithm) {
  return CHILD_OPS(get_validation_algorithm, type, h, validation_algorithm);
}

int ipv6_set_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t *key_id) {
  return CHILD_OPS(set_key_id, type, h, key_id);
}

int ipv6_get_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t **key_id,
                    uint8_t *key_id_size) {
