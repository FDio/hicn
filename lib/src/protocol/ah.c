...
int ah_init_packet_header(hicn_type_t type, hicn_protocol_t *h) {
  return HICN_LIB_ERROR_NOT_IMPLEMENTED;
}

int ah_reset_interest_for_hash(hicn_type_t type, hicn_protocol_t *h) {
  size_t signature_size;
  int rc = hicn_ops_vft[type.l1]->get_signature_size(type, h, &signature_size);
  if (rc < 0) return rc;
  memset(&(h->ah.validationPayload), 0, signature_size);
  return CHILD_OPS(reset_interest_for_hash, type, h);
}

int ah_reset_data_for_hash(hicn_type_t type, hicn_protocol_t *h) {
  size_t signature_size;
  int rc = hicn_ops_vft[type.l1]->get_signature_size(type, h, &signature_size);
  if (rc < 0) return rc;
  memset(&(h->ah.validationPayload), 0, signature_size);
...
int ah_update_checksums(hicn_type_t type, hicn_protocol_t *h, u16 partial_csum,
                        size_t payload_length) {
  /* Nothing to do as there is no checksum in AH */
  return HICN_LIB_ERROR_NONE;
}

int ah_verify_checksums(hicn_type_t type, hicn_protocol_t *h, u16 partial_csum,
                        size_t payload_length) {
  /* Nothing to do as there is no checksum in AH */
  return HICN_LIB_ERROR_NONE;
}

int ah_rewrite_interest(hicn_type_t type, hicn_protocol_t *h,
...
int ah_rewrite_data(hicn_type_t type, hicn_protocol_t *h,
...
int ah_get_length(hicn_type_t type, const hicn_protocol_t *h, size_t *length) {
  return HICN_LIB_ERROR_NOT_IMPLEMENTED;
}

int ah_get_current_header_length(hicn_type_t type, const hicn_protocol_t *h,
                                 size_t *header_length) {
  *header_length = AH_HDRLEN + (h->ah.payloadlen << 2);
  return HICN_LIB_ERROR_NONE;
}

int ah_get_header_length(hicn_type_t type, const hicn_protocol_t *h,
                         size_t *header_length) {
...
int ah_get_signature_size(hicn_type_t type, const hicn_protocol_t *h,
                          size_t *signature_size) {
  *signature_size = h->ah.payloadlen << 2;
  return HICN_LIB_ERROR_NONE;
}

int ah_set_signature_size(hicn_type_t type, hicn_protocol_t *h,
                          const size_t signature_size) {
  h->ah.payloadlen = signature_size >> 2;
  return HICN_LIB_ERROR_NONE;
}

int ah_set_signature_timestamp(hicn_type_t type, hicn_protocol_t *h,
                               uint64_t signature_timestamp) {
  h->ah.timestamp_as_u64 = signature_timestamp;
  return HICN_LIB_ERROR_NONE;
}

int ah_get_signature_timestamp(hicn_type_t type, const hicn_protocol_t *h,
                               uint64_t *signature_timestamp) {
  *signature_timestamp = h->ah.timestamp_as_u64;
  return HICN_LIB_ERROR_NONE;
}

int ah_set_validation_algorithm(hicn_type_t type, hicn_protocol_t *h,
                                uint8_t validation_algorithm) {
  h->ah.validationAlgorithm = validation_algorithm;
  return HICN_LIB_ERROR_NONE;
}

int ah_get_validation_algorithm(hicn_type_t type, const hicn_protocol_t *h,
                                uint8_t *validation_algorithm) {
  *validation_algorithm = h->ah.validationAlgorithm;
  return HICN_LIB_ERROR_NONE;
}

int ah_set_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t *key_id) {
  memcpy(h->ah.keyId, key_id, sizeof(h->ah.keyId));
  return HICN_LIB_ERROR_NONE;
}

int ah_get_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t **key_id,
                  uint8_t *key_id_size) {
  *key_id = h->ah.keyId;
