...
int tcp_init_packet_header(hicn_type_t type, hicn_protocol_t *h) {
  h->tcp = (_tcp_header_t){
...
int tcp_get_interest_name_suffix(hicn_type_t type, const hicn_protocol_t *h,
                                 hicn_name_suffix_t *suffix) {
  *suffix = ntohl(h->tcp.name_suffix);
  return HICN_LIB_ERROR_NONE;
}

int tcp_set_interest_name_suffix(hicn_type_t type, hicn_protocol_t *h,
...
int tcp_reset_interest_for_hash(hicn_type_t type, hicn_protocol_t *h) {
  memset(&(h->tcp), 0, 4);
...
int tcp_get_data_name_suffix(hicn_type_t type, const hicn_protocol_t *h,
                             hicn_name_suffix_t *suffix) {
  *suffix = ntohl(h->tcp.name_suffix);
  return HICN_LIB_ERROR_NONE;
}

int tcp_set_data_name_suffix(hicn_type_t type, hicn_protocol_t *h,
                             const hicn_name_suffix_t *suffix) {
  h->tcp.name_suffix = htonl(*suffix);
  return HICN_LIB_ERROR_NONE;
}

int tcp_get_data_pathlabel(hicn_type_t type, const hicn_protocol_t *h,
                           u32 *pathlabel) {
  *pathlabel = h->tcp.seq_ack;
  return HICN_LIB_ERROR_NONE;
}

int tcp_set_data_pathlabel(hicn_type_t type, hicn_protocol_t *h,
                           const u32 pathlabel) {
  h->tcp.seq_ack = pathlabel;
  return HICN_LIB_ERROR_NONE;
}

int tcp_update_data_pathlabel(hicn_type_t type, hicn_protocol_t *h,
                              const hicn_faceid_t face_id) {
  hicn_pathlabel_t pl = (hicn_pathlabel_t)(
      (h->tcp.pathlabel & HICN_PATH_LABEL_MASK) >> (32 - HICN_PATH_LABEL_SIZE));
...
int tcp_reset_data_for_hash(hicn_type_t type, hicn_protocol_t *h) {
  memset(&(h->tcp), 0, 4);
  memset(&(h->tcp.seq_ack), 0, 12);
...
int tcp_get_lifetime(hicn_type_t type, const hicn_protocol_t *h,
                     hicn_lifetime_t *lifetime) {
  *lifetime = ntohs(h->tcp.urg_ptr) << (h->tcp.data_offset_and_reserved & 0xF);
  return HICN_LIB_ERROR_NONE;
}

int tcp_set_lifetime(hicn_type_t type, hicn_protocol_t *h,
  ...
  if (PREDICT_FALSE(lifetime >= HICN_MAX_LIFETIME)) {
    ...
    h->tcp.data_offset_and_reserved =
        (h->tcp.data_offset_and_reserved & ~0xF) | HICN_MAX_LIFETIME_MULTIPLIER;
    return HICN_LIB_ERROR_NONE;
  }
...
int tcp_update_checksums(hicn_type_t type, hicn_protocol_t *h, u16 partial_csum,
...
int tcp_verify_checksums(hicn_type_t type, hicn_protocol_t *h, u16 partial_csum,
                         size_t payload_length) {
...
int tcp_rewrite_interest(hicn_type_t type, hicn_protocol_t *h,
...
int tcp_rewrite_data(hicn_type_t type, hicn_protocol_t *h,
...
int tcp_get_current_header_length(hicn_type_t type, const hicn_protocol_t *h,
                                  size_t *header_length) {
  *header_length = TCP_HDRLEN;
  return HICN_LIB_ERROR_NONE;
}

int tcp_get_header_length(hicn_type_t type, const hicn_protocol_t *h,
                          size_t *header_length) {
...
int tcp_get_signature_size(hicn_type_t type, const hicn_protocol_t *h,
                           size_t *signature_size) {
  return CHILD_OPS(get_signature_size, type, h, signature_size);
}

int tcp_set_signature_size(hicn_type_t type, hicn_protocol_t *h,
                           size_t signature_size) {
  return CHILD_OPS(set_signature_size, type, h, signature_size);
}

int tcp_set_signature_timestamp(hicn_type_t type, hicn_protocol_t *h,
                                uint64_t signature_timestamp) {
  return CHILD_OPS(set_signature_timestamp, type, h, signature_timestamp);
}

int tcp_get_signature_timestamp(hicn_type_t type, const hicn_protocol_t *h,
                                uint64_t *signature_timestamp) {
  return CHILD_OPS(get_signature_timestamp, type, h, signature_timestamp);
}

int tcp_set_validation_algorithm(hicn_type_t type, hicn_protocol_t *h,
                                 uint8_t validation_algorithm) {
  return CHILD_OPS(set_validation_algorithm, type, h, validation_algorithm);
}

int tcp_get_validation_algorithm(hicn_type_t type, const hicn_protocol_t *h,
                                 uint8_t *validation_algorithm) {
  return CHILD_OPS(get_validation_algorithm, type, h, validation_algorithm);
}

int tcp_set_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t *key_id) {
  return CHILD_OPS(set_key_id, type, h, key_id);
}

int tcp_get_key_id(hicn_type_t type, hicn_protocol_t *h, uint8_t **key_id,
                   uint8_t *key_id_size) {
