...
u8 hicn_name_get_length(const hicn_name_t *name) { return name->len; }
...
int hicn_name_compare(const hicn_name_t *name_1, const hicn_name_t *name_2,
  ...
  if ((name1->type == HNT_CONTIGUOUS_V4 && name2->type == HNT_CONTIGUOUS_V6) ||
      (name1->type == HNT_CONTIGUOUS_V6 && name2->type == HNT_CONTIGUOUS_V4)) {
    return -1;
  }
...
int hicn_name_copy_to_destination(u8 *dst, const hicn_name_t *src,
  ...
  switch (src->type) {
    ...
    case HNT_IOV_V6:
      ...
      if (copy_suffix) {
        memcpy(dst + src->iov.buffers[0].iov_len, src->iov.buffers[1].iov_base,
               src->iov.buffers[1].iov_len);
      }
      break;
    default:
      return HICN_LIB_ERROR_NOT_IMPLEMENTED;
...
int hicn_name_to_ip_address(const hicn_name_t *name, ip_address_t *ip_address) {
  switch (name->type) {
    case HNT_CONTIGUOUS_V6:
...
inline int ip_address_len(const ip_address_t *ip_address) {
  return (ip_address->family == AF_INET6)
             ? IPV6_ADDR_LEN
...
int hicn_ip_ntop(const ip_address_t *ip_address, char *dst, const size_t len) {
  const char *rc;
