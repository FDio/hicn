...
if (!strcmp(h->h_name,"host-int")){
 ...
 file = fopen ("/sys/class/net/eth2/statistics/rx_bytes", "r");
 rec = 0,snd=0;
 fscanf (file, "%d", &rec);
 fclose(file);
 file = fopen ("/sys/class/net/eth2/statistics/tx_bytes", "r");
 fscanf (file, "%d", &snd);
 fclose(file);  
  
 sr_val_set_xpath(&vals[1], "/hicn:hicn-state/host/eth2");
 vals[1].type = SR_UINT64_T;
 vals[1].data.uint64_val = snd+rec;

 file = fopen ("/sys/class/net/eth3/statistics/rx_bytes", "r");
 rec = 0,snd=0;
 fscanf (file, "%d", &rec);
 fclose(file);
