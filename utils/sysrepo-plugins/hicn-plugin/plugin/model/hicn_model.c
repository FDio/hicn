...
/*
uint64_t teth11=0,teth2=0,teth3=0;
...
static void host_speed(void){
    ...
    while (fgets(buf, 2000, fp)) {
        ...
        RemoveSpaces(ifname);
        if (!strcmp(ifname,"eth11"))
          teth11=r_bytes+t_bytes;
...
static int hicn_state_host_cb(const char *xpath, sr_val_t **values,
 ...
 if (!strcmp(h->h_name,"host-vs1")){

  FILE* file = fopen ("/sys/class/net/eth3/statistics/rx_bytes", "r");
  int rec = 0,snd=0;
  fscanf (file, "%d", &rec);
  fclose(file);
  file = fopen ("/sys/class/net/eth3/statistics/tx_bytes", "r");
  fscanf (file, "%d", &snd);
  fclose(file);

 sr_val_set_xpath(&vals[0], "/hicn:hicn-state/host/eth1");
 vals[0].type = SR_UINT64_T;
 vals[0].data.uint64_val = 0;
 ...
 vals[2].type = SR_UINT64_T;
 vals[2].data.uint64_val = snd+rec;
}
if (!strcmp(h->h_name,"host-vs2")){

 FILE* file = fopen ("/sys/class/net/eth11/statistics/rx_bytes", "r");
 int rec = 0,snd=0;
 fscanf (file, "%d", &rec);
 fclose(file);
 file = fopen ("/sys/class/net/eth11/statistics/tx_bytes", "r");
 fscanf (file, "%d", &snd);
 fclose(file);
 sr_val_set_xpath(&vals[0], "/hicn:hicn-state/host/eth1");
 vals[0].type = SR_UINT64_T;
