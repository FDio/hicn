  ...
  int setup() {
    ...
    if (configuration_.secure_) {
      consumer_socket_ = std::make_shared<P2PSecureConsumerSocket>(
          configuration_.transport_protocol_);
    ...
    if (!configuration_.rtc_) {
      ...
      // consumer_socket_->setSocketOption(ConsumerCallbacksOptions::READ_CALLBACK, key_callback_);
      // consumer_socket_->setSocketOption(GeneralTransportOptions::KEY_CONTENT, true);
      ret = consumer_socket_->setSocketOption(
...
public:
  ...
  int setup() {
    ...
    if (configuration_.rtc_) {
    ...
    } else if (configuration_.secure_) {
      ...
      producer_socket_ = std::make_unique<P2PSecureProducerSocket>(identity);
    } else {
