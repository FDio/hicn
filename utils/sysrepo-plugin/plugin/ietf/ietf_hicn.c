...
hicn_state_t  * hicn_state = NULL;
hicn_strategy_t * hicn_strategy = NULL;
hicn_strategies_t * hicn_strategies =NULL;
hicn_route_t * hicn_route = NULL;
hicn_face_ip_params_t *  hicn_face_ip_params = NULL;
...
int hicn_subscribe_events(sr_session_ctx_t *session,
  ...
  rc = sr_dp_get_items_subscribe(session, "/hicn:hicn-state",
                                 hicn_state_cb, NULL, SR_SUBSCR_CTX_REUSE,
                                 subscription);
  if (rc != SR_ERR_OK) {
    SRP_LOG_DBG_MSG("Problem in subscription /hicn:hicn-state\n");
    goto error;
  }
