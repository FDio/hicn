...
InterfaceSet *
system_Interfaces(Forwarder *forwarder)
{
    ...
    for (next = ifaddr; next != NULL; next = next->ifa_next) {
        ...
        if (iface == NULL) {
            ...
            iface = interfaceCreate(next->ifa_name,
        ...
        switch (family) {
            case AF_INET: {
                Address *address = addressCreateFromInet((struct sockaddr_in *) next->ifa_addr);
                interfaceAddAddress(iface, address);
                break;
            }

            case AF_INET6: {
                Address *address = addressCreateFromInet6((struct sockaddr_in6 *) next->ifa_addr);
                interfaceAddAddress(iface, address);
                break;
            }
            ...
            case AF_LINK: {
                ...
                if (addr_dl->sdl_alen > 0) {
                    ...
                    Address *address = addressCreateFromLink((uint8_t *) &addr_dl->sdl_data[ addr_dl->sdl_nlen], addr_dl->sdl_alen);
                    interfaceAddAddress(iface, address);
                }
                break;
            }
...
Address *
system_GetMacAddressByName(Forwarder *forwarder, const char *interfaceName)
{
...
unsigned
system_InterfaceMtu(Forwarder *forwarder, const char *interfaceName)
{
    unsigned mtu = 0;
