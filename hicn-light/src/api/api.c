...
int
hc_data_find(hc_data_t * data, const void * element, cmp_t cmp, void **found)
{
...
int
hc_parse_listener(void * in, hc_listener_t * listener)
{
...
int
hc_parse_connection(void * in, hc_connection_t * connection)
{
    ...
    if (state == CONNECTION_STATE_UNDEFINED)
         return LIBHICNCTRL_FAILURE;
...
int
hc_connection_set_state(hc_sock_t * s, const char * conn_id_or_name,
        face_state_t state)
{
...
/* /!\ Please update constants in header file upon changes */
size_t
hc_connection_snprintf(char * s, size_t size, hc_connection_t * connection)
{
    char local[MAXSZ_HC_URL];
...
int
hc_parse_route(void * in, hc_route_t * route)
{
...
int
hc_face_to_connection(const hc_face_t * face, hc_connection_t * connection)
{
    const face_t * f = &face->face;
...
hc_face_create(hc_sock_t * s, hc_face_t * face)
{
    ...
    switch(face->face.type)
    {
        ...
        case FACE_TYPE_UDP:
            ...
            /* Test */
            if (hc_data_find(listeners, &listener, (cmp_t)hc_listener_cmp, (void**)&found) < 0)
                 return LIBHICNCTRL_FAILURE;
...
#ifdef WITH_POLICY
int
hc_face_set_state(hc_sock_t * s, const char * conn_id_or_name,
        face_state_t state)
{
...
int
hc_punting_create(hc_sock_t * s /* XXX */)
{
     return LIBHICNCTRL_SUCCESS;
}
