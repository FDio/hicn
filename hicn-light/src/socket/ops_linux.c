...
//DEPRECATED|int _nl_header(int request, uint8_t * buffer, size_t len, uint32_t flags);
//DEPRECATED|int _nl_payload_rule(uint8_t table_id, uint8_t * buffer, size_t len);
//DEPRECATED|int _nl_payload_link(uint32_t ifindex, uint8_t * buffer, size_t len);
//DEPRECATED|int _nl_payload_route(uint8_t table_id, uint8_t dst_len, uint8_t * buffer, size_t len);
//DEPRECATED|
//DEPRECATED|int _nl_parse(uint8_t * buffer, size_t len);
//DEPRECATED|int _nl_parse_ret(uint8_t * buffer, size_t len);
//DEPRECATED|int _nl_parse_link_ifid(uint8_t * buffer, size_t len, uint32_t * interface_id);
//DEPRECATED|int _nl_parse_link_ip_addr(uint8_t * buffer, size_t len, struct in6_addr * addr);
 ...
 */
int _nl_get_output_ifid(const char * ip_address, uint8_t address_family, uint32_t * interface_id);
 ...
 * @see getifaddrs
 */
int _nl_get_ip_addr(uint32_t interface_id, uint8_t address_family, ip_address_t * ip_address);

int _nl_set_ip_addr(uint32_t interface_id, ip_address_t * ip_address);

int _nl_up_if(uint32_t interface_id);

int _nl_add_in_route_table(const ip_address_t * prefix, const uint32_t interface_id, const uint8_t table_id);
int _nl_add_in_route_table_s(const char * prefix, const uint32_t interface_id, const uint8_t table_id);
int _nl_add_in_route_s(const char * prefix, const uint32_t interface_id);

int _nl_add_out_route(const char * gateway, const uint8_t address_family, const uint8_t table_idi, int default_route);
int _nl_del_out_route(const char * gateway, const uint8_t address_family, const uint8_t table_id);

int _nl_del_lo_route(const ip_address_t * ip_address);

int _nl_add_rule(const char * interface_name, const uint8_t address_family,const uint8_t table_id);
int _nl_del_rule(const char * interface_name, const uint8_t address_family, const uint8_t table_id);

int _nl_add_neigh_proxy(const ip_address_t * ip_address, const uint32_t interface_id);

int _nl_add_prio_rule(const ip_address_t * ip_address, const uint8_t address_family, const uint32_t priority, const uint8_t table_id);
int _nl_add_lo_prio_rule(const ip_address_t * ip_address, const uint8_t address_family, const uint32_t priority);
int _nl_del_prio_rule(const ip_address_t * ip_address, const uint8_t address_family, const uint32_t priority, const uint8_t table_id);
int _nl_del_lo_prio_rule(const ip_address_t * ip_address, const uint8_t address_family, const uint32_t priority);
 ...
 *  - ROUTE
       ...
       . add input route (ip route add PREFIX dev INTERFACE) for punting interests
       . add output route (ip route add default GATEWAY table TABLE) for routing interests (2, 3)
       . delete local route towards IP (ip route del IP table local) for ???
         /!\ could this be avoided by removing the local attribute in the netlink call ?
 *  - RULE
 *     . add output rule (ip rule add iif interface table TABLE) for routing interests (2, 3)
 *  - ND PROXY
 *     . enable NDP proxy functionality for IP on interface ID (ip -6 neigh add proxy IP dev INTERFACE)
...
// ??
#define FLAGS_CREATE_MATCH NLM_F_REQUEST | NLM_F_CREATE | NLM_F_ACK | NLM_F_MATCH
...
int _nl_payload_rule(uint8_t table_id, uint8_t address_family, uint8_t * buffer, size_t len)
{
    struct nlmsghdr * nl = (struct nlmsghdr *)buffer;
...
int _nl_payload_link(uint32_t ifindex, uint8_t * buffer, size_t len)
{
    ...
    //ifi->ifi_type = 0;
    ifi->ifi_index = ifindex; // new interface, could be specified since linux 3.7
    ifi->ifi_flags = 0;
...
int _nl_payload_route(uint8_t table_id, uint8_t addr_family, uint8_t dst_len, uint8_t * buffer, size_t len)
{
    struct nlmsghdr * nl = (struct nlmsghdr *)buffer;
...
uint32_t _nl_get_ifid(const char * interface_name)
{
    ...
    struct rtattr a_ifname = { RTA_LENGTH(strlen(interface_name) + 1), IFLA_IFNAME };
...
int _nl_get_output_ifid(const char * ip_address, uint8_t family_address, uint32_t * interface_id)
{
    int rc;
	...
	struct rtattr * rta;
	for (rta = RTM_RTA(rtm); RTA_OK(rta, attrlen); rta = RTA_NEXT(rta, attrlen)) {
...
int _nl_set_ip_addr(uint32_t interface_id, ip_address_t * ip_address)
{
    ...
    struct rtattr ifa_address = { RTA_LENGTH(ip_address_len(ip_address)), IFA_ADDRESS };
    // XXX maybe the reason why we have a local route ?
    //struct rtattr ifa_local   = { RTA_LENGTH(ip_address_len(ip_address)), IFA_LOCAL };
    struct iovec iov[] = {
        { &msg, sizeof(msg) },
 ...
 */
int _nl_add_out_route(const char * gateway, uint8_t address_family, const uint8_t table_id, int default_route)
{
    char buffer[BUFSIZE];
    struct nlmsghdr * hdr = (struct nlmsghdr *)buffer;
    size_t n;
    ...
    if(address_family == AF_INET){
        ...
        _nl_header(RTM_NEWROUTE, (uint8_t *)buffer, BUFSIZE, NLM_F_REQUEST | NLM_F_ACK | NLM_F_MATCH | NLM_F_ATOMIC);
...
int _nl_del_out_route(const char * gateway, const uint8_t address_family, const uint8_t table_id)
{
    ...
    if(address_family == AF_INET){
        ...
        _nl_header(RTM_DELROUTE, (uint8_t *)buffer, BUFSIZE, NLM_F_REQUEST | NLM_F_ACK | NLM_F_MATCH | NLM_F_ATOMIC);
        _nl_payload_route(table_id, address_family, 0, (uint8_t *)buffer, BUFSIZE);
 ...
 */
int _nl_add_rule(const char * interface_name, uint8_t address_family, const uint8_t table_id)
{
    char buffer[BUFSIZE];
    struct nlmsghdr * hdr = (struct nlmsghdr *)buffer;
    size_t n;
    ...
    /* XXX iif */
    addAttr(hdr, BUFSIZE, FRA_IIFNAME, (void*)interface_name, strlen(interface_name));
 ...
 */
int _nl_del_rule(const char * interface_name, uint8_t address_family, const uint8_t table_id)
{
    char buffer[BUFSIZE];
    struct nlmsghdr * hdr = (struct nlmsghdr *)buffer;
    size_t n;
...
int _nl_add_in_route_table(const ip_address_t * prefix, const uint32_t interface_id, const uint8_t table_id)
{
    ...
    struct {
        ...
        .payload.rtm_scope = prefix->family == AF_INET6 ? RT_SCOPE_UNIVERSE : RT_SCOPE_LINK,
...
int _nl_add_prio_rule(const ip_address_t * ip_address, uint8_t address_family, const uint32_t priority, const uint8_t table_id)
{
    /* Buffer for holding the response, with appropriate casting on the header */
    char buffer[BUFSIZE];
    struct nlmsghdr * hdr = (struct nlmsghdr *)buffer;
...
int _nl_add_lo_prio_rule(const ip_address_t * ip_address, uint8_t address_family, const uint32_t priority)
{
    return _nl_add_prio_rule(ip_address, address_family, priority, RT_TABLE_LOCAL);
}

/* ip -6 rule del from all prio 0 table local */
int _nl_del_prio_rule(const ip_address_t * ip_address, uint8_t address_family, const uint32_t priority, const uint8_t table_id)
{
    /* Buffer for holding the response, with appropriate casting on the header */
    char buffer[BUFSIZE];
    struct nlmsghdr * hdr = (struct nlmsghdr *)buffer;
    ...
    if (hdr->nlmsg_type == NLMSG_ERROR) {
        struct nlmsgerr * err = (struct nlmsgerr *)NLMSG_DATA(hdr);
        if (err->error < 0 && err->error != -2) { //-2 is not such file or directory
            errno = -err->error;
            goto ERR;
        }
    }
...
int _nl_del_lo_prio_rule(const ip_address_t * ip_address, uint8_t address_family, const uint32_t priority)
{
    return _nl_del_prio_rule(ip_address, address_family, priority, RT_TABLE_LOCAL);
}
...
int linux_get_tun_name(const char * prefix, const char * identifier, char *tun_name)
{
  snprintf(tun_name, IF_NAMESIZE, "%s-%s", prefix, identifier ? identifier : "main");
  return 0;
}
