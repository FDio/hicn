...
void hicn_destroy()
{
    ...
    for(i = 0; i < rules_counter; i++){
        if(strcmp(rules_to_remove[i].tun_name, "NONE") !=0 ) {
           rc = ops.del_rule(rules_to_remove[i].tun_name, rules_to_remove[i].address_family, rules_to_remove[i].table_id);
            if (rc < 0) {
                goto ERR;
            }
        } else {
            rc = ops.del_prio_rule(&rules_to_remove[i].ip_address, rules_to_remove[i].address_family,
    ...
    for(i = 0; i < routes_counter; i++){
	    rc = ops.del_out_route(routes_to_remove[i].remote_ip_address, routes_to_remove[i].address_family, routes_to_remove[i].table_id);
	    if (rc < 0) {
		    goto ERR;
	    }
    }
...
hicn_socket_t * hicn_socket_find(hicn_socket_helper_t * hicn, int fd)
{
  ...
  hicn_socket_t ** socket = tfind(&search, &hicn->socket_root, (cmp_t)hicn_socket_cmp);
...
int hicn_set_local_endpoint(hicn_socket_t * socket, const char * local_ip_address, bool allow_null)
{
...
int hicn_get_local_address(const ip_address_t * remote_address, ip_address_t * local_address)
{
    int rc = 0;
    uint32_t interface_id;
    char remote_address_str[INET_MAX_ADDRSTRLEN];

    rc = hicn_ip_ntop(remote_address, remote_address_str, sizeof(remote_address_str));
    if (rc < 0) {
        rc = HICN_SOCKET_ERROR_BIND_REMOTE_REPR;
        goto ERR;
    }

    rc = ops.get_output_ifid(remote_address_str, remote_address->family, &interface_id);
 ...
 */
int hicn_set_remote_endpoint(hicn_socket_t * socket, const char * remote_ip_address)
{
    ...
    if (ip_address_empty(&socket->connection.tun_ip_address)) {
        ...
        /* Local interface id */
        //INFO("Getting interface_id from gateway IP address %s", remote_ip_address);
/////
        ...
        rc = ops.get_output_ifid(remote_ip_address, (uint8_t) addr_family, &socket->connection.interface_id);
        ...
        /* Local ip */
        rc = ops.get_ip_addr(socket->connection.interface_id, (uint8_t) addr_family, &addr);
        if (rc < 0) {
            rc = HICN_SOCKET_ERROR_BIND_REMOTE_NETMASK;
            goto ERR;
        }
 ...
 */
int hicn_socket(hicn_socket_helper_t * hicn, const char * identifier, const char * local_ip_address)
{
    int rc;
...
int hicn_listen(hicn_socket_helper_t * hicn, int fd, const char * prefix)
{
    ...
    socket->connection.table_id = socket->tun_id % MAX_TABLES; //this table should be unused

    if(punting_table_id == -1)
	punting_table_id = socket->connection.table_id;


    rc = ops.add_prio_rule(&ip_address, ip_address.family, 0, socket->connection.table_id);
    if (rc < 0) {
        return rc;
    }
 ...
 */
int hicn_bind(hicn_socket_helper_t * hicn, int fd, const char * remote_ip_address)
{
    // uint32_t interface_id;
    int rc = HICN_SOCKET_ERROR_NONE;
    ...
    socket->connection.table_id = socket->tun_id % MAX_TABLES; // interface_id; // ops.get_free_table_id();
    ...
    rc = ops.add_rule(socket->tun_name, (uint8_t) addr_family, socket->connection.table_id);
    if (rc < 0) {
    ...
    //INFO("Adding output route in table %d via gateway %s", socket->connection.table_id,
    //        remote_ip_address);

    //if the address is an IPv6 and start with fe80 we need to specify the device in the route
    u32 default_interface = ~0;
    if(addr_family == AF_INET6 && strncmp(LOCAL_IPV6_PREFIX, remote_ip_address, strlen(LOCAL_IPV6_PREFIX)) == 0){
        rc = ops.get_output_ifid(remote_ip_address, (uint8_t) addr_family, &default_interface);
        if(rc < 0){
            goto ERR;
        }
    }

    rc = ops.add_out_route(remote_ip_address, (uint8_t) addr_family, socket->connection.table_id, default_interface);
    if (rc < 0) {
	...
	//we use as an id (socket->tun_id - 1) % MAX_TABLES, so that we will hava a collision
	//only after 255 new interfaces
	punting_table_id = (socket->tun_id - 1) % MAX_TABLES;
    }
    rc =  ops.add_in_route_table(&socket->connection.tun_ip_address, socket->tun_id, punting_table_id);
    if (rc < 0) {
        rc = HICN_SOCKET_ERROR_BIND_ROUTE;
        goto ERR;
    }
