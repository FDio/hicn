...
static void             _strategyLoadBalancerWithPD_ReceiveObject(StrategyImpl *strategy, const NumberSet *egressId, const Message *objectMessage, Ticks rtt);
static void             _strategyLoadBalancerWithPD_OnTimeout(StrategyImpl *strategy, const NumberSet *egressId);
static NumberSet *_strategyLoadBalancerWithPD_LookupNexthop(StrategyImpl *strategy, const Message *interestMessage);
static NumberSet *_strategyLoadBalancerWithPD_ReturnNexthops(StrategyImpl *strategy);
static unsigned         _strategyLoadBalancerWithPD_CountNexthops(StrategyImpl *strategy);
static void             _strategyLoadBalancerWithPD_AddNexthop(StrategyImpl *strategy, unsigned connectionId);
static void             _strategyLoadBalancerWithPD_RemoveNexthop(StrategyImpl *strategy, unsigned connectionId);
static void             _strategyLoadBalancerWithPD_ImplDestroy(StrategyImpl **strategyPtr);
static strategy_type    _strategyLoadBalancerWithPD_GetStrategy(StrategyImpl *strategy);
...
strategyLoadBalancerWithPD_Create()
{
    ...
    parcAssertNotNull(strategy, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(StrategyLoadBalancerWithPD));
    ...
    parcAssertNotNull(impl, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(StrategyImpl));
...
void
strategyLoadBalancerWithPD_SetConnectionTable(StrategyImpl *strategy, ConnectionTable *connTable)
{
...
strategy_type
_strategyLoadBalancerWithPD_GetStrategy(StrategyImpl *strategy)
{
    return SET_STRATEGY_LOADBALANCER_WITH_DELAY;
}

static void
_update_Stats(StrategyLoadBalancerWithPD *strategy, StrategyNexthopStateWithPD *state, bool inc, Ticks rtt)
{
...
_sendProbes(StrategyLoadBalancerWithPD *strategy)
{
    ...
    for (unsigned i = 0; i < size; i++) {
        ...
        Connection *conn = (Connection *) connectionTable_FindById(strategy->connTable, nhop);
        if (conn != NULL) {
...
static void
_strategyLoadBalancerWithPD_ReceiveObject(StrategyImpl *strategy, const NumberSet *egressId, const Message *objectMessage, Ticks rtt)
{
    StrategyLoadBalancerWithPD *lb = (StrategyLoadBalancerWithPD *) strategy->context;
...
static void
_strategyLoadBalancerWithPD_OnTimeout(StrategyImpl *strategy, const NumberSet *egressId)
{
    StrategyLoadBalancerWithPD *lb = (StrategyLoadBalancerWithPD *) strategy->context;
...
//ATTENTION!! This interface force us to create a NumberSet which need to be delited somewhere
//The specification in the interface requires that this function never returns NULL. in case we have no output face we need to return an empty NumberSet
static NumberSet *
_strategyLoadBalancerWithPD_LookupNexthop(StrategyImpl *strategy, const Message *interestMessage)
{
    StrategyLoadBalancerWithPD *lb = (StrategyLoadBalancerWithPD *) strategy->context;
    ...
    parcTrapNotImplemented("Try to send an interest on a face that does not exists");
    }
...
static NumberSet *
_strategyLoadBalancerWithPD_ReturnNexthops(StrategyImpl *strategy)
{
    StrategyLoadBalancerWithPD *lb = (StrategyLoadBalancerWithPD *) strategy->context;
    return lb->nexthops;
}
...
unsigned
_strategyLoadBalancerWithPD_CountNexthops(StrategyImpl *strategy)
{
    StrategyLoadBalancerWithPD *lb = (StrategyLoadBalancerWithPD *) strategy->context;
...
static void
_strategyLoadBalancerWithPD_resetState(StrategyImpl *strategy)
{
    StrategyLoadBalancerWithPD *lb = (StrategyLoadBalancerWithPD *) strategy->context;
    lb->weights_sum = 0.0;
...
static void
_strategyLoadBalancerWithPD_AddNexthop(StrategyImpl *strategy, unsigned connectionId)
{
...
static void
_strategyLoadBalancerWithPD_RemoveNexthop(StrategyImpl *strategy, unsigned connectionId)
{
    StrategyLoadBalancerWithPD *lb = (StrategyLoadBalancerWithPD *) strategy->context;
...
static void
_strategyLoadBalancerWithPD_ImplDestroy(StrategyImpl **strategyPtr)
{
    parcAssertNotNull(strategyPtr, "Parameter must be non-null double pointer");
    parcAssertNotNull(*strategyPtr, "Parameter must dereference to non-null pointer");
