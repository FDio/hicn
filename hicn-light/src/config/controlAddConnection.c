...
static CommandReturn _controlAddConnection_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlAddConnection_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);

// ===================================================

static CommandReturn _controlAddConnection_HIcnHelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlAddConnection_HIcnExecute(CommandParser *parser, CommandOps *ops, PARCList *args);

static CommandReturn _controlAddConnection_UdpHelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlAddConnection_UdpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);

static CommandReturn _controlAddConnection_TcpHelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlAddConnection_TcpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
...
CommandOps *
controlAddConnection_Create(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnection, _controlAddConnection_Init,
                                    _controlAddConnection_Execute, commandOps_Destroy);
}

CommandOps *
controlAddConnection_HelpCreate(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnectionHelp, NULL,
                                    _controlAddConnection_HelpExecute, commandOps_Destroy);
}

// ===================================================

static CommandOps *
_controlAddConnection_HIcnCreate(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnectionHIcn, NULL,
                                    _controlAddConnection_HIcnExecute, commandOps_Destroy);
}

static CommandOps *
_controlAddConnection_UdpCreate(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnectionUdp, NULL,
                                    _controlAddConnection_UdpExecute, commandOps_Destroy);
}

static CommandOps *
_controlAddConnection_TcpCreate(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnectionTcp, NULL,
                                    _controlAddConnection_TcpExecute, commandOps_Destroy);
}
...
static CommandOps *
_controlAddConnection_HIcnHelpCreate(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnectionHIcnHelp, NULL,
                                    _controlAddConnection_HIcnHelpExecute, commandOps_Destroy);
}

static CommandOps *
_controlAddConnection_UdpHelpCreate(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnectionUdpHelp, NULL,
                                    _controlAddConnection_UdpHelpExecute, commandOps_Destroy);
}

static CommandOps *
_controlAddConnection_TcpHelpCreate(ControlState *state)
{
    return commandOps_Create(state, _commandAddConnectionTcpHelp, NULL,
                                    _controlAddConnection_TcpHelpExecute, commandOps_Destroy);
}
...
_controlAddConnection_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static void
_controlAddConnection_Init(CommandParser *parser, CommandOps *ops)
{
    ControlState *state = ops->closure;
    controlState_RegisterCommand(state, _controlAddConnection_HIcnHelpCreate(state));
    controlState_RegisterCommand(state, _controlAddConnection_UdpHelpCreate(state));
    controlState_RegisterCommand(state, _controlAddConnection_TcpHelpCreate(state));
...
static CommandReturn
_controlAddConnection_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
 ...
 *
 *      control_CreateTunnel(state, localAddress, remoteAddress, IPTUN_TCP, "conn7");
 *
...
static CommandReturn
_controlAddConnection_CreateTunnel(CommandParser *parser, CommandOps *ops, const char *local_ip, const char *local_port,
                    const char *remote_ip, const char *remote_port, connection_type tunnelType, const char *symbolic)
{
    ...
    add_connection_command *addConnectionCommand = parcMemory_AllocateAndClear(sizeof(add_connection_command));

    //check and set IP addresses
    if(inet_pton(AF_INET, remote_ip, &addConnectionCommand->remoteIp.ipv4) == 1 &&
       inet_pton(AF_INET, local_ip, &addConnectionCommand->localIp.ipv4) == 1){

        addConnectionCommand->ipType = ADDR_INET;

    } else if(inet_pton(AF_INET6, remote_ip, &addConnectionCommand->remoteIp.ipv6) == 1 &&
              inet_pton(AF_INET6, local_ip, &addConnectionCommand->localIp.ipv6) == 1){
    ...
    struct iovec *response =
        utils_SendRequest(state, ADD_CONNECTION, addConnectionCommand, sizeof(add_connection_command));
...
static CommandReturn
_controlAddConnection_IpHelp(CommandParser*parser,
                                  CommandOps*ops, PARCList*args, const char*protocol)
{
...
static CommandReturn
_controlAddConnection_HIcnHelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static CommandReturn
_controlAddConnection_HIcnExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    ...
    return _controlAddConnection_CreateTunnel(parser, ops,
        local_ip, port, remote_ip, port, HICN_CONN, symbolic);

}


static CommandReturn
_controlAddConnection_UdpHelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static CommandReturn
_controlAddConnection_UdpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    ...
    return _controlAddConnection_CreateTunnel(parser, ops,
        local_ip, local_port, remote_ip, remote_port, UDP_CONN, symbolic);
}

static CommandReturn
_controlAddConnection_TcpHelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static CommandReturn
_controlAddConnection_TcpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    static const int _indexSymbolic = 3;
    static const int _indexRemAddr = 4;
    static const int _indexRemPort = 5;
    ...
    return _controlAddConnection_CreateTunnel(parser, ops,
        local_ip, local_port, remote_ip, remote_port, TCP_CONN, symbolic);
