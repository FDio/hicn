...
static CommandReturn _controlAddListener_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlAddListener_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);

static const char *command_add_listener = "add listener";
static const char *command_help_add_listener = "help add listener";

CommandOps *
controlAddListener_Create(ControlState *state)
{
    return commandOps_Create(state,
                                    command_add_listener,
                                    NULL,
                                    _controlAddListener_Execute,
                                    commandOps_Destroy);
}

CommandOps *
controlAddListener_HelpCreate(ControlState *state)
{
    return commandOps_Create(state,
                                    command_help_add_listener,
                                    NULL,
                                    _controlAddListener_HelpExecute,
                                    commandOps_Destroy);
}
...
_controlAddListener_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    printf("commands:\n");
...
static CommandReturn
_CreateListener(CommandParser *parser, CommandOps *ops, const char *symbolic,
                     const char *addr, const char *port, listener_mode mode, connection_type type)
{

    ControlState *state = ops->closure;

    //allocate command payload
    add_listener_command *addListenerCommand = parcMemory_AllocateAndClear(sizeof(add_listener_command));
    ...
    } else if(inet_pton(AF_INET6, addr, &addListenerCommand->address.ipv6) == 1){
    ...
    struct iovec *response =
        utils_SendRequest(state, ADD_LISTENER, addListenerCommand, sizeof(add_listener_command));
...
static CommandReturn
_controlAddListener_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    ...
    if((strcasecmp("hicn", protocol) == 0)){
        const char *port = "1234"; //this is a random port number that will be ignored

        //here we discard the prefix len if it exists, since we don't use it in code but we
        //let libhicn to find the right ip address.
        return _CreateListener(parser, ops, symbolic, host, port, HICN_MODE, HICN_CONN);
    }

    const char *port = parcList_GetAtIndex(args, _indexPort);

    if((strcasecmp("udp", protocol) == 0)){
        return _CreateListener(parser, ops, symbolic, host, port, IP_MODE, UDP_CONN);
    }else if((strcasecmp("tcp", protocol) == 0)){
        return _CreateListener(parser, ops, symbolic, host, port, IP_MODE, TCP_CONN);
    } else{
        _controlAddListener_HelpExecute(parser, ops, args);
        return CommandReturn_Failure;
    }
