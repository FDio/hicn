...
static CommandReturn _controlListRoutes_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlListRoutes_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
...
CommandOps *
controlListRoutes_Create(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandListRoutes,
                                    NULL,
                                    _controlListRoutes_Execute,
                                    commandOps_Destroy);
}

CommandOps *
controlListRoutes_HelpCreate(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandListRoutesHelp,
                                    NULL,
                                    _controlListRoutes_HelpExecute,
                                    commandOps_Destroy);
}
...
_controlListRoutes_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static CommandReturn
_controlListRoutes_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    ...
    for (int i = 0; i < receivedHeader->length; i++) {
        ...
        list_routes_command *listRoutesCommand = (list_routes_command *)
            (receivedPayload + (i * sizeof(list_routes_command)));

        addrString = utils_CommandAddressToString(listRoutesCommand->addressType,
                                                            &listRoutesCommand->address,
                                                            &port);

        PARCBufferComposer *composer = parcBufferComposer_Create();

        parcBufferComposer_Format(composer, "%6u %8u %70.70s %3d",
                                listRoutesCommand->connid,
                                listRoutesCommand->cost,
                                addrString,
                                listRoutesCommand->len);
