...
ControlState *
controlState_Create(void *userdata, struct iovec *(*writeRead)(ControlState *state, struct iovec *msg), bool openControllerConnetion)
{
    ControlState *state = parcMemory_AllocateAndClear(sizeof(ControlState));
    parcAssertNotNull(state, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ControlState));
...
void
controlState_Destroy(ControlState **statePtr)
{
    parcAssertNotNull(statePtr, "Parameter statePtr must be non-null");
    parcAssertNotNull(*statePtr, "Parameter statePtr must dereference t non-null");
...
void
controlState_SetDebug(ControlState *state, bool debugFlag)
{
...
bool
controlState_GetDebug(ControlState *state)
{
    parcAssertNotNull(state, "Parameter state must be non-null");
...
void
controlState_RegisterCommand(ControlState *state, CommandOps *ops)
{
    parcAssertNotNull(state, "Parameter state must be non-null");
...
struct iovec *
controlState_WriteRead(ControlState *state, struct iovec *msg)
{
    parcAssertNotNull(state, "Parameter state must be non-null");
...
static PARCList *
_controlState_ParseStringIntoTokens(const char *originalString)
{
...
CommandReturn
controlState_DispatchCommand(ControlState *state, PARCList *args)
{
    parcAssertNotNull(state, "Parameter state must be non-null");
...
int
controlState_Interactive(ControlState *state)
{
    parcAssertNotNull(state, "Parameter state must be non-null");
    char *line = NULL;
    size_t linecap = 0;
...
void
controlState_SetCommandOutput(ControlState *state, char **commandData){
    state->commandOutput = commandData;
}

void
controlState_ReleaseCommandOutput(ControlState *state, char **commandData, size_t commandLenght){
...
char **
controlState_GetCommandOutput(ControlState *state){
...
void
controlState_SetInteractiveFlag(ControlState *state, bool interactive){
    state->isInteractive = interactive;
}


bool
controlState_IsInteractive(ControlState *state){
    return state->isInteractive;
}

int
controlState_GetSockfd(ControlState *state)
{
    parcAssertNotNull(state, "Parameter state must be non-null");
    return state->sockfd;
}
...
void *
controlState_GetUserdata(ControlState *state)
{
    parcAssertNotNull(state, "Parameter state must be non-null");
