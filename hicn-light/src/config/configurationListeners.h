 ...
 * @endcode
 */
void configurationListeners_SetupAll(const Configuration *config, uint16_t port, const char *localPath);

bool configurationListeners_Remove(const Configuration *config);

//light functions

struct iovec *configurationListeners_Add(Configuration *config, struct iovec *request, unsigned ingressId);

struct iovec *configurationListeners_AddPunting(Configuration *config, struct iovec *request, unsigned ingressId);
