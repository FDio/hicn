...
static CommandReturn _controlAddRoute_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlAddRoute_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);

static const char *_commandAddRoute = "add route";
static const char *_commandAddRouteHelp = "help add route";

CommandOps *
controlAddRoute_Create(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandAddRoute,
                                    NULL,
                                    _controlAddRoute_Execute,
                                    commandOps_Destroy);
}

CommandOps *
controlAddRoute_HelpCreate(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandAddRouteHelp,
                                    NULL,
                                    _controlAddRoute_HelpExecute,
                                    commandOps_Destroy);
}
...
_controlAddRoute_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    printf("commands:\n");
...
static CommandReturn
_controlAddRoute_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    ...
    add_route_command *addRouteCommand = parcMemory_AllocateAndClear(sizeof(add_route_command));
    ...
    struct iovec *response =
        utils_SendRequest(state, ADD_ROUTE, addRouteCommand, sizeof(add_route_command));
