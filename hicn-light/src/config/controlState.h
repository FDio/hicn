 ...
 * hicnLightControl program down to the forwarder or it could be an internal function within
 * hicn-light.
 *
 */
 ...
 * controlState_Create
 *
 * Creates the global state for the Control program.  The user provides the writeRead function
 * for sending and receiving the message wrapping command arguments.  For configuration file
 * inside hicn-light, it would make direct calls to Configuration -> Dispatcher.
 *
 * @param [in] userdata A closure passed back to the user when calling writeRead.
 * @param [in] writeRead The function to write then read configuration messages to hicn-light
 *
 ...
 *      control_Root_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
 *      {
 *          printf("Root Command\n");
 ...
 *
 *      static CommandReturn
 *      control_FooBar_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
 *      {
 ...
 * @return CommandReturn_Exit the command indicates that the interactive mode should exit
 *
 * Example:
 * @code
 * <#example#>
 ...
 *
 * Prints out much more diagnostic information about what hicn-light controller is doing.
 * yes, you would make a CommandOps to set and unset this :)
 *
...
void controlState_ReleaseCommandOutput(ControlState *state, char **commandData, size_t commandLenght);
