...
static CommandReturn _controlRemoveConnection_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlRemoveConnection_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
...
CommandOps *
controlRemoveConnection_Create(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandRemoveConnection,
                                    NULL,
...
CommandOps *
controlRemoveConnection_HelpCreate(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandRemoveConnectionHelp,
                                    NULL,
...
_controlRemoveConnection_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    printf("command:\n");
...
static CommandReturn
_controlRemoveConnection_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
   ...
   if (!utils_ValidateSymbolicName(symbolicOrConnid) &&
    ...
    remove_connection_command *removeConnectionCommand = parcMemory_AllocateAndClear(sizeof(remove_connection_command));
    //fill payload
    strcpy(removeConnectionCommand->symbolicOrConnid, symbolicOrConnid);

        //send message and receive response
    struct iovec *response =
        utils_SendRequest(state, REMOVE_CONNECTION, removeConnectionCommand, sizeof(remove_connection_command));
