...
static CommandReturn _controlList_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlList_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);

static const char *_commandList = "list";
static const char *_commandListHelp = "help list";

CommandOps *
controlList_Create(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandList,
                                    _controlList_Init,
                                    _controlList_Execute,
                                    commandOps_Destroy);
}

CommandOps *
controlList_HelpCreate(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandListHelp,
                                    NULL,
                                    _controlList_HelpExecute,
                                    commandOps_Destroy);
}
...
_controlList_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static void
_controlList_Init(CommandParser *parser, CommandOps *ops)
{
    ControlState *state = ops->closure;
    controlState_RegisterCommand(state, controlListConnections_HelpCreate(state));
    //controlState_RegisterCommand(state, controlListInterfaces_HelpCreate(state));
...
static CommandReturn
_controlList_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
