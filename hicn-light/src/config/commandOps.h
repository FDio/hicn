 ...
 * @endcode
 *
 * The control_Root_Init function will then begin adding the subtree under root.  For example:
 *
 * @code
...
struct command_ops {
    ...
    CommandReturn (*execute)(struct command_parser *parser, CommandOps *ops, PARCList *args);
 ...
 * @endcode
 */
CommandOps *commandOps_Create(void *closure,
                                            const char *command,
                                            void (*init)(struct command_parser *parser,
                                                         CommandOps *ops),
