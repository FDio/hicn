...
void configuration_SetupAllListeners(Configuration *config, uint16_t port, const char *localPath);

void configuration_ReceiveCommand(Configuration *config, command_id command, struct iovec *request, unsigned ingressId);
 ...
 * @endcode
 */
void   configuration_SetObjectStoreSize(Configuration *config, size_t maximumContentObjectCount);

strategy_type configuration_GetForwardingStrategy(Configuration *config, const char *prefix);

/**
 * Returns the Forwarder that owns the Configuration
 *
 * Returns the hicn-light Forwarder.  Used primarily by associated classes in the
 * configuration group.
 *
...
struct iovec *configuration_DispatchCommand(Configuration *config, command_id command,
