...
static CommandReturn _controlAddPunting_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlAddPunting_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
...
CommandOps *
controlAddPunting_Create(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandAddPunting,
                                    NULL,
                                    _controlAddPunting_Execute,
                                    commandOps_Destroy);
}

CommandOps *
controlAddPunting_HelpCreate(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandAddPuntingHelp,
                                    NULL,
                                    _controlAddPunting_HelpExecute,
                                    commandOps_Destroy);
}
...
_controlAddPunting_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static CommandReturn
_controlAddPunting_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    ...
    add_punting_command *addPuntingCommand = parcMemory_AllocateAndClear(sizeof(add_punting_command));
    ...
    struct iovec *response =
        utils_SendRequest(state, ADD_PUNTING, addPuntingCommand, sizeof(add_punting_command));
