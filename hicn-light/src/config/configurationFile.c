...
*/
struct iovec *
_writeRead(ControlState *state, struct iovec *msg)
{
    ConfigurationFile *configFile = (ConfigurationFile *) controlState_GetUserdata(state);

    parcAssertNotNull(msg, "Parameter msg must be non-null");
    struct iovec *response = configuration_DispatchCommand(forwarder_GetConfiguration(configFile->forwarder),
                            ((header_control_message *) msg[0].iov_base)->commandID,
                            msg,
                            0);
...
static void
_destroy(ConfigurationFile **configFilePtr)
{
...
parcObject_ExtendPARCObject(ConfigurationFile, _destroy, NULL, NULL, NULL, NULL, NULL, NULL);

parcObject_ImplementRelease(configurationFile, ConfigurationFile);

ConfigurationFile *
configurationFile_Create(Forwarder *forwarder, const char *filename)
{
    ...
    if (configFile) {
        ...
        // setup the control state for the command parser: last parameter NULL because
        //writeRead still not implemented from configuration file.
        configFile->controlState = controlState_Create(configFile, _writeRead, false);
        ...
        if (configFile->fh) {
            if (logger_IsLoggable(forwarder_GetLogger(forwarder),
                                         ...
                                         PARCLogLevel_Debug)) {
                logger_Log(forwarder_GetLogger(forwarder),
                                  LoggerFacility_Config,
                                  PARCLogLevel_Debug,
                                  __func__,
                                  "Open config file %s",
...
bool
configurationFile_Process(ConfigurationFile *configFile)
{
    ...
    while (success && fgets(buffer, BUFFERLEN, configFile->fh) != NULL) {
        ...
        if (strlen(stripedBuffer) > 0) {
            if (stripedBuffer[0] != '#') {
                ...
                CommandReturn result = controlState_DispatchCommand(configFile->controlState, args);
