...
static CommandReturn _controlSetWldr_Execute(CommandParser *parser, CommandOps *ops, PARCList *args);
static CommandReturn _controlSetWldr_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args);
...
CommandOps *
controlSetWldr_Create(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandSetWldr,
                                    NULL,
                                    _controlSetWldr_Execute,
                                    commandOps_Destroy);
}

CommandOps *
controlSetWldr_HelpCreate(ControlState *state)
{
    return commandOps_Create(state,
                                    _commandSetWldrHelp,
                                    NULL,
                                    _controlSetWldr_HelpExecute,
                                    commandOps_Destroy);
}
...
_controlSetWldr_HelpExecute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
...
static CommandReturn
_controlSetWldr_Execute(CommandParser *parser, CommandOps *ops, PARCList *args)
{
    ...
    set_wldr_command *setWldrCommand = parcMemory_AllocateAndClear(sizeof(set_wldr_command));
    ...
    struct iovec *response =
        utils_SendRequest(state, SET_WLDR, setWldrCommand, sizeof(set_wldr_command));
