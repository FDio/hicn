...
size_t commandOutputLen = 0;    //preserve the number of structs composing payload in case on not interactive call.

//REMINDER: when a new_command is added, the following array has to be updated with the sizeof(new_command).
//It allows to allocate the buffer for receiving the payload of the DAEMON RESPONSE after the header has beed read.
//Each command identifier (typedef enum command_id) corresponds to a position in the following array.
static int payloadLengthController[LAST_COMMAND_VALUE] = {sizeof(add_listener_command),
...
{
    printf("Usage: %s -h\n", programName);
    printf("hicn-light is the 1.0 source, which runs on each end system and as a software source\n");
...
struct iovec *
_writeAndReadMessage(ControlState *state, struct iovec *msg)
{
    ...
    if(((header_control_message *) msg[0].iov_base)->length > 0){     //command with payload
        //write header + payload (compatibility issue: two write needed instead of the writev)
        if (write(sockfd, msg[0].iov_base, msg[0].iov_len )<0 || write(sockfd, msg[1].iov_base, msg[1].iov_len)<0){
    ...
    if(headerResponse->messageType < RESPONSE_LIGHT ||
        ...
        if (recv(sockfd, checkFinMsg, sizeof(checkFinMsg), MSG_PEEK | MSG_DONTWAIT) == 0) {
    ...
    if((commandOutputLen = headerResponse->length) > 0){
        payloadResponse = parcMemory_AllocateAndClear(payloadLengthController[headerResponse->commandID]*headerResponse->length);

        if (recv(sockfd, payloadResponse, payloadLengthController[headerResponse->commandID]*headerResponse->length,0 )<0){
            printf("\nError in Receiving the Message \n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    response[1].iov_len = payloadLengthController[headerResponse->commandID]*headerResponse->length;
...
main(int argc, char *argv[])
{
    ...
    mainState.controlState = controlState_Create(&mainState, _writeAndReadMessage, true);
    ...
    if (parcList_Size(commands) > 0) {
        ...
        char **commandOutputMain = controlState_GetCommandOutput(mainState.controlState);
        if (commandOutputMain != NULL && commandOutputLen > 0){
            for (size_t j = 0; j < commandOutputLen; j++){
                printf("Output %zu: %s \n", j, commandOutputMain[j]);
            }
            controlState_ReleaseCommandOutput(mainState.controlState, commandOutputMain, commandOutputLen);
        }
        //release
