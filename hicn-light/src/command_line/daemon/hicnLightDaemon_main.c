...
_usage(int exitCode)
{
    printf("Usage: daemon [--port port] [--daemon] [--capacity objectStoreSize] [--log facility=level] [--log-file filename] [--config file]\n");
    printf("\n");
...
static void
_setLogLevelToLevel(int logLevelArray[LoggerFacility_END], LoggerFacility facility, const char *levelString)
{
 ...
 */
static void
_setLogLevel(int logLevelArray[LoggerFacility_END], const char *string)
{
    ...
    if (facilityString) {
        ...
        if (strcasecmp(facilityString, "all") == 0) {
            for (LoggerFacility facility = 0; facility < LoggerFacility_END; facility++) {
...
_daemonize(void)
{
    ...
    parcAssertTrue(nullfile >= 0, "Error opening file '%s': (%d) %s", devnull, errno, strerror(errno));

    int ret;
    ret = dup(nullfile);
    parcAssertTrue(ret == 1, "Error duping fd 1 got %d file: (%d) %s", ret, errno, strerror(errno));
    ret = dup(nullfile);
    parcAssertTrue(ret == 2, "Error duping fd 2, got %d file: (%d) %s", ret, errno, strerror(errno));
...
main(int argc, const char *argv[])
{
    ...
    logger_Log(logger,
                      ...
                      PARCLogLevel_Alert,
                      "daemon",
                      "hicn-light running port %d configuration-port %d",
                      port,
                      configurationPort);
