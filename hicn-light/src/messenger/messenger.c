 ...
 * schedule future events.  When someone calls messenger_Send(...), it will put the
...
static void messenger_Dequeue(int fd, PARCEventType which_event, void *messengerVoidPtr);

// =========================================
// Public API

Messenger *
messenger_Create(Dispatcher *dispatcher)
{
    Messenger *messenger = parcMemory_AllocateAndClear(sizeof(Messenger));
    parcAssertNotNull(messenger, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Messenger));
    ...
    messenger->timerEvent = dispatcher_CreateTimer(dispatcher, false, messenger_Dequeue, messenger);

    return messenger;
}

void
messenger_Destroy(Messenger **messengerPtr)
{
    parcAssertNotNull(messengerPtr, "Parameter must be non-null double pointer");
    parcAssertNotNull(*messengerPtr, "Parameter must dereference to non-null pointer");
...
void
messenger_Send(Messenger *messenger, Missive *missive)
{
    ...
    if (missiveDeque_Size(messenger->eventQueue) == 1) {
        ...
        dispatcher_StartTimer(messenger->dispatcher, messenger->timerEvent, &immediateTimeout);
    }
}

static void
removeRecipient(Messenger *messenger, const MessengerRecipient *recipient)
{
 ...
 */
void
messenger_Register(Messenger *messenger, const MessengerRecipient *recipient)
{
    parcAssertNotNull(messenger, "Parameter messenger must be non-null");
 ...
 */
void
messenger_Unregister(Messenger *messenger, const MessengerRecipient *recipient)
{
    parcAssertNotNull(messenger, "Parameter messenger must be non-null");
    parcAssertNotNull(recipient, "Parameter recipient must be non-null");
 ...
 */
static void
messenger_Dequeue(int fd, PARCEventType which_event, void *messengerVoidPtr)
{
    ...
    while ((missive = missiveDeque_RemoveFirst(messenger->eventQueue)) != NULL) {
        for (size_t i = 0; i < parcArrayList_Size(messenger->callbacklist); i++) {
            MessengerRecipient *recipient = parcArrayList_Get(messenger->callbacklist, i);
