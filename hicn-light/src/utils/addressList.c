...
static void
_addressListFreeAddress(void **addressVoidPtr)
{
...
addressListCreate()
{
    ...
    parcAssertNotNull(list, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(AddressList));
...
void
addressListDestroy(AddressList **addressListPtr)
{
    parcAssertNotNull(addressListPtr, "Parameter must be non-null double pointer");
    parcAssertNotNull(*addressListPtr, "Parameter must dereference to non-null pointer");
...
AddressList *
addressListAppend(AddressList *list, Address *address)
{
...
AddressList *
addressListCopy(const AddressList *original)
{
...
bool
addressListEquals(const AddressList *a, const AddressList *b)
{
    ...
    if (parcArrayList_Size(a->listOfAddress) != parcArrayList_Size(b->listOfAddress)) {
        return false;
    }
...
size_t
addressListLength(const AddressList *list)
{
...
const Address *
addressListGetItem(const AddressList *list, size_t item)
{
    parcAssertNotNull(list, "Parameter must be non-null");
    parcAssertTrue(item < addressListLength(list), "Asked for item %zu beyond end of list %zu", item, addressListLength(list));
...
char *
addressListToString(const AddressList *list)
{
