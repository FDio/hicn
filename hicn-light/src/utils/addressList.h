 ...
 *
 * @param [in] addressListPtr A pointer to a pointer to a valid {@link AddressList}.
 *
 *
 * Example:
 * @code
 ...
 *
 * Two AddressList instances are equal if, and only if, they have the same length,
 * with the same elements in the same order.
 *
 *
 * The following equivalence relations on non-null `AddressList` instances are maintained:
 *
 *  * It is reflexive: for any non-null reference value x, `AddressList_Equals(x, x)`
 *      must return true.
 *
 ...
 *
 * @param list A pointer to a AddressList.
 * @param item A value less than the number of items in the given {@link AddressList}.
...
/**
 * Get a nul-terminated, C-string representation of the given {@link AddressList}.
 *
 * @param list A pointer to a valid {@link AddressList} instance.
 *
 * @return An allocate string representation of the {@link AddressList} that must be freed via `parcMemory_Deallocate()`.
 *
 * Example:
 * @code
 * <#example#>
