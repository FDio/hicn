...
static struct address_type_str {
    ...
    { .type = ADDR_INET,  .str = "INET"  },
    { .type = ADDR_INET6, .str = "INET6" },
    { .type = ADDR_LINK,  .str = "LINK"  },
    { .type = ADDR_IFACE, .str = "IFACE" },
    { .type = ADDR_UNIX,  .str = "UNIX"  },
    { .type = 0,                    .str = NULL    }
};

void
addressDestroy(Address **addressPtr)
{
    parcAssertNotNull(addressPtr, "Parameter must be non-null double pointer");
    parcAssertNotNull(*addressPtr, "Parameter must dereference to non-null pointer");
...
void
addressAssertValid(const Address *address)
{
    parcAssertNotNull(address, "Parameter must be non-null Address *");
}

const char *
addressTypeToString(address_type type)
{
...
address_type
addressStringToType(const char *str)
{
    for (int i = 0; addressTypeString[i].str != NULL; i++) {
...
static Address *
_addressCreate(address_type addressType, PARCBuffer *buffer)
{
    Address *result = parcMemory_AllocateAndClear(sizeof(Address));

    parcAssertNotNull(result, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Address));
    if (result != NULL) {
...
Address *
addressCreateFromInet(struct sockaddr_in *addr_in)
{
...
Address *
addressCreateFromInet6(struct sockaddr_in6 *addr_in6)
{
...
Address *
addressCreateFromLink(const uint8_t *linkaddr, size_t length)
{
...
Address *
addressCreateFromInterface(unsigned interfaceIndex)
{
...
Address *
addressCreateFromUnix(struct sockaddr_un *addr_un)
{
...
Address *
addressCopy(const Address *original)
{
    addressAssertValid(original);

    Address *result = _addressCreate(original->addressType, parcBuffer_Copy(original->blob));
    return result;
}

bool
addressEquals(const Address *a, const Address *b)
{
    if (a == b) {
        return true;
    }
...
address_type
addressGetType(const Address *address)
{
...
bool
addressGetInet(const Address *address, struct sockaddr_in *addr_in)
{
    addressAssertValid(address);
    parcAssertNotNull(addr_in, "Parameter addr_in must be non-null");

    if (address->addressType == ADDR_INET) {
        parcAssertTrue(parcBuffer_Remaining(address->blob) == sizeof(struct sockaddr_in),
...
bool
addressGetInet6(const Address *address, struct sockaddr_in6 *addr_in6)
{
    addressAssertValid(address);
    parcAssertNotNull(addr_in6, "Parameter addr_in6 must be non-null");

    if (address->addressType == ADDR_INET6) {
        parcAssertTrue(parcBuffer_Remaining(address->blob) == sizeof(struct sockaddr_in6),
                   "Address corrupted. Expected length %zu, actual length %zu",
...
bool
addressGetUnix(const Address *address, struct sockaddr_un *addr_un)
{
    addressAssertValid(address);
    parcAssertNotNull(addr_un, "Parameter addr_in6 must be non-null");

    if (address->addressType == ADDR_UNIX) {
        parcAssertTrue(parcBuffer_Remaining(address->blob) == sizeof(struct sockaddr_un),
                   "Address corrupted. Expected length %zu, actual length %zu",
...
bool
addressGetInterfaceIndex(const Address *address, uint32_t *ifidx)
{
    addressAssertValid(address);
...
PARCBuffer *
addressGetLinkAddress(const Address *address)
{
    addressAssertValid(address);
...
_Inet_BuildString(const Address *address, PARCBufferComposer *composer)
{
    addressAssertValid(address);
...
static PARCBufferComposer *
_Inet6_BuildString(const Address *address, PARCBufferComposer *composer)
{
    addressAssertValid(address);
...
static PARCBufferComposer *
_Link_BuildString(const Address *address, PARCBufferComposer *composer)
{
    addressAssertValid(address);
...
PARCBufferComposer *
addressBuildString(const Address *address, PARCBufferComposer *composer)
{
    if (address != NULL) {
...
char *
addressToString(const Address *address)
{
    addressAssertValid(address);
    ...
    parcAssertNotNull(output, "parcMemory_Allocate(%zu) returned NULL", alloc_size);
    ssize_t output_length = snprintf(output, alloc_size, "{ .type=%s, .data=%s }", addressTypeToString(address->addressType), addrstr);

    parcAssertTrue(output_length < alloc_size, "allocated size too small, needed %zd", output_length);
    parcAssertFalse(output_length < 0, "snprintf error: (%d) %s", errno, strerror(errno));

    return output;
}

PARCHashCode
addressHashCode(const Address *address)
{
    addressAssertValid(address);
