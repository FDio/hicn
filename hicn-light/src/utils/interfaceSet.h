 ...
 * @endcode
 */
Interface *interfaceSetGetByOrdinalIndex(InterfaceSet *set, size_t ordinalIndex);
 ...
 * @endcode
 */
Interface *interfaceSetGetByInterfaceIndex(const InterfaceSet *set, unsigned interfaceIndex);
 ...
 *
 * Two InterfaceSet instances are equal if, and only if, the sets contain the same elements
 * - order independent.
 * Each element is compared via <code>interfaceEquals()</code>
 *
 * The following equivalence relations on non-null `InterfaceSet` instances are maintained:
 *
 *  * It is reflexive: for any non-null reference value x, `InterfaceSet_Equals(x, x)`
 *      must return true.
 *
