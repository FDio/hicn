...
InterfaceSet *
interfaceSetCreate(void)
{
    InterfaceSet *set = parcMemory_AllocateAndClear(sizeof(InterfaceSet));
    parcAssertNotNull(set, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(InterfaceSet));
    set->listOfInterfaces = parcArrayList_Create(_destroyInterface);
    return set;
}

void
interfaceSetDestroy(InterfaceSet **setPtr)
{
...
bool
interfaceSetAdd(InterfaceSet *set, Interface *iface)
{
    ...
    for (size_t i = 0; i < length; i++) {
        Interface *listEntry = (Interface *) parcArrayList_Get(set->listOfInterfaces, i);
...
size_t
interfaceSetLength(const InterfaceSet *set)
{
    parcAssertNotNull(set, "Parameter set must be non-null");
...
Interface *
interfaceSetGetByOrdinalIndex(InterfaceSet *set, size_t ordinalIndex)
{
    parcAssertNotNull(set, "Parameter set must be non-null");
...
Interface *
interfaceSetGetByInterfaceIndex(const InterfaceSet *set, unsigned interfaceIndex)
{
    size_t length = parcArrayList_Size(set->listOfInterfaces);
    for (size_t i = 0; i < length; i++) {
        Interface *listEntry = (Interface *) parcArrayList_Get(set->listOfInterfaces, i);
 ...
 */
Interface *
interfaceSetGetByName(InterfaceSet *set, const char *name)
{
    size_t length = parcArrayList_Size(set->listOfInterfaces);
    for (size_t i = 0; i < length; i++) {
        Interface *listEntry = (Interface *) parcArrayList_Get(set->listOfInterfaces, i);
...
bool
interfaceSetEquals(const InterfaceSet *a, const InterfaceSet *b)
{
    if (a == NULL && b == NULL) {
        return true;
    }
    ...
    if (length_a == length_b) {
        for (size_t i = 0; i < length_a; i++) {
            Interface *iface_a = (Interface *) parcArrayList_Get(a->listOfInterfaces, i);

            // the set is unique by interface id, so if it exists in set b, it
            // exists there by interface id
            Interface *iface_b = interfaceSetGetByInterfaceIndex(b, interfaceGetInterfaceIndex(iface_a));
