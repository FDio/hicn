 ...
 * @return non-NULL A pointer to a static string representation of the `address_type`.
 *
 * Example:
 * @code
 * {
 ...
 *
 * @param [in] typeAsString A nul-terminated, C string representation of a `address_type`.
 *
...
/**
 * Create a new `Address` instance from an IPv4 IP address, the port is optional.
 *
 * The sockaddr_in should be filled in network byte order. The newly created instance must
 * eventually be destroyed by calling {@link addressDestroy}().
 *
 * @param [in] addr_in The `sockaddr_in` representing the IPv4 IP address with which to initialize the new `Address` instance.
 * @return A new instance of `Address` that must eventually be destroyed by calling {@link addressDestroy}().
 *
 * Example:
 * @code
 * {
...
/**
 * Create a new `Address` instance from an IPv6 IP address, the port is optional.
 *
 *
 * The sockaddr_in should be filled in network byte order. The newly created instance must
 * eventually be destroyed by calling {@link addressDestroy}().
 *
 * @param [in] addr_in6 A `sockaddr_in6` from which to initialize a new instance of Address
 * @return A new instance of `Address` that must eventually be destroyed by calling {@link addressDestroy}()
 *
 * Example:
 * @code
 ...
 *
 * The newly created instance must eventually be destroyed by calling {@link addressDestroy}().
 *
...
/**
 * Create a deep copy of an instance of a `Address`. A completely new, indedependent instance is created.
 *
 * The newly created instance must eventually be destroyed by calling {@link addressDestroy}().
 *
 * @param [in] original A pointer to a `Address` instance to be copied.
 * @return A new instance of a Address, deep copied from the `original` instance.
 *
 * Example:
 * @code
 * {
 ...
 *
 * The Address instance is deallocated, and any referenced data is also deallocated.
 ...
 *
 *
 * The following equivalence relations on non-null `Address` instances are maintained:
 *
 ...
 *
 * If the specified Address instance is of type {@link commandAddrType_INET6}, then
 * populate the supplied `struct sockaddr_in6` from the Address and return true. If the
 * Address is not of type `commandAddrType_INET6`, this function returns false.
 *
 * @param [in] address A pointer to a `Address` instance of type {@link commandAddrType_INET6}.
 * @param [in] addr_in6 A pointer to a `struct sockaddr_in6`. Must be non-NULL.
 * @return true If the Address instance is of type `commandAddrType_INET6` and `addr_in6` was filled in
 * @return false If the Address instance was not of type `commandAddrType_INET6` or `addr_in6` could not be filled in.
 *
 * @see addressGetType
 */
 ...
 *
 * If the specified `Address` instance is of type {@link commandAddrType_IFACE}, then
 * populate the supplied `uint32_t` from the Address and return true. If the
 * `Address` is not of type `commandAddrType_INET6`, this function returns false.
 *
 * @param [in] address A pointer to a `Address` instance of type {@link commandAddrType_IFACE}.
 * @param [in] interfaceIndex A pointer to a `uint32_t` to fill in. Must be non-NULL.
 * @return true If the Address instance is of type `commandAddrType_IFACE` and `interfaceIndex` was filled in.
 * @return false If the Address instance was not of type `commandAddrType_IFACE` or `interfaceIndex` could not be filled in.
 *
 * @see addressGetType
 */
 ...
 *
 * If the specified `Address` instance is of type {@link commandAddrType_LINK}, then return a pointer
 * to the {@link PARCBuffer} containing the link address. If the `Address` is not of type {@link commandAddrType_LINK},
 * then return NULL. The returned PARCBuffer pointer points to memory managed by the Address instance, and
 * does not need to be destroyed or released on its own.
 *
 * @param [in] address A pointer to a `Address` instance of type {@link commandAddrType_LINK}.
...
/**
 * Append the string representation of a `Address` to a specified `PARCBufferComposer`.
 *
 ...
 *     PARCBufferComposer *composer = addressBuildString(address, parcBufferComposer_Create());
 ...
 */
PARCBufferComposer *addressBuildString(const Address *address, PARCBufferComposer *composer);
 ...
 *
 * If commandAddrA == commandAddrB, then addressHashCode(commandAddrA) == addressHashCode(commandAddrB)
 *
 * @param [in] address A pointer to a Address instance.
