...
Punting *
puntingCreate(const char *listenerName, Address *prefix, uint32_t len)
{
    ...
    if (punting) {
        punting->symbolic = parcMemory_StringDuplicate(listenerName, strlen(listenerName));
...
void
puntingRelease(Punting **puntingPtr)
{
    parcAssertNotNull(puntingPtr, "Parameter puntingPtr must be non-null double pointer");
    parcAssertNotNull(*puntingPtr, "Parameter puntingPtr dereference to non-null pointer");
...
bool
puntingEquals(const Punting *a, const Punting *b)
{
    ...
    if ((strcmp(a->symbolic, b->symbolic) == 0) &&
        (addressEquals(a->prefix, b->prefix)) &&
        (a->len == b->len)){
        return true;
    }
...
const char *
puntingGetSymbolicName(const Punting *punting)
{
...
Address *
puntingGetAddress(const Punting *punting)
{
    parcAssertNotNull(punting, "Parameter listener must be non-null");
...
uint32_t
puntingPrefixLen(const Punting *punting)
{
    parcAssertNotNull(punting, "Parameter listener must be non-null");
    return punting->len;
}
