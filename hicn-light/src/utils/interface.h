 ...
 * @endcode
 */
Interface *interfaceCreate(const char *name, unsigned interfaceIndex, bool loopback, bool supportMulticast, unsigned mtu);
 ...
 *
 *
 * The following equivalence relations on non-null `InterfaceName` instances are maintained:
 *
 *  * It is reflexive: for any non-null reference value x, `InterfaceName_Equals(x, x)`
 *      must return true.
 *
 ...
 * @return An allocate string representation of the Interface that must be freed via parcMemory_Deallocate().
 *
 * Example:
 * @code
 * <#example#>
