...
uint32_t
utils_GetNextSequenceNumber(void)
{
    uint32_t seqnum;
    ...
    parcAssertTrue(result == 0, "Got error from pthread_mutex_unlock: %d", result);
 ...
 */
bool
utils_IsNumber(const char *string)
{
 ...
 */
bool
utils_ValidateSymbolicName(const char *symbolic)
{
    bool success = false;
...
Address *
utils_AddressFromInet(in_addr_t *addr4, in_port_t *port){
...
Address *
utils_AddressFromInet6(struct in6_addr *addr6, in_port_t *port){
...
struct iovec *
utils_CreateAck(header_control_message *header, void *payload, size_t payloadLen){
...
struct iovec *
utils_CreateNack(header_control_message *header, void *payload, size_t payloadLen){
...
char *
utils_BuildStringFromInet(in_addr_t *addr4, in_port_t *port){
...
char *
utils_BuildStringFromInet6(struct in6_addr *addr6, in_port_t *port){
...
utils_SendRequest(ControlState *state, command_id command, void* payload, size_t payloadLen){
    ...
    header_control_message *headerControlMessage  = parcMemory_AllocateAndClear(sizeof(header_control_message));
...
const char *
utils_PrefixLenToString(address_type addressType, union commandAddr *address, uint8_t *prefixLen){
