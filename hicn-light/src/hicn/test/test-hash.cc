...
#include "hash_functions/murmur.h"

#define XXH_INLINE_ALL
#include "hash_functions/xxhash.h"
}
...
TEST(HashTest, PerformanceComparisonSmallStruct) {
  ...
  // MurmurHash
  auto time_murmur = get_execution_time([&]() {
    for (int i = 0; i < N_HASHES; i++) {
      small_struct.data[0] = i;
      murmurhash(&small_struct, sizeof(small_struct_t), init_val);
    }
  });

  // XXH32
  auto time_xxhash = get_execution_time([&]() {
    for (int i = 0; i < N_HASHES; i++) {
      small_struct.data[0] = i;
      // CAVEAT: If no assignment, the hash computation is skipped
      uint32_t h = XXH32(&small_struct, sizeof(small_struct_t), init_val);
    }
  });
  ...
  std::cout << "Murmur: " << time_murmur << "ms\n";
  std::cout << "xxHash (32 bits): " << time_xxhash << "ms\n";

  EXPECT_TRUE(time_jenkins < time_fnv);
}
...
TEST(HashTest, PerformanceComparisonBigStruct) {
  ...
  auto time_murmur = get_execution_time([&]() {
    for (int i = 0; i < N_HASHES; i++) {
      big_struct.data[0] = i;
      murmurhash(&big_struct, sizeof(big_struct_t), init_val);
    }
  });
  ...
  auto time_xxhash = get_execution_time([&]() {
    for (int i = 0; i < N_HASHES; i++) {
      big_struct.data[0] = i;
      // CAVEAT: If no assignment, the hash computation is skipped
      uint32_t h = XXH32(&big_struct, sizeof(big_struct_t), init_val);
    }
  });
...
TEST(HashTest, CollisionsComparison) {
  ...
  for (int i = 0; i < N_HASHES; i++) {
    small_struct.data[0] = i;
    uint32_t h = murmurhash(&small_struct, sizeof(small_struct_t), init_val);

    if (hashes.find(h) != hashes.end()) n_collisions_murmur++;
    hashes.insert(h);
  }
  ...
  for (int i = 0; i < N_HASHES; i++) {
    ...
    if (hashes.find(h) != hashes.end()) n_collisions_xxhash++;
    hashes.insert(h);
  }
  ...
  std::cout << "Murmur: " << n_collisions_murmur << " collision/s\n";
  std::cout << "xxHash (32 bits): " << n_collisions_xxhash << " collision/s\n";

  EXPECT_TRUE(n_collisions_jenkins < n_collisions_fnv);
}