...
uint8_t *
configuration_on_route_remove(Configuration * config, uint8_t * packet,
        unsigned ingress_id)
{
    assert(config);
    assert(packet);
...
uint8_t *
configuration_on_route_list(Configuration * config, uint8_t * packet,
        unsigned ingress_id)
{
    assert(config);
    assert(packet);
    ...
    // XXX TODO iterator !
    fib_entry_list_t *fibList = forwarder_get_fib_entries(config->forwarder);
    ...
    for (size_t i = 0; i < fib_size; i++) {
        ...
        NameBitvector *prefix = name_GetContentName(fib_entry_get_prefix(fib_entry));
...
strategy_type_t
configuration_get_strategy(Configuration * config, const char *prefix)
{
...
uint8_t *
configuration_on_punting_add(Configuration * config, uint8_t * packet,
        unsigned ingress_id)
{
...
uint8_t *
configuration_on_mapme_set_discovery(Configuration * config, uint8_t * packet,
        unsigned ingress_id)
{
    assert(config);
    assert(packet);
...
uint8_t *
configuration_on_policy_remove(Configuration * config, uint8_t * packet,
        unsigned ingress_id)
{
    assert(config);
    assert(packet);
...
uint8_t *
configuration_on_policy_list(Configuration * config, uint8_t * packet,
        unsigned ingress_id)
{
    assert(config);
    assert(packet);
    ...
    for (size_t i = 0; i < n; i++) {
        const fib_entry_t * entry = fib_entry_list_get(fibList, i);
...
size_t
configuration_content_store_get_size(Configuration * config)
{
    return config->maximumContentObjectStoreSize;
}

void
configuration_content_store_set_size(Configuration * config, size_t size)
{
...
forwarder_t *
configuration_get_forwarder(const Configuration * config) {
...
void configuration_receive_command(Configuration * config,
