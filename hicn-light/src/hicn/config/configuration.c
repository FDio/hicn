...
configuration_on_connection_add(Configuration *config, uint8_t * packet)
{
    ...
    forwarder_on_connection_event(config->forwarder, conn, event);
#endif /* WITH_MAPME */
        ...
        unsigned ingress_id)
{
    ...
    if (!forwarder_add_or_update_route(config->forwarder, &prefix, ingress_id))
        goto NACK;
...
uint8_t *
configuration_on_content_store_set_store(Configuration *config, uint8_t * packet)
{
    assert(config);
    assert(packet);

    msg_content_store_set_store_t * msg = packet;
    cmd_content_store_set_store_t * control = &msg->payload;

    if ((control->activate != 0) && (control->activate != 1))
        goto NACK;
    bool value = (bool)control->activate;

    forwarder_content_store_set_store_flag(config->forwarder, value);
    /* XXX Why do we need to check ? */
    if (forwarder_content_store_get_store_flag(config->forwarder) != value)
        goto NACK;
...
uint8_t *
configuration_on_content_store_set_serve(Configuration *config, uint8_t * packet)
{
    assert(config);
    assert(packet);

    msg_content_store_set_serve_t * msg = packet;
    cmd_content_store_set_serve_t * control = &msg->payload;

    if ((control->activate != 0) && (control->activate != 1))
        goto NACK;
    bool value = (bool)control->activate;

    forwarder_content_store_set_serve_flag(config->forwarder, value);
    /* XXX Why do we need to check ? */
    if (forwarder_content_store_get_serve_flag(config->forwarder) != value)
        goto NACK;
...
uint8_t *
configuration_on_content_store_clear(Configuration *config, uint8_t * packet)
{
    assert(config);
    assert(packet);
...
void configuration_content_store_set_size(Configuration *config,
...
Forwarder *configuration_get_forwarder(const Configuration *config) {
...
uint8_t *
configuration_dispatch_command(Configuration *config, command_type_t command_type,
        ...
        command_type_t command_type, uint8_t * packet, unsigned conn_id)
{
    ...
    uint8_t * reply = configuration_dispatch_command(config, command_type, reply, conn_id);
    if (!reply) {
        reply = packet;
        make_nack(reply);
        nack = true;
