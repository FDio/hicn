 ...
 * @endcode
 */
Configuration * configuration_create(struct forwarder_s * forwarder);
...
void configuration_setup_all_listeners(Configuration *config, uint16_t port,
                                     const char *localPath);

void configuration_receive_command(Configuration *config, command_type_t command,
 ...
 * @endcode
 */
size_t configuration_content_store_get_size(Configuration *config);
 ...
 * @endcode
 */
void configuration_content_store_set_size(Configuration *config, size_t size);

strategy_type_t configuration_get_strategy(Configuration *config, const char *prefix);
 ...
 * @endcode
 */
struct forwarder_s * configuration_get_forwarder(const Configuration *config);
