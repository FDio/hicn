...
process_interest(forwarder_t * forwarder, listener_t * listener,
        connection_t * connection, uint8_t * packet, size_t size, address_pair_t * pair)
{
    if (!connection) {
        ...
        connection = listener_create_connection(listener, &pair);
    }
...
process_packet(forwarder_t * forwarder, listener_t * listener, uint8_t * packet, size_t size, address_pair_t * pair)
{
    ...
    if (messageHandler_IsTCP(packet)) {
        if (messageHandler_IsData(packet)) {
            process_data(forwarder, listener, conn, packet, size, pair);
            return;
        } else if (messageHandler_IsInterest(packet)) {
            process_interest(forwarder, listener, conn, packet, size, pair);
        } else {
            continue;
        }
        ...
        /* Hooks should not be processed during batch XXX */
        ...
        bb->commands[commands] = packet;
        bb->commands_conn_id[commands++] = conn_id;
        continue;
    }

    size_t length = messageHandler_GetTotalPacketLength(packet);
    msgbuf_from_packet(&bb->messages[valid], packet, length, pktType, conn_id, ticks_now());
    bb->messages_conn[valid] = conn;
    valid++;
   ...
   forwarder_handle_hooks(packet, size, conn_id);
            bool processed = forwarder_handle_hooks(listener->forwarder, packet, size, listener, fd, conn_id, &pair);
    // Still we don't create the connection here as we do not always need to
}

/* Generic callback */
static
void
_read_callback(forwarder_t * forwarder, listener_t * listener, int fd, address_t * local_addr, batch_buffer_t * bb)
{
    assert(bb);
    ...
    for (int i = 0; i < r; i++) {
        ...
        size_t size = msg->msg_hdr.msg_iov_len;
