...
static
int
listener_tcp_punt(listener_t * listener, const char * prefix_s)
{
    return -1;
}
...
static void _conn_readcb(PARCEventQueue *events, PARCEventType type,
        void *ioOpsVoid);

static void _conn_eventcb(PARCEventQueue *events,
...
int
connection_tcp_accept(connection_t * connection,
        Forwarder *forwarder, int fd, address_pair_t *pair, bool isLocal,
        unsigned connection_id)
{
    ...
    *connection = {
        ...
        .forwarder = forwarder,
        .events = parcEventQueue_Create( eventBase, fd,
            PARCEventQueueOption_CloseOnFree | PARCEventQueueOption_DeferCallbacks),
        .closed = false,
    };

    parcEventQueue_SetCallbacks(stream->events, _conn_readcb, NULL,
            _conn_eventcb, connection);
    parcEventQueue_Enable(stream->events, PARCEventType_Read);

    //char *str = pair_ToString(udp->pair);
    INFO("%s connection %p created for address %s (local=%s)",
            connection_type_str(connection->type), udp, "N/A",
...
static
int
connection_tcp_initialize(connection_t * connection, void **pdata)
{
    assert(connection);
    assert(connection->type = CONNECTION_TYPE_TCP);
    assert(connection->forwarder);
    assert(pair);
    assert(address_pair_valid(pair));
    assert(forwarder);

    connection_tcp_data_t * data = malloc(sizeof(connection_tcp_data_t));
    if (!data)
        return -1;
    *pdata = data;
    ...
    connection->fd = make_socket(pair);
    if (listener->fd < 0) {
        ERROR("Error creating TCP socket");
        return -1;
    }

    // XXX remove the need for event
    Dispatcher * dispatcher = forwarder_GetDispatcher(connection->forwarder);
    connection->event = dispatcher_CreateNetworkEvent(dispatcher,
            true, _readcb, (void*)connection, connection->fd);
    dispatcher_StartNetworkEvent(dispatcher, connection->event);

    //char *pair_str = address_pair_ToString(pair);
    INFO("%s connection %p connect for address pair %s",
            connection_type_str(connection->type), connection, "N/A");
    //free(pair_str);
...
{
    parcEventQueue_Destroy(connection->events);
    ...
    INFO("%s connection %p destroyed", connection_type_str(connection->type),
            connection);
...
static
void
connection_tcp_send_packet(const connection_t * connection, const uint8_t packet)
{
...
static
void
connection_tcp_read_callback(int fd, PARCEventType what, void * connection_void)
{
    assert(!!(what & PARCEventType_Read));
    assert(connection_void);

    connection_t * connection = (connection_t *)connection_void;
    ...
    for (;;) {
        ssize_t n = recv(data->fd, data->buf + data->woff,
        ...
        ssize_t used = connection_process_buffer(connection, packet, size);
        if (used < 0)
            return; // XXX close connection ?
        if (used == 0)
