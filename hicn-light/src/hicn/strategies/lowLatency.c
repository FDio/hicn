...
static void _strategyLowLatency_ImplDestroy(StrategyImpl **strategyPtr) {
  ...
  PARCIterator *it2 = parcHashMap_CreateKeyIterator(strategy->pending_probes);
  while (parcIterator_HasNext(it2)) {
    PARCUnsigned *cid = parcIterator_Next(it2);
    PARCUnsigned *state =
        (PARCUnsigned *)parcHashMap_Get(strategy->pending_probes, cid);
    parcUnsigned_Release(&state);
  }
  parcIterator_Release(&it2);
