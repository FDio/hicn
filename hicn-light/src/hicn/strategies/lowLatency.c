...
static void strategyLowLatency_SendProbesCB(int fd, PARCEventType which_event,
  ...
  //TODO
  //use the ll->strategy_state instead of the nexthop list in the fibEntry
  ...
  for (unsigned i = 0; i < size; i++) {
    ...
    PARCUnsigned *conn_id = parcUnsigned_Create(nhop);
    parcHashMap_Put(ll->pending_probes_ticks, parc_seq, parc_time);
    parcHashMap_Put(ll->pending_probes_faces, parc_seq, conn_id);
    StrategyNexthopStateLL *state =
        (StrategyNexthopStateLL *) parcHashMap_Get(ll->strategy_state, conn_id);
    strategyNexthopStateLL_SentProbe(state);
    parcUnsigned_Release(&parc_seq);
    parcUnsigned_Release(&parc_time);
    parcUnsigned_Release(&conn_id);
  }
