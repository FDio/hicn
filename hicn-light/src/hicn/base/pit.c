...
pit_on_interest(pit_t * pit, msgbuf_t * interest_msgbuf)
{
    ...
    assert(msgbuf_get_type(interest_msgbuf) == MessagePacketType_Interest);
    ...
    Ticks expiry_time = _pit_calculate_lifetime(pit, interest_msgbuf);
    ...
    if (pit_entry_ingress_contains(entry, connection_id)) {
        ...
        // forward it.
        DEBUG("Message %p existing entry (expiry %" PRIu64
...
TIMEOUT:
    fib_entry_t * fib_entry = pit_entry_fib_entry(entry);
    if (fib_entry)
        fib_entry_on_timeout(fib_entry, pit_entry_egress(entry));

    // it's an old entry, remove it
    khiter_t k = kh_get(pit_name, pit->index_by_name, msgbuf_name(interest_msgbuf));
    if (k != kh_end(pit->index_by_name))
...
NOT_FOUND:
    ...
    msgbuf_t *key = message_Acquire(interest_msgbuf);

    Ticks expiry_time = _pit_calculate_lifetime(pit, interest_msgbuf);

    pit_entry_t * pitEntry =
        pit_entry_create(key, expiry_time, ticks_now());

    parcHashCodeTable_Add(pit->table, key, pitEntry);

    DEBUG("Message %p added to PIT (expiry %" PRIu64 ") ingress %u",
            interest_msgbuf, pitEntry_GetExpiryTime(pitEntry),
...
nexthops_t *
pit_on_data(pit_t * pit, const msgbuf_t * msgbuf)
{
    assert(pit);
    assert(msgbuf);
    ...
    /* Lookup entry by name */
    khiter_t k = kh_get_name(pit->index_by_name, msgbuf_name(msgbuf));
    if (k == kh_end(pit->index_by_name))
        goto NOT_FOUND;
    ...
    Ticks now = ticks_now();
    if (now >= pit_entry_expiry_time(entry))
        goto TIMEOUT;

    /* PIT entry is not expired, use it */
    fib_entry_t * fib_entry = pit_entry_fib_entry(entry);
    if (fib_entry)
        fibEntry_ReceiveObjectMessage(fibEntry,
                pitEntry_GetEgressSet(pitEntry), objectMessage,
    ...
    nexthops = pit_entry_ingress(entry);
...
void
pit_remove(pit_t * pit, const msgbuf_t * msgbuf)
{
    assert(pit);
    assert(msgbuf);
    assert(msgbuf_type(msgbuf) == MessagePacketType_Interest);

    khiter_t k = kh_get(pit_name, pit->index_by_name, msgbuf_name(msgbuf));
    if (k == kh_end(pit->index_by_name))
        return;
...
pit_entry_t *
pit_lookup(const pit_t * pit, const msgbuf_t * msgbuf)
{
    assert(pit);
    assert(msgbuf);
    assert(msgbuf_type(msgbuf_type) == MessagePacketType_Interest);
