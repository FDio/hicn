...
connection_t *
connection_create_on_listener(const listener_t * listener, face_type_t type, const char * name,
        const address_pair_t * pair, forwarder_t * forwarder)
{
    ...
    int fd = listener_get_socket(listener, pair);
    bool local = address_is_local(&pair->local);

    return connection_initialize(connection, type, name, interface_name, fd, pair, local, connection_id);
}
...
connection_initialize(connection_t * connection, face_type_t type,
        ...
        const address_pair_t * pair, bool local, unsigned connection_id,
...
connection_send(const connection_t * connection, msgbuf_t * msgbuf, bool queue)
{
    ...
    if (msgbuf_type(msgbuf) == MessagePacketType_ContentObject) {
        ...
        msgbuf_update_pathlabel(message, conn_id);
    }

    if (conn->wldr)
        wldr_set_label(conn->wldr, message);
    else
        msgbuf_reset_wldr_label(msgbuf);

    return _connection_send(connection, message, queue);
}
        ...
        notification)
{
    ...
    if (msgbuf_get_type(msgbuf) == MessagePacketType_ContentObject) {
      uint8_t conn_id = (uint8_t)connection_get_id(connection);
...
bool connection_SendIOVBuffer(const connection_t * conn, struct iovec *msg,
    size_t size) {
...
void connection_HandleProbe(connection_t * conn, uint8_t *probe){
  parcAssertNotNull(conn, "Parameter conn must be non-null");
...
void
connection_detect_losses(const connection_t * connection, msgbuf_t * msgbuf)
{
    if (!connection->wldr)
        return;
...
void
connection_handle_notification(const connection_t * connection, msgbuf_t * msgbuf)
{
    if (!connection->wldr)
        return;
