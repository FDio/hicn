...
#define msgbuf_from_packet(msgbuf, packet, type, connid, recv_time, logger_)     \
do {                                                                            \
  *msgbuf = (msgbuf_t) {                                                        \
    .logger = logger_Acquire(logger_),                                           \
    .receiveTime = (recv_time),                                                 \
    .connection_id = (connid),                                                  \
    .messageHead = (packet),                                                    \
    .length = messageHandler_GetTotalPacketLength(packet),                      \
    .packetType = (type),                                                       \
    .name = (type != MessagePacketType_WldrNotification                         \
        ? name_CreateFromPacket((packet), (type))                               \
        : NULL),                                                                \
  };                                                                            \
} while(0)

#define msgbuf_name(msgbuf) ((msgbuf)->name)
#define msgbuf_connection_id(msgbuf) ((msgbuf)->connection_id)
#define msgbuf_type(msgbuf) ((msgbuf)->packetType)
#define msgbuf_has_wldr(msgbuf) (messageHandler_HasWldr((msgbuf)->messageHead))
#define msgbuf_len(msgbuf) ((msgbuf)->length)
#define msgbuf_packet(msgbuf) ((msgbuf)->messageHead)

#define msgbuf_interest_lifetime(msgbuf) (forwarder_NanosToTicks(messageHandler_GetInterestLifetime((msgbuf)->messageHead) * 1000000ULL))
