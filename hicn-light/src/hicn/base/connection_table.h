...
#define connection_table_allocate(table, conn_ptr, pair)                        \
do {                                                                            \
    pool_get(table->connections, conn_ptr);                                     \
    off_t connection_id = conn_ptr - table->connections;                        \
    int res;                                                                    \
...
#define connection_table_get_connection_id(table, conn_ptr) \
    (conn_ptr - table->connections)

#define connection_table_foreach(table, conn_ptr, BODY) \
    pool_foreach(table->connections, conn_ptr, BODY)

connection_table_t * connection_table_create();
void connection_table_free(connection_table_t * table);

Connection ** connection_table_lookup(const connection_table_t * table,
