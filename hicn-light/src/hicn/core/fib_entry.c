...
// XXX TODO DUPLICATE
void
fib_entry_set_strategy(fib_entry_t *fib_entry, strategy_type_t strategy_type,
        ...
        unsigned ingress_id, bool prefer_local)
{
    ...
    connection_t *  conn = NULL; // XXX
    unsigned nexthop, i;
    uint_fast32_t flags;

    policy_t policy = fib_entry_get_policy(fib_entry);
 ...
 */
nexthops_t *
fib_entry_get_available_next_hops(fib_entry_t *fib_entry, unsigned in_connection, nexthops_t * new_nexthops)
{
    connection_table_t * table = forwarder_get_connection_table(fib_entry->forwarder);
...
void
fib_entry_set_policy(fib_entry_t *fib_entry, policy_t policy)
{
    fib_entry->policy = policy;
    mapme_reconsiderfib_entry_t(forwarder_get_mapme(fib_entry->forwarder), fib_entry);
}
...
const nexthops_t *
fib_entry_nexthops_get_from_strategy(fib_entry_t * fib_entry,
...
void
fib_entry_receive_data(fib_entry_t *fib_entry,
...
void
fib_entry_on_timeout(fib_entry_t *fib_entry, const nexthops_t * nexthops)
{
    parcAssertNotNull(fib_entry, "Parameter fib_entry must be non-null");
...
void fib_entry_set_user_data(fib_entry_t *fib_entry, const void *userData,
