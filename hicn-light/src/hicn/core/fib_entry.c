 ...
 */
nexthops_t *
fib_entry_nexthops_get_available(fib_entry_t * entry, unsigned ingress_id, nexthops_t * new_nexthops)
{
    ...
    if (ingress_id == ~0) {
        ...
        connection_table_foreach(table, connection, {
            new_nexthops->elts[nexthops_len(new_nexthops)] = connection_table_get_connection_id(table, connection);
    ...
    return fib_entry_filter_nexthops(entry, fib_entry_nexthops(entry), ingress_id, true);
}
...
{
    entry->policy = policy;
    mapme_reconsider_fib_entry(forwarder_get_mapme(entry->forwarder), entry);
}
...
{
    nexthops_add(fib_entry_nexthops(entry), nexthop);
    // XXX TODO
...
{
    nexthops_remove(fib_entry_nexthops(entry), nexthop);
    // XXX TODO
    strategy_vft[entry->strategy.type]->remove_nexthop(&entry->strategy, nexthop, NULL);
}

const nexthops_t *
fib_entry_nexthops_get_from_strategy(fib_entry_t * entry,
        const msgbuf_t * msgbuf, bool is_retransmission)
{
    ...
    nexthops_t * nexthops = fib_entry_nexthops_get_available(entry,
...
void
fib_entry_receive_data(fib_entry_t * entry,
