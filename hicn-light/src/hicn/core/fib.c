...
fib_node_free(fib_node_t * node)
{
    ...
    // NOTE: Acquired in forwarder_add_or_update_route at creation
    fib_entry_release(&node->entry);
        ...
        fib_entry_list_t * list)
{
    ...
    fib_node_remove_connection_id(node->right, conn_id, list);
    fib_node_remove_connection_id(node->left, conn_id, list);
}
...
fib_remove_connection_id(fib_t * fib, unsigned conn_id)
{
    ...
    fib_node_remove_connection_id(fib->root, conn_id, list);

    for (int i = 0; i < fib_entry_list_length(list); i++)
        fib_node_remove(fib, fib_entry_get_prefix(fib_entry_list_get(list, i)));
...
static
void
fib_node_collect_entries(fib_node_t * node, fib_entry_list_t * list)
{
    assert(list);
    ...
    if(node->is_used)
        fib_entry_list_append(list, node->entry);

    fib_node_collect_entries(node->right, list);
    fib_node_collect_entries(node->left, list);
}

fib_entry_list_t *
fib_get_entries(const fib_t * fib)
{
    assert(fib);

    fib_entry_list_t * list = fib_entry_list_create();
    fib_node_collect_entries(fib->root, list);
    return list;
}
