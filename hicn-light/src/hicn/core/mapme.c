...
static mapme_t mapme_default = {
    ...
    .Tu = MAPME_DEFAULT_TU,
    .remove_fib_entries = false,
};

/******************************************************************************/

bool mapme_create(mapme_t **mapme, void *forwarder) {
  *mapme = malloc(sizeof(mapme_t));
...
static mapme_tfib_t *mapmeTFIB_Create() {
  mapme_tfib_t *tfib;
...
void mapmeTFIB_Release(mapme_tfib_t **tfibPtr) {
...
#define TFIB(fib_entry) ((mapme_tfib_t *)fib_entry_getUserData(fib_entry))

static const PARCEventTimer *mapmeTFIB_Get(const mapme_tfib_t *tfib,
                                           unsigned conn_id) {
...
static void mapmeTFIB_Put(mapme_tfib_t *tfib, unsigned conn_id,
                          const PARCEventTimer *timer) {
...
static void mapmeTFIB_Remove(mapme_tfib_t *tfib, unsigned conn_id) {
...
static PARCIterator *mapmeTFIB_CreateKeyIterator(const mapme_tfib_t *tfib) {
...
static
int
mapme_createMessage(const mapme_t *mapme, const Name *name,
...
static
int
mapme_createAckMessage(const mapme_t *mapme, const uint8_t *msgBuffer, const
        mapme_params_t *params, msgbuf_t * msgbuf)
{
  Ticks now = forwarder_GetTicks(mapme->forwarder);
...
static bool mapme_setFacePending(const mapme_t *mapme, const Name *name,
 ...
 * interest header.
 */
static hicn_mapme_type_t mapme_getTypeFromHeuristic(const mapme_t *mapme,
                                                    fib_entry_t *fib_entry) {
 ...
 */
static bool mapme_hasLocalNextHops(const mapme_t *mapme,
...
void
mapme_send_updates(const mapme_t * mapme, fib_entry_t * fib_entry, const nexthops_t * nexthops)
{
...
void
mapme_maybe_send_updates(const mapme_t * mapme, fib_entry_t * fib_entry, const nexthops_t * nexthops)
{
  /* Detect change */
...
void
mapme_reconsider_fib_entry(const mapme_t *mapme, fib_entry_t * fib_entry)
{
  /*
 ...
 */
void
mapme_on_connection_event(const mapme_t *mapme, const Connection *conn_added, connection_event_t event) {
  ...
  fib_foreach(fib, entry, {
    mapme_reconsider_fib_entry(mapme, entry);
  };
 ...
 */
static bool mapme_onSpecialInterest(const mapme_t *mapme,
...
void mapme_onSpecialInterestAck(const mapme_t *mapme, const uint8_t *msgBuffer,
                                unsigned conn_in_id, hicn_prefix_t *prefix,
                                mapme_params_t *params) {
