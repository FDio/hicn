...
int mapme_create_fib_entry(const mapme_t *mapme, const hicn_prefix_t *prefix,
        unsigned ingress_id) {
   ...
   */
  configuration_t *config = forwarder_get_configuration(mapme->forwarder);
  ...
  if (!entry) {
    entry = fib_entry_create(&prefix, strategy_type, NULL, forwarder);

  fib_entry_t *lpm = fib_match_prefix(forwarder->fib, prefix);
  ...
  nexthops_foreach(lpm_nexthops, nh, {
    fib_entry_nexthops_add(entry, nh);
  });
  return 0;
}
#endif
