...
#define LOG(log_level, fmt, ...)                                                \
  do {                                                                          \
    ...
    if (logger_IsLoggable(logger, LOG_FACILITY, log_level)) {                   \
      logger_Log(logger, LOG_FACILITY, log_level, __func__, "[MAP-Me] " fmt,    \
                 ##__VA_ARGS__);                                                \
    }                                                                           \
  } while (0)

#define WARN(fmt, ...) LOG(PARCLogLevel_Warning, fmt, ##__VA_ARGS__)
#define ERR(fmt, ...) LOG(PARCLogLevel_Error, fmt, ##__VA_ARGS__)
#define INFO(fmt, ...) LOG(PARCLogLevel_Info, fmt, ##__VA_ARGS__)
#define DEBUG(fmt, ...) LOG(PARCLogLevel_Debug, fmt, ##__VA_ARGS__)
        ...
        mapme_params_t *params, msgbuf_t * msgbuf)
{
  ...
  if (rc < 0) {
    ERR("Failed to create lib's name");
    goto ERR_NAME;
  }
  ...
  if (len == 0) {
    ERR("Failed to create mapme packet through lib");
    goto ERR_CREATE;
  }
  ...
  msgbuf_from_packet(msgbuf, icmp_pkt, MessagePacketType_Interest,
          CONNECTION_ID_INVALID, now, logger);
  return 0;
        ...
        mapme_params_t *params, msgbuf_t * msgbuf)
{
  ...
  if (len != size) {
    ERR("Failed to create mapme ack packet through lib");
    return -1;
  }
