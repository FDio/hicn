...
typedef struct {
    ...
    uint32_t countDroppedconnection_tNotFound;
...
{
    ConfigurationFile *cfg = configurationFile_Create(forwarder, filename);
    if (!cfg)
        return;

    configurationFile_Process(cfg);
    configurationFile_Release(&cfg);
}
...
{
    return messageProcessor_GetCacheStoreFlag(forwarder->forwarder);
    assert(forwarder);
}
...
static
bool
forwarder_forward_via_fib(forwarder_t * forwarder,
        msgbuf_t *msgbuf, pit_verdict_t verdict)
{
    ...
    fib_entry_t *fib_entry = fib_MatchMessage(forwarder->fib, msgbuf);
    if (!fib_entry)
        return false;

    // XXX TODO PROBE HOOK MIGHT BE HANDLED ELSEWHERE
    if (msgbuf_is_probe(msgbuf)) {
        connection_table_t * table = forwarder_GetConnectionTable(forwarder->forwarder);
...
_satisfy_from_content_store(forwarder_t * forwarder, msgbuf_t *interest_msgbuf)
{
    ...
    if (msgbuf_interest_lifetime(interest_msgbuf) == 0)
        return false;

    if (!forwarder->serve_from_cache)
        return false;

    // See if there's a match in the store.
    msgbuf_t * data_msgbuf = content_store_match(forwarder->content_store,
            interest_msgbuf, forwarder_GetTicks(forwarder->forwarder));
        ...
        msgbuf_t *msgbuf)
{
    ...
    if (!ingressSetUnion) {
        ...
        if (msgbuf_is_probe(msgbuf)) {
            fib_entry_t *fibEntry = fib_MatchMessage(forwarder->fib, msgbuf);
            if(fibEntry && fib_entry_strategy_type(fibEntry) == STRATEGY_TYPE_LOW_LATENCY) {
                ...
                fib_entry_ReceiveObjectMessage(fibEntry, &probe_nexthops, msgbuf, 0,
                        forwarder_GetTicks(forwarder->forwarder));
        ...
        const connection_table_t * table = forwarder_GetConnectionTable(forwarder->forwarder);
        const Connection * conn = connection_table_get_by_id(table, msgbuf_connection_id(msgbuf));

        if (forwarder->store_in_cache && connection_IsLocal(conn)) {
            uint64_t now = forwarder_GetTicks(forwarder->forwarder);
            content_store_add(forwarder->content_store, msgbuf, now);
...
void
forwarder_receive(forwarder_t * forwarder, msgbuf_t * message, unsigned new_batch)
{
    assert(forwarder);
    assert(msgbuf);
    ...
    connection_table_t * table = forwarder_Getconnection_tTable(forwarder);
    const connection_t *conn = connection_table_get_by_id(table, msgbuf_connection_id(msgbuf));
    if (!conn)
        return;
    ...
    if (msgbuf_has_wldr(msgbuf)) {
        ...
        if (connection_HasWldr(conn) && connection_WldrAutoStartAllowed(conn)) {
            ...
            connection_DisableWldr((connection_t *)conn);
        }
    }
    ...
    DEBUG( "Message %p ingress %3u length %5u received name %s", message,
            msgbuf_connection_id(message), msgbuf_len(message), nameString);
    free(nameString);

    switch (msgbuf_type(message)) {
        case MessagePacketType_Interest:
            forwarder_receive_interest(forwarder, message);
            break;

        case MessagePacketType_ContentObject:
            forwarder_receive_data(forwarder, message);
            break;

        default:
            forwarder_drop(forwarder, message);
            break;
    }

    /* Send batch ? */
    if (forwarder->pending_batch == 0) {
        const connection_table_t * table = forwarder_get_connection_table(forwarder->forwarder);
        for (unsigned i = 0; i < forwarder->num_pending_conn; i++) {
            const Connection * conn = connection_table_at(table, forwarder->pending_conn[i]);
            // flush
        ...
        unsigned ingress_id)
{
    ...
    Configuration *config = forwarder_GetConfiguration(forwarder);
    ...
    strategy_type_t strategy_type = configuration_GetForwardingStrategy(config, prefix_s);

    Name * name_prefix = name_CreateFromAddress(name_prefix->family,
            &name_prefix->address, name_prefix->len);
    fib_entry_t * entry = fib_Contains(forwarder->fib, name_prefix);
    if (!entry) {
        entry = fib_entry_Create(name_prefix, strategy_type, NULL, forwarder->forwarder);
        ...
        unsigned ingress_id)
{
    ...
    Name *name_prefix = name_prefix_CreateFromAddress(prefix->family,
        ...
        policy_t * policy)
{
    ...
    if (!entry)
        return false;
    fib_entry_SetPolicy(entry, policy);
...
forwarder_remove_policy(forwarder_t * forwarder, ip_prefix_t * prefix)
{
    ...
    Name *name_prefix = name_CreateFromAddress(control->family, control->address,
            control->len);
    fib_entry_t *entry = fib_Contains(forwarder->fib, name_prefix);
    name_Release(&name_prefix);
        ...
        unsigned connection_id)
{
    ...
    fib_RemoveConnectionId(forwarder->fib, connection_id);
}
...
fib_entry_list_t *
forwarder_get_fib_entries(forwarder_t * forwarder)
{
    assert(forwarder);

    return fib_GetEntries(forwarder->fib);
}
...
void
forwarder_process_mapme(const forwarder_t * forwarder, const uint8_t * packet,
        unsigned conn_id)
{
 ...
 */
bool
forwarder_handle_hooks(const forwarder_t * forwarder, const uint8_t * packet,
        listener_t * listener, int fd, unsigned conn_id, address_pair_t * pair)
{
    ...
    if (conn_id == ~0)
        conn_id = listener->create_connection(listener, fd, pair);

    /* BEGIN Process */

#ifdef WITH_MAPME
    if (is_mapme)
        forwarder_process_mapme(forwarder, packet, conn_id);
#endif /* WITH_MAPME */
