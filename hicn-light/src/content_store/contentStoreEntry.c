...
ContentStoreEntry *
contentStoreEntry_Create(Message *contentMessage, ListLru *listLRU)
{
    parcAssertNotNull(contentMessage, "Parameter objectMessage must be non-null");

    ContentStoreEntry *entry = parcMemory_AllocateAndClear(sizeof(ContentStoreEntry));
    parcAssertNotNull(entry, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ContentStoreEntry));
...
ContentStoreEntry *
contentStoreEntry_Acquire(const ContentStoreEntry *original)
{
...
void
contentStoreEntry_Release(ContentStoreEntry **entryPtr)
{
    parcAssertNotNull(entryPtr, "Parameter must be non-null double pointer");
    parcAssertNotNull(*entryPtr, "Parameter must dereference to non-null pointer");
...
Message *
contentStoreEntry_GetMessage(const ContentStoreEntry *storeEntry)
{
...
bool
contentStoreEntry_HasExpiryTimeTicks(const ContentStoreEntry *storeEntry)
{
    parcAssertNotNull(storeEntry, "Parameter must be non-null");
...
uint64_t
contentStoreEntry_GetExpiryTimeTicks(const ContentStoreEntry *storeEntry)
{
    parcAssertNotNull(storeEntry, "Parameter must be non-null");
    parcAssertTrue(storeEntry->hasExpiryTimeTicks,
               "storeEntry has no ExpiryTimeTicks. Did you call contentStoreEntry_HasExpiryTimeTicks() first?");
    return storeEntry->expiryTimeTicks;
}

int
contentStoreEntry_CompareExpiryTime(const ContentStoreEntry *value1,
                                           const ContentStoreEntry *value2)
{
    ...
    if (v1->expiryTimeTicks < v2->expiryTimeTicks) {
        ...
        // At this point, the times are the same. Use the address of the message as the decider.
        // This allows us to store multiple messages with the same expiry/cache time.
...
void
contentStoreEntry_MoveToHead(ContentStoreEntry *storeEntry)
{
    parcAssertNotNull(storeEntry, "Parameter must be non-null");
    parcAssertNotNull(storeEntry->lruEntry, "ContentStoreEntry is not attached to an ListLru");
