...
void
listLRU_EntryDestroy(ListLruEntry **entryPtr)
{
    parcAssertNotNull(entryPtr, "Parameter entryPtr must be non-null double pointer");

    ListLruEntry *entry = *entryPtr;
    if (entry->inList) {
        TAILQ_REMOVE(&entry->parentList->head, entry, list);
        parcAssertTrue(entry->parentList->itemsInList > 0, "Invalid state, removed entry from list, but itemsInList is 0");
...
void
listLRU_EntryMoveToHead(ListLruEntry *entry)
{
...
void *
listLRU_EntryGetData(ListLruEntry *entry)
{
...
listLRU_Create()
{
    ...
    parcAssertNotNull(list, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ListLru));
    list->itemsInList = 0;
...
void
listLRU_Destroy(ListLru **lruPtr)
{
...
ListLruEntry *
listLRU_NewHeadEntry(ListLru *lru, void *data)
{
    ...
    parcAssertNotNull(entry, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ListLruEntry));
    entry->userData = data;
...
listLRU_PopTail(ListLru *lru)
{
    ...
    if (entry) {
        parcAssertTrue(lru->itemsInList > 0, "Invalid state, removed entry from list, but itemsInList is 0");
        lru->itemsInList--;
...
size_t
listLRU_Length(const ListLru *lru)
{
    parcAssertNotNull(lru, "Parameter lru must be non-null");
