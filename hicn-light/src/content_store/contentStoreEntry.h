...
/**
 * Creates a new `ContentStoreEntry` instance, acquiring a reference to the supplied `Message`.
 *
 * @param message the message to store
 * @param listLRU the LRU list that this entry will be stored in.
 * @return A newly created `ContentStoreEntry` instance that must eventually be released by calling
 *         {@link contentStoreEntry_Release}.
 *
 * @see contentStoreEntry_Release
 */
ContentStoreEntry *contentStoreEntry_Create(Message *objectMessage, ListLru *listLRU);
 ...
 * @return Reference counted copy, must call <code>contentStoreEntry_Destroy()</code> on it.
 */
 ...
 * The caller must called {@link message_Acquire()} if they want to keep a reference to the returned
 * message.
 *
 * @param storeEntry the ContentStoreEntry from which to retrieve the `Message` pointer.
...
/**
 * Return true if the message stored in this `ContentStoreEntry` has an ExpiryTime.
 *
 ...
 * @return the address of the `Message` contained in the storeEntry.
 */
uint64_t contentStoreEntry_GetExpiryTimeTicks(const ContentStoreEntry *storeEntry);

/**
 * A signum function comparing two `ContentStoreEntry` instances, using their
 * ExpiryTime and, if necessary, the addresses of the referenced Message. In other words, if
 * two ContentStoreEntries have the same ExpiryTime, the comparison will then be made on the
 * memory addresses of the Messages referenced by the ContentStoreEntrys. So, the only way two
 * ContentStoreEntrys will compare equally (0) is if they both have the same ExpiryTime and reference
 * the same Message.
 *
 * Used to determine the ordering relationship of two `ContentStoreEntry` instances.
 * This is used by the {@link ListTimeOrdered} to keep a list of ContentStoreEntrys, sorted by
 * ExpiryTime.
 *
 * @param [in] storeEntry1 A pointer to a `ContentStoreEntry` instance.
 * @param [in] storeEntry2 A pointer to a `ContentStoreEntry` instance to be compared to `storeEntry1`.
 *
 ...
 * @endcode
 */
int contentStoreEntry_CompareExpiryTime(const ContentStoreEntry *storeEntry1, const ContentStoreEntry *storeEntry2);
