...
static void
_destroyIndexes(_ContentStoreLRU *store)
{
...
static void
_contentStoreInterface_Destroy(ContentStoreInterface **storeImplPtr)
{
...
static bool
_contentStoreLRU_Destructor(_ContentStoreLRU **storePtr)
{
...
parcObject_Override(_ContentStoreLRU, PARCObject,
                    .destructor = (PARCObjectDestructor *) _contentStoreLRU_Destructor
                    );

parcObject_ExtendPARCObject(ContentStoreInterface,
                            _contentStoreInterface_Destroy, NULL, NULL, NULL, NULL, NULL, NULL);
...
static bool
_contentStoreLRU_Init(_ContentStoreLRU *store, ContentStoreConfig *config, Logger *logger)
{
    bool result = false;
    ...
    store->indexByExpirationTime =
        listTimeOrdered_Create((TimeOrderList_KeyCompare *) contentStoreEntry_CompareExpiryTime);
    ...
    if ((store->indexByExpirationTime == NULL)
        || (store->storageByName == NULL)
        || (store->lru == NULL)) {
        if (logger_IsLoggable(store->logger, LoggerFacility_Processor, PARCLogLevel_Error)) {
            logger_Log(store->logger, LoggerFacility_Processor, PARCLogLevel_Error, __func__,
                              "ContentStoreLRU could not be created. Could not allocate all index tables.",
 ...
 */
static void
_contentStoreLRU_PurgeStoreEntry(_ContentStoreLRU *store, ContentStoreEntry *entryToPurge)
{
...
static bool
_contentStoreLRU_RemoveLeastUsed(_ContentStoreLRU *store)
{
    bool result = false;
    ...
    if (store->objectCount > 0) {
        ...
        if (logger_IsLoggable(store->logger, LoggerFacility_Processor, PARCLogLevel_Debug)) {
            logger_Log(store->logger, LoggerFacility_Processor, PARCLogLevel_Debug, __func__,
...
static void
_evictByStorePolicy(_ContentStoreLRU *store, uint64_t currentTimeInTicks)
{
    // We need to make room. Here's the plan:
    //  1) Check to see if anything has expired. If so, remove it and we're done. If not,
    //  2) Remove the least recently used item.

    ContentStoreEntry *entry = listTimeOrdered_GetOldest(store->indexByExpirationTime);
    if (entry
        && contentStoreEntry_HasExpiryTimeTicks(entry)
...
static bool
_contentStoreLRU_PutContent(ContentStoreInterface *storeImpl, Message *content, uint64_t currentTimeTicks)

{
    bool result = false;
    _ContentStoreLRU *store = (_ContentStoreLRU *) contentStoreInterface_GetPrivateData(storeImpl);
    ...
    if (entry != NULL) {
        if (parcHashCodeTable_Add(store->storageByName, content, entry)) {
            ...
            if (logger_IsLoggable(store->logger, LoggerFacility_Processor, PARCLogLevel_Warning)) {
                logger_Log(store->logger, LoggerFacility_Processor, PARCLogLevel_Warning, __func__,
...
static Message *
_contentStoreLRU_MatchInterest(ContentStoreInterface *storeImpl, Message *interest, uint64_t currentTimeTicks)
{
    Message *result = NULL;
    ...
    if (foundEntry) {
        ...
        if (logger_IsLoggable(store->logger, LoggerFacility_Processor, PARCLogLevel_Debug)) {
            logger_Log(store->logger, LoggerFacility_Processor, PARCLogLevel_Debug, __func__,
                              "ContentStoreLRU %p matched interest %p (hits %" PRIu64 ", misses %" PRIu64 ")",
        ...
        if (logger_IsLoggable(store->logger, LoggerFacility_Processor, PARCLogLevel_Debug)) {
            logger_Log(store->logger, LoggerFacility_Processor, PARCLogLevel_Debug, __func__,
                              "ContentStoreLRU %p missed interest %p (hits %" PRIu64 ", misses %" PRIu64 ")",
                              (void *) store, (void *) interest, store->stats.countHits, store->stats.countMisses);
        }
    }
...
static bool
_contentStoreLRU_RemoveContent(ContentStoreInterface *storeImpl, Message *content)
{
    bool result = false;
    _ContentStoreLRU *store = (_ContentStoreLRU *) contentStoreInterface_GetPrivateData(storeImpl);
...
static void
_contentStoreLRU_Log(ContentStoreInterface *storeImpl)
{
    _ContentStoreLRU *store = (_ContentStoreLRU *) contentStoreInterface_GetPrivateData(storeImpl);

    logger_Log(store->logger, LoggerFacility_Processor, PARCLogLevel_All, __func__,
...
static size_t
_contentStoreLRU_GetObjectCapacity(ContentStoreInterface *storeImpl)
{
    _ContentStoreLRU *store = (_ContentStoreLRU *) contentStoreInterface_GetPrivateData(storeImpl);
...
static size_t
_contentStoreLRU_GetObjectCount(ContentStoreInterface *storeImpl)
{
    _ContentStoreLRU *store = (_ContentStoreLRU *) contentStoreInterface_GetPrivateData(storeImpl);
...
static size_t
_contentStoreLRU_SetObjectCapacity(ContentStoreInterface *storeImpl, size_t newCapacity)
{
    _ContentStoreLRU *store = (_ContentStoreLRU *) contentStoreInterface_GetPrivateData(storeImpl);
...
contentStoreLRU_Create(ContentStoreConfig *config, Logger *logger)
{
    ...
    if (storeImpl != NULL) {
        storeImpl->_privateData = parcObject_CreateAndClearInstance(_ContentStoreLRU);
        ...
        if (_contentStoreLRU_Init(storeImpl->_privateData, config, logger)) {
            ...
            if (logger_IsLoggable(logger, LoggerFacility_Processor, PARCLogLevel_Info)) {
                logger_Log(logger, LoggerFacility_Processor, PARCLogLevel_Info, __func__,
                                  "ContentStoreLRU %p created with capacity %zu",
                                  (void *) storeImpl, contentStoreInterface_GetObjectCapacity(storeImpl));
            }
        }
    } else {
