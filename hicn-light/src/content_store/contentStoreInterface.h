...
struct contentstore_interface {
    /**
     * Place a Message representing a ContentObject into the ContentStore. If necessary to make room,
     ...
     * @param currentTimeTicks - the current time, in hicn-light ticks, since the UTC epoch.
     */
     ...
     *
     * @param storeImpl - a pointer to this ContentStoreInterface instance.
     * @param interest - a pointer to a `Message` representing the Interest to match.
     *
     ...
     * to be released by calling {@link contentStoreInterface_Release}.
     *
     * @param storeImpl - a pointer to this ContentStoreInterface instance.
     */
 ...
 * @param currentTimeTicks - the current time, in hicn-light ticks, since the UTC epoch.
 */
bool contentStoreInterface_PutContent(ContentStoreInterface *storeImpl, Message *content, uint64_t currentTimeTicks);
 ...
 * @param content - a pointer to a `Message` to remove from the store.
 */
bool contentStoreInterface_RemoveContent(ContentStoreInterface *storeImpl, Message *content);
 ...
 * @return NULL if no matching ContentObject was found
 */
Message *contentStoreInterface_MatchInterest(ContentStoreInterface *storeImpl, Message *interest, uint64_t currentTimeTicks);
 ...
 * @return the maximum number of ContentObjects that can be stored
 */
size_t contentStoreInterface_GetObjectCapacity(ContentStoreInterface *storeImpl);
 ...
 * @param storeImpl - a pointer to this ContentStoreInterface instance.
 */
ContentStoreInterface *contentStoreInterface_Aquire(const ContentStoreInterface *storeImpl);
