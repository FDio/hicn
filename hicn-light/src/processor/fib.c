...
FibNode *
_createNode(FibNode *left, FibNode *right, FibEntry *entry, unsigned pos)
{
...
fIB_Create()
{
    ...
    parcAssertNotNull(hicnFib, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(FIB));

    hicnFib->root = _createNode(NULL, NULL, NULL, NULL_POS); //the pos will decrease going down in the trie
...
void
fIB_Add(FIB *fib, FibEntry *entry)
{
    ...
    if(curr->entry != NULL && nameBitvector_Equals(name,
                                                    name_GetContentName(fibEntry_GetPrefix(
                                                    curr->entry)))) {
    ...
    //if the name is not in the FIB search for the first different bit between the new name to add and the node found in the trie
    uint8_t pos = MSB_POS;
    if(curr->entry != NULL)
        pos = nameBitvector_firstDiff(name,
...
FibEntry *
fIB_Contains(const FIB *fib, const Name *prefix)
{
    parcAssertNotNull(fib, "Parameter must be non-null");
...
void
_removeNode(FIB *fib, const Name *prefix)
{
    parcAssertNotNull(fib, "Parameter must be non-null");
    parcAssertNotNull(prefix, "Parameter must be non-null");

    FibNode *grand = NULL;      //grandparent
    FibNode *prev = fib->root;  //parent: it will points to curr of the next hop in the trie 
    ...
    if(!nameBitvector_Equals(name, name_GetContentName(fibEntry_GetPrefix(curr->entry)))) {
...
void 
fIB_Remove(FIB *fib, const Name *name, unsigned connId)
{
    parcAssertNotNull(fib, "Parameter must be non-null");
...
void
_removeConnectionId(FibNode *n, unsigned pos, unsigned connectionId, FibEntryList *list)
{
    if(n->pos < pos){
...
void 
fIB_RemoveConnectionId(FIB *fib, unsigned connectionId)
{
    parcAssertNotNull(fib, "Parameter must be non-null");
...
FibEntry *
fIB_Match(const FIB *fib, const Message *interestMessage)
{
    parcAssertNotNull(fib, "Parameter must be non-null");
    ...
    while(prev->pos > curr->pos){
        ...
        if(curr->entry != NULL){
            if(nameBitvector_StartsWith(name, name_GetContentName(fibEntry_GetPrefix(curr->entry))) &&
                nameBitvector_GetLength(name_GetContentName(fibEntry_GetPrefix(curr->entry))) > len){
                match = curr;
                len = nameBitvector_GetLength(name_GetContentName(fibEntry_GetPrefix(curr->entry)));
            }
        }
...
void
_collectFibEntries(FibNode *n, int pos, FibEntryList *list)
{
