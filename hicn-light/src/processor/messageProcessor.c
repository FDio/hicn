...
static void messageProcessor_Drop(MessageProcessor *processor, Message *message);
static void messageProcessor_ReceiveInterest(MessageProcessor *processor, Message *interestMessage);
static void messageProcessor_ReceiveContentObject(MessageProcessor *processor, Message *objectMessage);
static unsigned messageProcessor_ForwardToNexthops(MessageProcessor *processor, Message *message, const NumberSet *nexthops);

static void messageProcessor_ForwardToInterfaceId(MessageProcessor *processor, Message *message, unsigned interfaceId);
...
{
    size_t objectStoreSize = configuration_GetObjectStoreSize(forwarder_GetConfiguration(forwarder));

    MessageProcessor *processor = parcMemory_AllocateAndClear(sizeof(MessageProcessor));
    parcAssertNotNull(processor, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(MessageProcessor));
    ...
    if (logger_IsLoggable(processor->logger, LoggerFacility_Processor, PARCLogLevel_Debug)) {
        logger_Log(processor->logger, LoggerFacility_Processor, PARCLogLevel_Debug, __func__,
    ...
    processor->contentStore = contentStoreLRU_Create(&contentStoreConfig, processor->logger);
...
{
    parcAssertNotNull(processor, "Parameter processor must be non-null");
    size_t objectStoreSize = configuration_GetObjectStoreSize(forwarder_GetConfiguration(processor->forwarder));
...
messageProcessor_GetContentObjectStore(const MessageProcessor *processor)
{
    parcAssertNotNull(processor, "Parameter processor must be non-null");
...
void
messageProcessor_Receive(MessageProcessor *processor, Message *message)
{
    parcAssertNotNull(processor, "Parameter processor must be non-null");
...
bool messageProcessor_AddOrUpdateRoute(MessageProcessor *processor, add_route_command *control, unsigned ifidx){
    ...
    strategy_type fwdStrategy = configuration_GetForwardingStrategy(config, prefixStr);
    if(fwdStrategy == LAST_STRATEGY_VALUE){
        fwdStrategy = SET_STRATEGY_LOADBALANCER;
    }

    Name *prefix = name_CreateFromAddress(control->addressType, control->address, control->len);
    ...
    if(newEntry && (fwdStrategy == SET_STRATEGY_LOADBALANCER_WITH_DELAY)){
        strategyLoadBalancerWithPD_SetConnectionTable(fibEntry_GetFwdStrategy(entry),
...
bool
messageProcessor_RemoveRoute(MessageProcessor *processor, remove_route_command *control, unsigned ifidx)
{
    Name *name = name_CreateFromAddress(control->addressType, control->address, control->len);
...
void
messageProcessor_RemoveConnectionIdFromRoutes(MessageProcessor *processor, unsigned connectionId)
{
    fIB_RemoveConnectionId(processor->fib, connectionId);
}

void
processor_SetStrategy(MessageProcessor *processor, Name *prefix, strategy_type strategy)
{
    FibEntry * entry = fIB_Contains(processor->fib, prefix);
    if(entry != NULL){
 ...
 *
 *   The default action for a message is to destroy it in <code>messageProcessor_Receive()</code>,
 *   so this function does not need to do that.
 *
 */
static void
messageProcessor_Drop(MessageProcessor *processor, Message *message)
{
 ...
 */
static bool
messageProcessor_AggregateInterestInPit(MessageProcessor *processor, Message *interestMessage)
{
...
static bool
_satisfyFromContentStore(MessageProcessor *processor, Message *interestMessage)
{
    bool result = false;
    ...
    Message *objectMessage = contentStoreInterface_MatchInterest(processor->contentStore, interestMessage,
    ...
    if (objectMessage != NULL) {
        ...
        parcAssertNotNull(nexthops, "Illegal state: got a null nexthops for an interest we just inserted.");
 ...
 * @discussion
 *   This calls <code>messageProcessor_ForwardToNexthops()</code>, so if we find any nexthops,
 *   the interest will be sent on its way.  Depending on the IoOperations of each nexthop,
 ...
 */
static bool
messageProcessor_ForwardViaFib(MessageProcessor *processor, Message *interestMessage)
{
    ...
    NumberSet *nexthops = (NumberSet *) fibEntry_GetNexthopsFromForwardingStrategy(fibEntry,
                                                                                                        interestMessage);
 ...
 */
static void
messageProcessor_ReceiveInterest(MessageProcessor *processor, Message *interestMessage)
{
    ...
    if (_satisfyFromContentStore(processor, interestMessage)) {
        // done
        // If we found a content object in the CS, messageProcess_SatisfyFromContentStore already
 ...
 */
static void
messageProcessor_ReceiveContentObject(MessageProcessor *processor, Message *message)
{
    ...
    if (numberSet_Length(ingressSetUnion) == 0) {
        ...
        //we store the packets in the content store enven in the case where there is no match in the PIT table
        //in this way the applications can push the content in the CS of the forwarder. We allow this only for local
        //faces
        bool isLocal = connection_IsLocal(
                connectionTable_FindById(forwarder_GetConnectionTable(processor->forwarder),
        ...
        if (processor->store_in_cache && isLocal) {
            ...
            contentStoreInterface_PutContent(processor->contentStore, message, currentTimeTicks);
            if (logger_IsLoggable(processor->logger, LoggerFacility_Processor, PARCLogLevel_Debug)) {
                logger_Log(processor->logger, LoggerFacility_Processor, PARCLogLevel_Debug, __func__,
                                    "Message %p sotred in the CS anyway",
                                    (void *) message);
            }
        }
 ...
 */
static unsigned
messageProcessor_ForwardToNexthops(MessageProcessor *processor, Message *message, const NumberSet *nexthops)
{
    ...
    for (size_t i = 0; i < length; i++) {
        ...
        if (egressId != ingressId) {
            ...
            if(message_GetType(message) == MessagePacketType_ContentObject){
                //everytime we send out a message we need to restore the original path label of the message
                //this is important because we keep a single copy of the message (single pointer) and we modify
                //the path label at each send.
 ...
 */
static void
messageProcessor_SendWithGoodHopLimit(MessageProcessor *processor, Message *message, unsigned interfaceId, const Connection *conn)
{
 ...
 */
static void
messageProcessor_ForwardToInterfaceId(MessageProcessor *processor, Message *message, unsigned interfaceId)
{
    ConnectionTable *connectionTable = forwarder_GetConnectionTable(processor->forwarder);
    const Connection *conn = connectionTable_FindById(connectionTable, interfaceId);
