...
static void
_pit_PitEntryDestroyer(void **dataPtr)
{
    pitEntry_Release((PitEntry **) dataPtr);
}

static bool
_pit_IngressSetContains(PitEntry *pitEntry, unsigned connectionId)
{
...
static Ticks
_pit_CalculateLifetime(StandardPIT *pit, Message *interestMessage)
{
    uint64_t interestLifetimeTicks = message_GetInterestLifetimeTicks(interestMessage);
...
static void
_pit_StoreInTable(StandardPIT *pit, Message *interestMessage)
{
    Message *key = message_Acquire(interestMessage);

    Ticks expiryTime = _pit_CalculateLifetime(pit, interestMessage);

    PitEntry *pitEntry = pitEntry_Create(key, expiryTime, forwarder_GetTicks(pit->forwarder));

    parcHashCodeTable_Add(pit->table, key, pitEntry);

    if (logger_IsLoggable(pit->logger, LoggerFacility_Processor, PARCLogLevel_Debug)) {
        logger_Log(pit->logger, LoggerFacility_Processor, PARCLogLevel_Debug, __func__,
                          "Message %p added to PIT (expiry %" PRIu64 ") ingress %u",
                          (void *) interestMessage,
                          pitEntry_GetExpiryTime(pitEntry),
                          message_GetIngressConnectionId(interestMessage));
    }
}

static void
_pit_ExtendLifetime(StandardPIT *pit, PitEntry *pitEntry, Message *interestMessage)
{
    Ticks expiryTime = _pit_CalculateLifetime(pit, interestMessage);
...
static void
_pitStandard_Destroy(PIT **pitPtr)
{
...
static PITVerdict
_pitStandard_ReceiveInterest(PIT *generic, Message *interestMessage)
{
    parcAssertNotNull(generic, "Parameter pit must be non-null");
    parcAssertNotNull(interestMessage, "Parameter interestMessage must be non-null");
    ...
    if (pitEntry) {
        ...
        if (now < pitEntry_GetExpiryTime(pitEntry)) {
            ...
            if (_pit_IngressSetContains(pitEntry, message_GetIngressConnectionId(interestMessage))) {
            ...
            pitEntry_AddIngressId(pitEntry, message_GetIngressConnectionId(interestMessage));
...
static NumberSet *
_pitStandard_SatisfyInterest(PIT *generic, const Message *objectMessage)
{
    parcAssertNotNull(generic, "Parameter pit must be non-null");
    ...
    if(pitEntry){
        ...
        if (now < pitEntry_GetExpiryTime(pitEntry)) {
            ...
            if (fibEntry != NULL) {
                ...
                Ticks rtt = forwarder_GetTicks(pit->forwarder) - pitEntry_GetCreationTime(pitEntry);
                fibEntry_ReceiveObjectMessage(fibEntry, pitEntry_GetEgressSet(pitEntry), objectMessage, rtt); //need to implement RTT
            }
            const NumberSet *is = pitEntry_GetIngressSet(pitEntry);
            numberSet_AddSet(ingressSet,is); //with this we do a copy so we can remove the entry from the PIT
        }
        //remove the entry from the PIT.  Key is a reference counted copy of the pit entry message
...
static void
_pitStandard_RemoveInterest(PIT *generic, const Message *interestMessage)
{
    parcAssertNotNull(generic, "Parameter pit must be non-null");
    parcAssertNotNull(interestMessage, "Parameter interestMessage must be non-null");
...
static PitEntry *
_pitStandard_GetPitEntry(const PIT *generic, const Message *interestMessage)
{
    parcAssertNotNull(generic, "Parameter pit must be non-null");
    parcAssertNotNull(interestMessage, "Parameter interestMessage must be non-null");
...
PIT *
pitStandard_Create(Forwarder *forwarder)
{
    ...
    parcAssertNotNull(generic, "parcMemory_AllocateAndClear(%zu) returned NULL", allocation);
    ...
    pit->table = parcHashCodeTable_Create_Size(hashTableFunction_MessageNameEquals,
                                          hashTableFunction_MessageNameHashCode,
                                          NULL,
                                          _pit_PitEntryDestroyer,
                                          initialSize);
