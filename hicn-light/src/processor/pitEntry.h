 ...
 * @discussion
 *   When the PIT entry is destroyed, will call <code>message_Release()</code> on the message.
 *
 */
 ...
 *
 * @param [in,out] pitEntryPtr A pointer to a PitEntry instance pointer, which will be set to zero on return.
 *
 */
 ...
 *
 * The ExpiryTime is computed when the PIT entry is added (or via pitEntry_SetExpiryTime).
