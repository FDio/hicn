...
static void
fibEntryList_ListDestroyer(void **voidPtr)
{
...
fibEntryList_Create()
{
    FibEntryList *fibEntryList = parcMemory_AllocateAndClear(sizeof(FibEntryList));
    parcAssertNotNull(fibEntryList, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(FibEntryList));
    fibEntryList->listOfFibEntries = parcArrayList_Create(fibEntryList_ListDestroyer);
    return fibEntryList;
}

void
fibEntryList_Destroy(FibEntryList **listPtr)
{
...
void
fibEntryList_Append(FibEntryList *list, FibEntry *fibEntry)
{
...
size_t
fibEntryList_Length(const FibEntryList *list)
{
    parcAssertNotNull(list, "Parameter list must be non-null pointer");
...
const FibEntry *
fibEntryList_Get(const FibEntryList *list, size_t index)
{
    parcAssertNotNull(list, "Parameter list must be non-null pointer");
