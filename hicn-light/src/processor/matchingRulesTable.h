 ...
 *
 *     When used in the PIT, one calls <code>matchingRulesTable_AddToBestTable()</code> to
 *     add an interest to the "best" (i.e. most restrictive match) table, then calls
 *     <code>matchingRulesTable_GetUnion()</code> on a content object to match against
 *     all of them.
 *
 *     When used in a ContentStore, one calls <code>matchingRulesTable_AddToAllTables()</code>
 *     to index a Content Object in all the tables.  one then calls <code>matchingRulesTable_Get()</code>
...
/**
 * Creates a MatchigRulesTable and specifies the function to call to de-allocate an entry
 *
 * The datadestroyer will be called when an entry is removed from a table. It may be NULL.
 */
MatchingRulesTable *matchingRulesTable_Create(PARCHashCodeTable_Destroyer dataDestroyer);
 ...
 */
void *matchingRulesTable_Get(const MatchingRulesTable *table, const Message *message);
 ...
 */
PARCArrayList *matchingRulesTable_GetUnion(const MatchingRulesTable *table, const Message *message);
 ...
 */
bool matchingRulesTable_AddToBestTable(MatchingRulesTable *rulesTable, Message *key, void *data);
 ...
 */
void matchingRulesTable_RemoveFromBest(MatchingRulesTable *rulesTable, const Message *message);

/**
 * @function matchingRulesTable_RemoveFromAll
 * @abstract Removes the message from all tables
 */
void matchingRulesTable_RemoveFromAll(MatchingRulesTable *rulesTable, const Message *message);
