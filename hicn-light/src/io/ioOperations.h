 ...
 *
 *   IoOperations *
 *   etherConnection_Create(Forwarder *forwarder, GenericEther *ether, AddressPair *pair)
 *   {
 ...
 * @constant class A unique identifier for each class that instantiates IoOperations.
 ...
 * memory related to the IoOperations.  It should NULL the input parameter.
 *
 ...
 *
 *      _EtherState *etherConnState = (_EtherState *) ioOperations_GetClosure(ops);
 ...
 * @endcode
 */
bool ioOperations_Send(IoOperations *ops, const Address *nexthop, Message *message);
 ...
 * @code
 * {
 *    Address *local =  addressCreateFromLink((uint8_t []) { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06 }, 6);
 *    Address *remote = addressCreateFromLink((uint8_t []) { 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F }, 6);
 *    AddressPair *pair = addressPair_Create(local, remote);
 *    IoOperations *ops = etherConnection_Create(forwarder, ether, pair);
 *
 ...
 * when a connection goes UP or DOWN, it should subscribe as a Missive listener.
 *
 * @param [in] ops The connection implementation.
 *
 ...
 *     if (!ioOperations(ingressConnectionOps) {
 *        uint8_t hoplimit = message_GetHopLimit(interestMessage);
 *        if (hoplimit == 0) {
 *           // error
 *        } else {
 *           hoplimit--;
...
/**
 * Returns the connection ID represented by this IoOperations in the ConnectionTable.
 *
 ...
 *     const Connection *conn = connectionTable_FindById(forwarder->connectionTable, id);
 * }
 * @endcode
 */
 ...
 *
 * TCP and AF_UNIX are both stream connections and will both return "Connection_TCP".
 * Ethernet will return "Connection_L2".
 *
 * @param [in] ops The connection implementation.
 *
 ...
 * @code
 * {
 *     ConnectionType type = ioOperations_GetConnectionType(connection_GetIoOperations(connection));
 *     Connection *Conn = Connection_Create(connection_GetConnectionId(connection), localAddress, remoteAddress, type);
 * }
 * @endcode
 */
list_connections_type ioOperations_GetConnectionType(const IoOperations *ops);

Ticks ioOperations_SendProbe(IoOperations *ops, unsigned probeType, uint8_t *message);
#endif // io_h
