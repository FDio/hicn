 ...
 *
 * Two AddressPair instances are equal if, and only if, the local and remote addresses are identical.
 * Equality is determined by addressEquals(a->local, b->local) and
 * Adress_Equals(a->remote, b->remote).
 *
 * The following equivalence relations on non-null `AddressPair` instances are maintained:
 *
 *  * It is reflexive: for any non-null reference value x, `AddressPair_Equals(x, x)`
 *      must return true.
 *
 ...
 */
bool addressPair_EqualsAddresses(const AddressPair *a, const Address *local, const Address *remote);
