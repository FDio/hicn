...
// Prototypes
static bool                     _streamConnection_Send(IoOperations *ops,
                                                              const Address *nexthop,
                                                              Message *message);
static const Address        *_streamConnection_GetRemoteAddress(const IoOperations *ops);
static const AddressPair  *_streamConnection_GetAddressPair(const IoOperations *ops);
...
static list_connections_type        _streamConnection_GetConnectionType(const IoOperations *ops);
static Ticks               _sendProbe(IoOperations *ops, unsigned probeType, uint8_t *message);

//REMINDER: when a new_command is added, the following array has to be updated with the sizeof(new_command).
//It allows to allocate the buffer for receiving the payload of the CONTROLLER REQUEST after the header has beed read.
//Each command identifier (typedef enum command_id) corresponds to a position in the following array.
static int payloadLengthDaemon[LAST_COMMAND_VALUE] = {sizeof(add_listener_command),
 ...
 */
static const void *
_streamConnection_Class(const IoOperations *ops)
{
...
IoOperations *
streamConnection_AcceptConnection(Forwarder *forwarder, int fd, AddressPair *pair, bool isLocal)
{
    _StreamState *stream = parcMemory_AllocateAndClear(sizeof(_StreamState));
    parcAssertNotNull(stream, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(_StreamState));
    ...
    parcAssertNotNull(io_ops, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(IoOperations));
...
IoOperations *
streamConnection_OpenConnection(Forwarder *forwarder, AddressPair *pair, bool isLocal)
{
    ...
    PARCEventQueue *bufferEventVector = dispatcher_StreamBufferConnect(forwarder_GetDispatcher(forwarder), pair);
...
_streamConnection_DestroyOperations(IoOperations **opsPtr)
{
    ...
    parcAssertNotNull(*opsPtr, "Parameter opsPtr must dereference to non-null pointer");

    IoOperations *ops = *opsPtr;
    parcAssertNotNull(ioOperations_GetClosure(ops), "ops->context must not be null");
...
static bool
_streamConnection_IsUp(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _StreamState *stream = (const _StreamState *) ioOperations_GetClosure(ops);
    return stream->isUp;
}

static bool
_streamConnection_IsLocal(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _StreamState *stream = (const _StreamState *) ioOperations_GetClosure(ops);
...
_streamConnection_GetRemoteAddress(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _StreamState *stream = (const _StreamState *) ioOperations_GetClosure(ops);
...
static const AddressPair *
_streamConnection_GetAddressPair(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _StreamState *stream = (const _StreamState *) ioOperations_GetClosure(ops);
...
static unsigned
_streamConnection_GetConnectionId(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _StreamState *stream = (const _StreamState *) ioOperations_GetClosure(ops);
    return stream->id;
...
bool
streamState_SendCommandResponse(IoOperations *ops, struct iovec *response)
{
    ...
    bool success = false;
    if (conn->isUp) {
        PARCEventBuffer *buffer = parcEventBuffer_GetQueueBufferOutput(conn->bufferEventVector);
        size_t buffer_backlog = parcEventBuffer_GetLength(buffer);
        parcEventBuffer_Destroy(&buffer);

        if (buffer_backlog < OUTPUT_QUEUE_BYTES) {
            if (logger_IsLoggable(conn->logger, LoggerFacility_IO, PARCLogLevel_Debug)) {
                logger_Log(conn->logger, LoggerFacility_IO, PARCLogLevel_Debug, __func__,
                                  ...
                                  conn->id,
                                  (response[0].iov_len + response[1].iov_len), //NEW: take total lenght
                                  buffer_backlog);
            }

            //NEW: write directly ino the parcEventQueue without passing through message
            int failure = parcEventQueue_Write(conn->bufferEventVector,response[0].iov_base , response[0].iov_len) +
                          parcEventQueue_Write(conn->bufferEventVector,response[1].iov_base , response[1].iov_len) ;

            if (failure == 0) {
                success = true;
            }
        } else {
            if (logger_IsLoggable(conn->logger, LoggerFacility_IO, PARCLogLevel_Warning)) {
                logger_Log(conn->logger, LoggerFacility_IO, PARCLogLevel_Warning, __func__,
        ...
        if (logger_IsLoggable(conn->logger, LoggerFacility_IO, PARCLogLevel_Error)) {
            logger_Log(conn->logger, LoggerFacility_IO, PARCLogLevel_Error, __func__,
 ...
 */
static bool
_streamConnection_Send(IoOperations *ops, const Address *dummy, Message *message)
{
    parcAssertNotNull(ops, "Parameter ops must be non-null");
...
_streamConnection_GetConnectionType(const IoOperations *ops)
{
    return CONN_TCP;
}

static Ticks
_sendProbe(IoOperations *ops, unsigned probeType, uint8_t * message)
{
...
_isACommand(PARCEventBuffer *input)
{
    ...
    parcAssertTrue(bytesAvailable >= sizeof(header_control_message), "Called with too short an input: %zu", bytesAvailable);
...
PARCEventBuffer *
_tryReadControlMessage(_StreamState *stream, PARCEventBuffer *input, command_id command, struct iovec **request){

    size_t bytesAvailable = parcEventBuffer_GetLength(input);

    if (stream->nextMessageLength == 0) {
        stream->nextMessageLength = sizeof(header_control_message) +
    ...
    if (bytesAvailable >= stream->nextMessageLength){
        ...
        int bytesRead = parcEventBuffer_ReadIntoBuffer(input, message, stream->nextMessageLength);
        parcAssertTrue(bytesRead == stream->nextMessageLength, "Partial read, expected %zu got %d", stream->nextMessageLength, bytesRead);

        uint8_t *control = parcEventBuffer_Pullup(message, stream->nextMessageLength);
        if(!(*request = (struct iovec *) parcMemory_AllocateAndClear( sizeof(struct iovec) * 2))){
            return NULL;
        }
        (*request)[0].iov_base = control;                                      //header
        (*request)[0].iov_len = sizeof(header_control_message);
        if (payloadLengthDaemon[command] > 0){
            (*request)[1].iov_base = control + sizeof(header_control_message);     //payload
        } else {
...
static bool
_isAnHIcnPacket(PARCEventBuffer *input)
{
    size_t bytesAvailable = parcEventBuffer_GetLength(input);
    parcAssertTrue(bytesAvailable >= sizeof(header_control_message), "Called with too short an input: %zu", bytesAvailable);
...
static Message *
_readMessage(_StreamState *stream, Ticks time, PARCEventBuffer *input)
{
  Message *message = message_CreateFromEventBuffer(input,
...
static void
_startNewMessage(_StreamState *stream, PARCEventBuffer *input, size_t inputBytesAvailable)
{
    parcAssertTrue(stream->nextMessageLength == 0, "Invalid state, nextMessageLength not zero: %zu", stream->nextMessageLength);
    parcAssertTrue(inputBytesAvailable >= sizeof(header_control_message), "read_length not a whole fixed header!: %zd", inputBytesAvailable);

    // this linearizes the first messageHandler_GetIPv6HeaderLength() bytes of the input buffer's iovecs and
...
static Message *
_tryReadMessage(PARCEventBuffer *input, _StreamState *stream)
{
    size_t bytesAvailable = parcEventBuffer_GetLength(input);
    parcAssertTrue(bytesAvailable >= sizeof(header_control_message), "Called with too short an input: %zu", bytesAvailable);
    ...
    if (bytesAvailable >= stream->nextMessageLength) {
        Message *message = _readMessage(stream, forwarder_GetTicks(stream->forwarder), input);
...
_conn_readcb(PARCEventQueue *event, PARCEventType type, void *ioOpsVoid)
{
    ...
    //notice that we always try to read at least 8 bytes (sizeof(header_control_message)). This is enough to read the length of all kind of packets
    while (parcEventBuffer_GetLength(input) >= sizeof(header_control_message) && parcEventBuffer_GetLength(input) >= stream->nextMessageLength) {
        if((command = _isACommand(input)) != LAST_COMMAND_VALUE){
            struct iovec *rx;
            //Get message from the stream and set the stream->nextMessageLength
            PARCEventBuffer *message = _tryReadControlMessage(stream, input, command, &rx);
            ...
            //this is an HIcn packet (here we should distinguish between IPv4 and IPv6
            //tryReadMessage may set nextMessageLength
            ...
            parcAssertTrue(false,"(Local stream connection) malformend packet received");
        }
    }

    if (stream->nextMessageLength == 0) {
        // we don't have the next header, so set it to the header length
        streamBuffer_SetWatermark(event, true, false, sizeof(header_control_message), 0);
    } else {
...
static void
_setConnectionState(_StreamState *stream, bool isUp)
{
