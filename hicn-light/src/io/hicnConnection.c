...
static void _refreshProbeDestAddress(_HicnState * hicnConnState, const uint8_t *message);

IoOperations *
hicnConnection_Create(Forwarder *forwarder, int fd, const AddressPair *pair, bool isLocal)
{
    IoOperations *io_ops = NULL;

    _HicnState *hicnConnState = parcMemory_AllocateAndClear(sizeof(_HicnState));
    parcAssertNotNull(hicnConnState, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(_HicnState));
    ...
    if (saved) {
        ...
        if (logger_IsLoggable(hicnConnState->logger, LoggerFacility_IO, PARCLogLevel_Info)) {
            char *str = addressPair_ToString(hicnConnState->addressPair);
            logger_Log(hicnConnState->logger, LoggerFacility_IO, PARCLogLevel_Info, __func__,
...
_destroy(IoOperations **opsPtr)
{
    ...
    messenger_Send(forwarder_GetMessenger(hicnConnState->forwarder),
                          missive_Create(MissiveType_ConnectionDestroyed, hicnConnState->id));

    if (logger_IsLoggable(hicnConnState->logger, LoggerFacility_IO, PARCLogLevel_Info)) {
        logger_Log(hicnConnState->logger, LoggerFacility_IO, PARCLogLevel_Info, __func__,
                          "HIcnConnection %p destroyed",
                          (void *) hicnConnState);
    }
...
static bool
_isUp(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _HicnState *hicnConnState = (const _HicnState *) ioOperations_GetClosure(ops);
    return hicnConnState->isUp;
}

static bool
_isLocal(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _HicnState *hicnConnState = (const _HicnState *) ioOperations_GetClosure(ops);
...
_getRemoteAddress(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _HicnState *hicnConnState = (const _HicnState *) ioOperations_GetClosure(ops);
...
_getAddressPair(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _HicnState *hicnConnState = (const _HicnState *) ioOperations_GetClosure(ops);
...
static unsigned
_getConnectionId(const IoOperations *ops)
{
    parcAssertNotNull(ops, "Parameter must be non-null");
    const _HicnState *hicnConnState = (const _HicnState *) ioOperations_GetClosure(ops);
 ...
 */
static bool
_send(IoOperations *ops, const Address *dummy, Message *message)
{
    ...
    if(message_GetType(message) == MessagePacketType_ContentObject){
        ...
        if(messageHandler_GetIPPacketType(message_FixedHeader(message)) == IPv6_TYPE){
            ...
            messageHandler_SetDestination_IPv4((uint8_t *) message_FixedHeader(message),
                                             &(((struct sockaddr_in *) hicnConnState->peerAddress)->sin_addr.s_addr));
        }
        ...
        if(messageHandler_GetIPPacketType(message_FixedHeader(message)) == IPv6_TYPE){
            ...
            messageHandler_SetSource_IPv4((uint8_t *) message_FixedHeader(message),
                                             &(((struct sockaddr_in *) hicnConnState->localAddress)->sin_addr.s_addr));
        }
    ...
    ssize_t writeLength = write(hicnConnState->hicnListenerSocket, message_FixedHeader(message),
...
static list_connections_type
_getConnectionType(const IoOperations *ops)
{
    return CONN_HICN;
}
...
_sendProbe(IoOperations *ops, unsigned probeType, uint8_t *message)
{
    ...
    if(hicnConnState->probeDestAddress == NULL && probeType == PACKET_TYPE_PROBE_REPLY){

        uint8_t * pkt = parcMemory_AllocateAndClear(messageHandler_GetICMPPacketSize(IPv6_TYPE));
        messageHandler_SetProbePacket(pkt, probeType, (struct in6_addr *) messageHandler_GetDestination(message),
                                                (struct in6_addr *) messageHandler_GetSource(message));

        ssize_t writeLength = write(hicnConnState->hicnListenerSocket, pkt, messageHandler_GetICMPPacketSize(IPv6_TYPE));
    ...
    } else if (hicnConnState->probeDestAddress != NULL && probeType == PACKET_TYPE_PROBE_REQUEST){

        hicnConnState->refreshProbeDestAddress = true;

        uint8_t * pkt = parcMemory_AllocateAndClear(messageHandler_GetICMPPacketSize(IPv6_TYPE));
        messageHandler_SetProbePacket(pkt, probeType, &((struct sockaddr_in6 *) hicnConnState->localAddress)->sin6_addr,
        ...
        if(hicnConnState->probeDestAddress == NULL && probeType == PACKET_TYPE_PROBE_REQUEST){
...
static bool
_saveSockaddr(_HicnState *hicnConnState, const AddressPair *pair)
{
    bool success = false;
    ...
    switch (addressGetType(remoteAddress)) { //local must be of the same type
        ...
        case ADDR_INET: {
            ...
            parcAssertNotNull(hicnConnState->peerAddress, "parcMemory_Allocate(%zu) returned NULL", bytes);

            addressGetInet(remoteAddress, (struct sockaddr_in *) hicnConnState->peerAddress);
            hicnConnState->peerAddressLength = (socklen_t) bytes;

            hicnConnState->localAddress = parcMemory_Allocate(bytes);
            parcAssertNotNull(hicnConnState->localAddress, "parcMemory_Allocate(%zu) returned NULL", bytes);

            addressGetInet(localAddress, (struct sockaddr_in *) hicnConnState->localAddress);
        ...
        case ADDR_INET6: {
            ...
            addressGetInet6(remoteAddress, (struct sockaddr_in6 *) hicnConnState->peerAddress);
            hicnConnState->peerAddressLength = (socklen_t) bytes;

            hicnConnState->localAddress = parcMemory_Allocate(bytes);
            parcAssertNotNull(hicnConnState->localAddress, "parcMemory_Allocate(%zu) returned NULL", bytes);

            addressGetInet6(localAddress, (struct sockaddr_in6 *) hicnConnState->localAddress);
            hicnConnState->localAddressLength = (socklen_t) bytes;
        ...
        default:
            if (logger_IsLoggable(hicnConnState->logger, LoggerFacility_IO, PARCLogLevel_Error)) {
                char *str = addressToString(remoteAddress);
                logger_Log(hicnConnState->logger, LoggerFacility_IO, PARCLogLevel_Error, __func__,
...
static void
_refreshProbeDestAddress(_HicnState * hicnConnState, const uint8_t *message)
{
    if((hicnConnState->peerAddressLength == sizeof(struct sockaddr_in)) || (hicnConnState->localAddressLength == sizeof(struct sockaddr_in)))
        return;

    if(hicnConnState->probeDestAddress == NULL){
        hicnConnState->probeDestAddress = parcMemory_AllocateAndClear(sizeof(struct sockaddr_in6));
        parcAssertNotNull(hicnConnState->probeDestAddress, "parcMemory_Allocate(%zu) returned NULL", sizeof(struct sockaddr_in6));
    }

    ((struct sockaddr_in6 *) hicnConnState->probeDestAddress)->sin6_family = AF_INET6;
    ((struct sockaddr_in6 *) hicnConnState->probeDestAddress)->sin6_port = htons(1234);
    ...
    ((struct sockaddr_in6 *) hicnConnState->probeDestAddress)->sin6_addr = *((struct in6_addr *) messageHandler_GetDestination(
        message));
    hicnConnState->refreshProbeDestAddress = false;
}

static void
_setConnectionState(_HicnState *hicnConnState, bool isUp)
{
    ...
    if (oldStateIsUp && !isUp) {
        ...
        Missive *missive = missive_Create(MissiveType_ConnectionDown, hicnConnState->id);
    ...
    if (!oldStateIsUp && isUp) {
        // bring connection UP
        Missive *missive = missive_Create(MissiveType_ConnectionUp, hicnConnState->id);
        messenger_Send(messenger, missive);
        return;
    }
}
