...
static bool _sendCommandResponse(IoOperations *ops, struct iovec *message){
  ...
  printf("try to write in the udp socket\n");
  //TODO
  //remove iovec linearization (writev seems to do not work)
  uint8_t * msg = parcMemory_AllocateAndClear(message[0].iov_len
                                              + message[1].iov_len);
  memcpy(msg, message[0].iov_base, message[0].iov_len);
  memcpy(msg + message[0].iov_len, message[1].iov_base, message[1].iov_len);
  ssize_t writeLength =
      //writev(udpConnState->udpListenerSocket, message, 2);
      sendto(udpConnState->udpListenerSocket, msg,
        (int) (message[0].iov_len + message[1].iov_len), 0,
        udpConnState->peerAddress, udpConnState->peerAddressLength);
