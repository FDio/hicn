...
IoOperations *
udpConnection_Create(Forwarder *forwarder, int fd, const AddressPair *pair, bool isLocal)
{
    IoOperations *io_ops = NULL;

    _UdpState *udpConnState = parcMemory_AllocateAndClear(sizeof(_UdpState));
    parcAssertNotNull(udpConnState, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(_UdpState));
    ...
    if (saved) {
        ...
        if (logger_IsLoggable(udpConnState->logger, LoggerFacility_IO, PARCLogLevel_Info)) {
            char *str = addressPair_ToString(udpConnState->addressPair);
            logger_Log(udpConnState->logger, LoggerFacility_IO, PARCLogLevel_Info, __func__,
                            "UdpConnection %p created for address %s (isLocal %d)",
                            (void *) udpConnState, str, udpConnState->isLocal);
            free(str);
        }

        messenger_Send(forwarder_GetMessenger(forwarder), missive_Create(MissiveType_ConnectionCreate, udpConnState->id));
        messenger_Send(forwarder_GetMessenger(forwarder), missive_Create(MissiveType_ConnectionUp, udpConnState->id));
    } else {
...
_destroy(IoOperations **opsPtr)
{
    ...
    messenger_Send(forwarder_GetMessenger(udpConnState->forwarder), missive_Create(MissiveType_ConnectionDestroyed, udpConnState->id));
...
_isUp(const IoOperations *ops)
{
    ...
    const _UdpState *udpConnState = (const _UdpState *) ioOperations_GetClosure(ops);
...
_send(IoOperations *ops, const Address *dummy, Message *message)
{
    ...
    //NAT for HICN
    //in this particular connection we don't need natting beacause we send the packet to the next
    //hop using upd connection
    ...
    ssize_t writeLength = sendto(udpConnState->udpListenerSocket, message_FixedHeader(message),
                        message_Length(message), 0, udpConnState->peerAddress, udpConnState->peerAddressLength);
...
static bool
_saveSockaddr(_UdpState *udpConnState, const AddressPair *pair)
{
    bool success = false;
    ...
    switch (addressGetType(remoteAddress)) {
        case ADDR_INET: {
            ...
            parcAssertNotNull(udpConnState->peerAddress, "parcMemory_Allocate(%zu) returned NULL", bytes);

            addressGetInet(remoteAddress, (struct sockaddr_in *) udpConnState->peerAddress);
        ...
        case ADDR_INET6: {
            ...
            addressGetInet6(remoteAddress, (struct sockaddr_in6 *) udpConnState->peerAddress);
            udpConnState->peerAddressLength = (socklen_t) bytes;
        ...
        default:
            if (logger_IsLoggable(udpConnState->logger, LoggerFacility_IO, PARCLogLevel_Error)) {
                char *str = addressToString(remoteAddress);
                logger_Log(udpConnState->logger, LoggerFacility_IO, PARCLogLevel_Error, __func__,
...
static void
_setConnectionState(_UdpState *udpConnState, bool isUp)
{
    ...
    if (oldStateIsUp && !isUp) {
        ...
        Missive *missive = missive_Create(MissiveType_ConnectionDown, udpConnState->id);
    ...
    if (!oldStateIsUp && isUp) {
        // bring connection UP
        Missive *missive = missive_Create(MissiveType_ConnectionUp, udpConnState->id);
        messenger_Send(messenger, missive);
        return;
    }
}
