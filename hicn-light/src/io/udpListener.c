...
ListenerOps *
udpListener_CreateInet6(Forwarder *forwarder, struct sockaddr_in6 sin6)
{
     ListenerOps *ops = NULL;

    UdpListener *udp = parcMemory_AllocateAndClear(sizeof(UdpListener));
    parcAssertNotNull(udp, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(UdpListener));
    ...
    parcAssertFalse(udp->udp_socket < 0, "Error opening UDP socket: (%d) %s", errno, strerror(errno));
    ...
    if (failure == 0) {
        udp->udp_event = dispatcher_CreateNetworkEvent(forwarder_GetDispatcher(forwarder), true, _readcb, (void *) udp, udp->udp_socket);
        dispatcher_StartNetworkEvent(forwarder_GetDispatcher(forwarder), udp->udp_event);
...
ListenerOps *
udpListener_CreateInet(Forwarder *forwarder, struct sockaddr_in sin)
{
    ListenerOps *ops = NULL;
...
static void
udpListener_Destroy(UdpListener **listenerPtr)
{
    ...
    close(udp->udp_socket);
    addressDestroy(&udp->localAddress);
    dispatcher_DestroyNetworkEvent(forwarder_GetDispatcher(udp->forwarder), &udp->udp_event);
    logger_Release(&udp->logger);
    parcMemory_Deallocate((void **) &udp);
    *listenerPtr = NULL;
}
 ...
 */
static size_t
_peekMessageLength(UdpListener *udp, int fd, struct sockaddr *peerIpAddress, socklen_t *peerIpAddressLengthPtr)
{
    ...
    ssize_t res = recvfrom(fd, fixedHeader, messageHandler_GetIPHeaderLength(IPv6),
                    MSG_PEEK, (struct sockaddr *) peerIpAddress, peerIpAddressLengthPtr);

    if(res == messageHandler_GetIPHeaderLength(IPv6)){
        packetLength = messageHandler_GetTotalPacketLength((const uint8_t *) &fixedHeader);
    }else{
        if(res < 0){
 ...
 */
static AddressPair *
_constructAddressPair(UdpListener *udp, struct sockaddr *peerIpAddress, socklen_t peerIpAddressLength)
{
    Address *remoteAddress;

    switch (peerIpAddress->sa_family) {
        case AF_INET:
            remoteAddress = addressCreateFromInet((struct sockaddr_in *) peerIpAddress);
            break;

        case AF_INET6:
            remoteAddress = addressCreateFromInet6((struct sockaddr_in6 *) peerIpAddress);
            break;

        default:
            parcTrapIllegalValue(peerIpAddress, "Peer address unrecognized family for IP: %d", peerIpAddress->sa_family);
    }
 ...
 */
static bool
_lookupConnectionId(UdpListener *udp, AddressPair *pair, unsigned *outputConnectionIdPtr)
{
...
static unsigned
_createNewConnection(UdpListener *udp, int fd, const AddressPair *pair)
{
  bool isLocal = false;
...
static void
_handleProbeMessage(UdpListener *udp, uint8_t * msgBuffer)
{
    //TODO
...
static void
_handleWldrNotification(UdpListener * udp, unsigned connId, uint8_t * msgBuffer)
{
    const Connection * conn =  connectionTable_FindById(forwarder_GetConnectionTable(udp->forwarder), connId);
    if(conn == NULL){
        return;
    }
  ...
  Message * message =
        message_CreateFromByteArray(connId, msgBuffer,
...
static Message *
_readMessage(UdpListener *udp, int fd, size_t packetLength, AddressPair *pair)
{
...
static void
_receivePacket(UdpListener *udp, int fd, size_t packetLength, struct sockaddr_storage *peerIpAddress, socklen_t peerIpAddressLength)
{
    AddressPair *pair = _constructAddressPair(udp, (struct sockaddr *) peerIpAddress, peerIpAddressLength);
...
static void
_readFrameToDiscard(UdpListener *udp, int fd)
{
