...
static void _tcpListener_Listen(int, struct sockaddr *, int socklen, void *tcpVoid);


ListenerOps *
tcpListener_CreateInet6(Forwarder *forwarder, struct sockaddr_in6 sin6)
{
    _TcpListener *tcp = parcMemory_AllocateAndClear(sizeof(_TcpListener));
    parcAssertNotNull(tcp, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(_TcpListener));

    tcp->forwarder = forwarder;
    tcp->logger = logger_Acquire(forwarder_GetLogger(forwarder));

    tcp->listener = dispatcher_CreateListener(forwarder_GetDispatcher(forwarder), _tcpListener_Listen,
...
ListenerOps *
tcpListener_CreateInet(Forwarder *forwarder, struct sockaddr_in sin)
{
    _TcpListener *tcp = parcMemory_AllocateAndClear(sizeof(_TcpListener));
    parcAssertNotNull(tcp, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(_TcpListener));
...
static void
_tcpListener_Destroy(_TcpListener **listenerPtr)
{
  ...
  logger_Release(&tcp->logger);
  dispatcher_DestroyListener(forwarder_GetDispatcher(tcp->forwarder), &tcp->listener);
...
_tcpListener_Listen(int fd, struct sockaddr *sa, int socklen, void *tcpVoid)
{
    ...
    IoOperations *ops = streamConnection_AcceptConnection(tcp->forwarder, fd, pair, tcp->isLocalAddressLocal);
...
static void
_tcpListener_OpsDestroy(ListenerOps **listenerOpsPtr)
{
...
static unsigned
_tcpListener_OpsGetInterfaceIndex(const ListenerOps *ops)
{
    _TcpListener *tcp = (_TcpListener *) ops->context;
    return tcp->id;
}
...
_tcpListener_OpsGetListenAddress(const ListenerOps *ops)
{
    _TcpListener *tcp = (_TcpListener *) ops->context;
...
static EncapType
_tcpListener_OpsGetEncapType(const ListenerOps *ops)
{
    return ENCAP_TCP;
}
