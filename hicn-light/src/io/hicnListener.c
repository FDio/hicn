...
struct hicn_listener {
    ...
    Address *localAddress; //this is the local address  or 0::0 in case of the main listener
                              //this is the address used inside forwarder to identify the listener. Notice that this
                              //address is the same as the fisical interfaces on which we create the TUN. it is
                              //NOT the TUN address which is given by libhicn after the bind operation
                              //However the user alway uses this address since is the only one available at
                              //configuration time

    unsigned inetFamily;


    int connection_id; //this is used only if the listener is used to receive data packets
                            //we assume that 1 connection is associated to one listener in this case
                            //so we set the connection_id we the connection is create. if this id is
                            //not set and a data packet is received, the packet is dropped
...
static bool
_isEmptyAddressIPv6(Address * address)
{
    struct sockaddr_in6 * addr6 = parcMemory_AllocateAndClear(sizeof(struct sockaddr_in6));
    parcAssertNotNull(addr6, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(addr6));
...
static bool
_isEmptyAddressIPv4(Address * address)
{
    bool res = false;

    if(strcmp("inet4://0.0.0.0:1234",addressToString(address)) == 0)
        res = true;
    return res;
}
...
ListenerOps *
hicnListener_CreateInet(Forwarder *forwarder, char *symbolic, Address *address)
{
    HIcnListener *hicn = parcMemory_AllocateAndClear(sizeof(HIcnListener));
    parcAssertNotNull(hicn, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(HIcnListener));
    ...
    hicn_socket_helper_t * hicnSocketHelper = forwarder_GetHIcnSocketHelper(forwarder);

    if(_isEmptyAddressIPv4(address)){
        hicn->hicn_fd = hicn_socket(hicnSocketHelper, symbolic, NULL);
    } else {

        struct sockaddr_in * tmpAddr = parcMemory_AllocateAndClear(sizeof(struct sockaddr_in));
        parcAssertNotNull(tmpAddr, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(tmpAddr));
    ...
    parcAssertTrue(flags != -1, "fcntl failed to obtain file descriptor flags (%d)", errno);
    int failure = fcntl(hicn->hicn_fd, F_SETFL, flags | O_NONBLOCK);
    parcAssertFalse(failure, "fcntl failed to set file descriptor flags (%d)", errno);

    hicn->hicn_event = dispatcher_CreateNetworkEvent(forwarder_GetDispatcher(forwarder),
                                                            true,
                                                            _hicnListener_readcb,
                                                            (void *) hicn,
                                                            hicn->hicn_fd);
    dispatcher_StartNetworkEvent(forwarder_GetDispatcher(forwarder), hicn->hicn_event);

    ListenerOps *ops = parcMemory_AllocateAndClear(sizeof(ListenerOps));
    parcAssertNotNull(ops, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ListenerOps));
...
ListenerOps *
hicnListener_CreateInet6(Forwarder *forwarder, char *symbolic, Address *address)
{
    HIcnListener *hicn = parcMemory_AllocateAndClear(sizeof(HIcnListener));
    parcAssertNotNull(hicn, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(HIcnListener));
    ...
    //in both cases we need to set only the identifier. In the case of normal listener
    //(listener for data packet) we let the library select the right ip address
    //we just need to set the right type of packet
	...
	//create listener for the connetion
	    struct sockaddr_in6 * tmpAddr = parcMemory_AllocateAndClear(sizeof(struct sockaddr_in6));
...
_hicnListener_BindInet6(ListenerOps *ops, const Address *remoteAddress)
{
    ...
    hicn_socket_helper_t * hicnSocketHelper = forwarder_GetHIcnSocketHelper(hicn->forwarder);
...
hicnListener_Punting(ListenerOps *ops, const char *prefix)
{
    ...
    if(res < 0){
        if (logger_IsLoggable(hicn->logger, LoggerFacility_IO, PARCLogLevel_Debug)) {                                                                                          logger_Log(hicn->logger, LoggerFacility_IO, PARCLogLevel_Debug, __func__,
...
_hicnListener_Destroy(HIcnListener **listenerPtr)
{
    ...
    parcAssertNotNull(*listenerPtr, "Parameter must derefernce to non-null pointer");

    HIcnListener *hicn = *listenerPtr;

    //close(hicn->hicn_fd); //XXX close the fd in the hicnlib (detroy listener?)
    dispatcher_DestroyNetworkEvent(forwarder_GetDispatcher(hicn->forwarder), &hicn->hicn_event);
    logger_Release(&hicn->logger);
    addressDestroy(&hicn->localAddress);
    parcMemory_Deallocate((void **) &hicn);
    *listenerPtr = NULL;
...
static int
_getSocket(const ListenerOps *ops)
{
    HIcnListener *hicn = (HIcnListener *) ops->context;
    return hicn->hicn_fd;
}
...
_readFrameToDiscard(HIcnListener *hicn, int fd)
{
    ...
    if (nread > 0) {
        ...
        printf("Error trying to discard frame from fd %d: (%d) %s", fd, errno, strerror(errno));
        if (logger_IsLoggable(hicn->logger, LoggerFacility_IO, PARCLogLevel_Error)) {
...
static unsigned
_createNewConnection(HIcnListener *hicn, int fd, const AddressPair *pair)
{
    bool isLocal = false;
...
const Connection *
_findConnectionFromPacket(HIcnListener *hicn, Address *packetSourceAddress){

    const Connection * conn = NULL;
    if(hicn->connection_id != -1){
        conn = connectionTable_FindById(forwarder_GetConnectionTable(hicn->forwarder), hicn->connection_id);
    }else{
        if(packetSourceAddress != NULL){
            //in this first check we try to retrieve the standard connection generated by the hicn-light
            AddressPair *pair = addressPair_Create(hicn->localAddress, packetSourceAddress);
            conn = connectionTable_FindByAddressPair(forwarder_GetConnectionTable(hicn->forwarder), pair);
...
static Address *
_createAddressFromPacket(uint8_t * msgBuffer)
{
    ...
    if(messageHandler_GetIPPacketType(msgBuffer) == IPv6_TYPE) {
        ...
        memcpy(&addr_in6.sin6_addr, (struct in6_addr *)messageHandler_GetSource(msgBuffer), 16);
        ...
        memcpy(&addr_in.sin_addr, (struct in_addr *)messageHandler_GetSource(msgBuffer), 4);
...
static void
_handleProbeMessage(HIcnListener *hicn, uint8_t * msgBuffer)
{
    ...
    if(packetAddr != NULL){
        ...
        if(conn != NULL){
            ...
            connection_HandleProbe((Connection *) conn, msgBuffer, forwarder_GetTicks(hicn->forwarder));
        }
    }
...
static void
_handleWldrNotification(HIcnListener * hicn,  uint8_t * msgBuffer)
{
    Address *packetAddr = _createAddressFromPacket(msgBuffer);
    ...
    Message * message =
        message_CreateFromByteArray(connection_GetConnectionId(conn), msgBuffer,
...
#ifdef WITH_MAPME
static void
_handleMapMe(HIcnListener * hicn, int fd, uint8_t * msgBuffer)
{
    Address *packetAddr = _createAddressFromPacket(msgBuffer);
    ...
    if(conn == NULL) {
        ...
        ip_address_t remote_address = { .family = AF_INET6, .prefix_len = IPV6_ADDR_LEN_BITS };
        memcpy(&remote_address.buffer, &sockaddr.sin6_addr, ip_address_len(&remote_address));
        ...
        IoOperations *ops = hicnTunnel_Create(hicn->forwarder, localAddr, packetAddr);
        ...
        connectionTable_Add(forwarder_GetConnectionTable(hicn->forwarder), (Connection *)conn);
    }
...
static Message *
_readMessage(HIcnListener *hicn, int fd, uint8_t * msgBuffer)
{
    ...
    if(messageHandler_IsTCP(msgBuffer)){
        ...
        if(messageHandler_IsData(msgBuffer)){
            ...
            //notice that the connections for the interest (the one that we create at run time)
            //uses as a local address 0::0, so the main tun
        ...
        message = message_CreateFromByteArray(connid, msgBuffer, 
                pktType,
                forwarder_GetTicks(hicn->forwarder),
                forwarder_GetLogger(hicn->forwarder));
        if(message == NULL){
...
static void
_receivePacket(HIcnListener *hicn, int fd)
{
    Message *msg = NULL;
...
static void
_hicnListener_readcb(int fd, PARCEventType what, void *hicnVoid)
{
