 ...
 *
 */
void streamBuffer_SetWatermark(PARCEventQueue *buffer, bool setRead, bool setWrite, size_t low, size_t high);
 ...
 */
int streamBuffer_FlushCheckpoint(PARCEventQueue *buffer, bool flushRead, bool flushWrite);

/**
 * @function streamBuffer_FlushFinished
 * @abstract Flush the stream and indicate the end of new data
 */
int streamBuffer_FlushFinished(PARCEventQueue *buffer, bool flushRead, bool flushWrite);

/**
 * @typedef StreamBufferReadWriteCallback
 * @abstract Callback when data is available or write space available
 * @constant user_data opaque data passed to <code>StreamBuffer_SetCallbacks()</code>
 */
typedef void (StreamBufferReadWriteCallback)(PARCEventQueue *buffer, void *user_data);

/**
 * @typedef StreamBufferEventCallback
 * @abstract Callback on error or other event on the stream buffer
 * @constant what logical or of STREAM events.  STREAM_READING and STREAM_WRITING
 *                indicate if the error was on the read or write direction.  The conditions
 *                may be STREAM_ERROR, STREAM_EOF, STREAM_TIMEOUT, or STREAM_CONNECTED.
 * @constant user_data opaque data passed to <code>StreamBuffer_SetCallbacks()</code>
 */
typedef void (StreamBufferEventCallback)(PARCEventQueue *buffer, short what, void *user_data);
 ...
 */
void streamBuffer_EnableCallbacks(PARCEventQueue *buffer, bool enableRead, bool enableWrite);
 ...
 */
void streamBuffer_DisableCallbacks(PARCEventQueue *buffer, bool disableRead, bool disableWrite);
