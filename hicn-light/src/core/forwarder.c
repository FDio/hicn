 ...
 *
 * Packet processing is done in dispatcher.c, which is the actual wrapper around the event scheduler
 */
...
struct forwarder {
    ...
    hicn_socket_helper_t * hicnSocketHelper; //state required to manage hicn connections
 ...
 */
static void
forwarder_Seed(Forwarder *forwarder)
{
    int fd;
    ssize_t res;
...
Logger *
forwarder_GetLogger(const Forwarder *forwarder)
{
...
Forwarder *
forwarder_Create(Logger *logger)
{
    ...
    forwarder->signal_term = dispatcher_CreateSignalEvent(forwarder->dispatcher, _signal_cb, forwarder, SIGTERM);
    dispatcher_StartSignalEvent(forwarder->dispatcher, forwarder->signal_term);

    forwarder->signal_int = dispatcher_CreateSignalEvent(forwarder->dispatcher, _signal_cb, forwarder, SIGINT);
    dispatcher_StartSignalEvent(forwarder->dispatcher, forwarder->signal_int);

    forwarder->signal_usr1 = dispatcher_CreateSignalEvent(forwarder->dispatcher, _signal_cb, forwarder, SIGPIPE);
    dispatcher_StartSignalEvent(forwarder->dispatcher, forwarder->signal_usr1);

#ifndef __APPLE__
    forwarder->hicnSocketHelper = hicn_create();
    if(forwarder->hicnSocketHelper == NULL)
        return NULL;
#endif /* __APPLE__ */
    /* ignore child */
...
#ifdef WITH_MAPME
    if (!(mapMe_Init(&forwarder->mapme, forwarder)))
        return NULL;
#endif /* WITH_MAPME */

    PARCEventScheduler *base = dispatcher_GetEventScheduler(forwarder->dispatcher);
    forwarder->keepalive_event = parcEventTimer_Create(base, PARCEventType_Persist, _keepalive_cb, (void *) forwarder);
...
forwarder_Destroy(Forwarder **ptr)
{
    ...
    dispatcher_DestroySignalEvent(forwarder->dispatcher, &(forwarder->signal_int));
    dispatcher_DestroySignalEvent(forwarder->dispatcher, &(forwarder->signal_term));
    dispatcher_DestroySignalEvent(forwarder->dispatcher, &(forwarder->signal_usr1));
...
void
forwarder_SetupAllListeners(Forwarder *forwarder, uint16_t port, const char *localPath)
{
...
void
forwarder_SetupFromConfigFile(Forwarder *forwarder, const char *filename)
{
...
Configuration *
forwarder_GetConfiguration(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
unsigned
forwarder_GetNextConnectionId(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
Messenger *
forwarder_GetMessenger(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
Dispatcher *
forwarder_GetDispatcher(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
ConnectionTable *
forwarder_GetConnectionTable(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
ListenerSet *
forwarder_GetListenerSet(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
void
forwarder_SetChacheStoreFlag(Forwarder *forwarder, bool val)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
bool
forwarder_GetChacheStoreFlag(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
void
forwarder_SetChacheServeFlag(Forwarder *forwarder, bool val)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
bool
forwarder_GetChacheServeFlag(Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
void
forwarder_ReceiveCommand(Forwarder *forwarder, command_id command, struct iovec *message, unsigned ingressId)
{
    configuration_ReceiveCommand(forwarder->config, command, message, ingressId);
}

void
forwarder_Receive(Forwarder *forwarder, Message *message)
{
    ...
    const Connection *conn =
        connectionTable_FindById(forwarder->connectionTable, message_GetIngressConnectionId(message));
    ...
    if(message_HasWldr(message)){
        ...
        } else if(!connection_HasWldr(conn) && connection_WldrAutoStartAllowed(conn)){
...
Ticks
forwarder_GetTicks(const Forwarder *forwarder)
{
    parcAssertNotNull(forwarder, "Parameter must be non-null");
...
Ticks
forwarder_NanosToTicks(uint64_t nanos)
{
    return NSEC_TO_TICKS(nanos);
}

uint64_t
forwarder_TicksToNanos(Ticks ticks)
{
    return (1000000000ULL) * ticks / HZ;
}

bool
forwarder_AddOrUpdateRoute(Forwarder *forwarder, add_route_command *control, unsigned ifidx)
{
    parcAssertNotNull(forwarder, "Parameter hicn-light must be non-null");
    parcAssertNotNull(control, "Parameter route must be non-null");

    // we only have one message processor
    bool res = messageProcessor_AddOrUpdateRoute(forwarder->processor, control, ifidx);

    return res;
}

bool
forwarder_RemoveRoute(Forwarder *forwarder, remove_route_command *control, unsigned ifidx)
{
    parcAssertNotNull(forwarder, "Parameter hicn-light must be non-null");
    parcAssertNotNull(control, "Parameter route must be non-null");
...
void
forwarder_RemoveConnectionIdFromRoutes(Forwarder *forwarder, unsigned connectionId)
{
    parcAssertNotNull(forwarder, "Parameter hicn-light must be non-null");
    messageProcessor_RemoveConnectionIdFromRoutes(forwarder->processor, connectionId);
}

void
forwarder_SetStrategy(Forwarder *forwarder, Name *prefix, strategy_type strategy)
{
    parcAssertNotNull(forwarder, "Parameter hicn-light must be non-null");
...
FibEntryList *
forwarder_GetFibEntries(Forwarder *forwarder)
{
    return messageProcessor_GetFibEntries(forwarder->processor);
}

void
forwarder_SetContentObjectStoreSize(Forwarder *forwarder, size_t maximumContentStoreSize)
{
    messageProcessor_SetContentObjectStoreSize(forwarder->processor, maximumContentStoreSize);
}

void
forwarder_ClearCache(Forwarder *forwarder)
{
    messageProcessor_ClearCache(forwarder->processor);
}

PARCClock *
forwarder_GetClock(const Forwarder *forwarder)
{
...
_signal_cb(int sig, PARCEventType events, void *user_data)
{
    ...
    logger_Log(forwarder->logger, LoggerFacility_Core, PARCLogLevel_Warning, __func__,
...
{
    parcAssertTrue(what & PARCEventType_Timeout, "Got unexpected tick_cb: %d", what);
    // function is just a keepalive for hicn-light, does not do anything
}

#ifdef WITH_MAPME
FIB *
forwarder_getFib(Forwarder * forwarder)
{
    return messageProcessor_getFib(forwarder->processor);
}

void
forwarder_onConnectionAdded(Forwarder * forwarder, const Connection * conn)
{
    mapMe_onConnectionAdded(forwarder->mapme, conn);
}

void
forwarder_onConnectionRemoved(Forwarder * forwarder, const Connection * conn)
{
}


void
forwarder_ProcessMapMe(Forwarder * forwarder, uint8_t * msgBuffer, unsigned conn_id)
{
