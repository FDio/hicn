...
wldr_Init()
{
    ...
    parcAssertNotNull(wldr, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Wldr));
    wldr->expected_label = 1;
    wldr->next_label = 1;
    for (int i = 0; i < BUFFER_SIZE; i++) {
        WldrBuffer *entry = parcMemory_AllocateAndClear(sizeof(WldrBuffer));
        parcAssertNotNull(entry, "WldrBuffer init: parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(WldrBuffer));
        entry->message = NULL;
        entry->rtx_counter = 0;
        wldr->buffer[i] = entry;
    }
...
static void
_wldr_RetransmitPacket(Wldr *wldr, const Connection *conn, uint16_t label)
{
...
static void
_wldr_SendWldrNotificaiton(Wldr *wldr, const Connection *conn, Message *message, uint16_t expected_lbl, uint16_t received_lbl)
{
    //here we need to create a new packet that is used to send the wldr notification to the prevoius hop.
    //the destionation address of the notification is the source address of the message for which we want to
    //create a notification. in fact, if message is an interest the prevoius hop is identified by the src.
    //if message is a data, we need to send the notification message with the content name has a source address
    //in this way the message will be trapped by the pounting rules in the next hop
    //We define the notification as an interest message so that the NAT in the send function
    //will set the src address of the local connection. Notice that in this way the notification packet will be
    //dispaced to the right connection at the next hop.

    Message *notification = message_CreateWldrNotification(message, expected_lbl, received_lbl);
...
void
wldr_SetLabel(Wldr *wldr, Message *message)
{
    ...
    wldr->next_label++;
    if(wldr->next_label == 0) //we alwasy skip label 0 beacause it means that wldr is not active
        wldr->next_label++;
}

void
wldr_DetectLosses(Wldr *wldr, const Connection *conn, Message *message)
{
    if (message_HasWldr(message)) {
        ...
        if (pkt_lbl != wldr->expected_label) {
            ...
            //usually we are in the case where a remote note disconnected for a while
            ...
            if ((pkt_lbl != 1) || (wldr->expected_label < pkt_lbl)) {
                _wldr_SendWldrNotificaiton(wldr, conn, message, wldr->expected_label, pkt_lbl);
            }
...
void
wldr_HandleWldrNotification(Wldr *wldr, const Connection *conn, Message *message)
{
