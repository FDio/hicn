...
#define NEVER_EXPIRE 16777216       //2^16 (max urgent pointer) * 2^8 (max reserved + NS bits)
...
messageHandler_GetIPPacketType(const uint8_t *message)
{
    return HICN_IP_VERSION(message);
}

static inline void
messageHandler_UpdateTCPCheckSum(uint8_t *message,  uint16_t *old_val, uint16_t *new_val, uint8_t size)
{
...
static inline void
messageHandler_UpdateIPv4CheckSum(uint8_t *message,  uint16_t *old_val, uint16_t *new_val, uint8_t size)
{
    for(uint8_t i = 0; i < size; i++){
...
messageHandler_GetEmptyTCPPacketSize(unsigned ipVersion)
{
...
static inline size_t
messageHandler_GetICMPPacketSize(unsigned ipVersion)
{
    if(ipVersion == IPv4_TYPE)
...
static inline size_t
messageHandler_GetIPHeaderLength(unsigned ipVersion)
{
    if(ipVersion == IPv4_TYPE)
        return IPV4_HDRLEN;
    else if (ipVersion == IPv6_TYPE)
...
static inline  bool
messageHandler_IsValidHIcnPacket(const uint8_t *message)
{
...
static inline uint8_t
messageHandler_NextHeaderType(const uint8_t * message)
{
    switch (messageHandler_GetIPPacketType(message)) {
        case IPv6_TYPE:
...
static inline bool
messageHandler_IsTCP(const uint8_t *message) 
{
    if(messageHandler_NextHeaderType(message) != IPPROTO_TCP)
        return false;
    return true;
...
static inline bool
messageHandler_IsInterest(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return false;

    bool flag;
    hicn_packet_test_ece((hicn_header_t *) message, &flag); //ECE flag is set to 0 in interest packets
    if(flag == false)
        return true;
    return false;
}
...
static inline bool
messageHandler_IsData(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return false;

    bool flag;
    hicn_packet_test_ece((hicn_header_t *) message, &flag); //ECE flag is set to 1 in data packets
    if(flag == true)
        return true;
    return false;
}
...
static inline bool
messageHandler_IsWldrNotification(const uint8_t *message)
{
    //this function returns true only if the packet is an ICMP packet in Wldr form.
    //type must be equal to ICMP_WLDR_TYPE and code equal to ICMP_WLDR_CODE
...
static inline bool
messageHandler_IsLoadBalancerProbe(const uint8_t *message)
{
...
messageHandler_GetTotalPacketLength(const uint8_t *message)
{
    switch (messageHandler_GetIPPacketType(message)) {
        case IPv6_TYPE:
...
messageHandler_GetSegment(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return 0;
...
static inline uint16_t
messageHandler_GetExpectedWldrLabel(const uint8_t *message)
{
    const uint8_t * icmp_ptr;
    switch (messageHandler_GetIPPacketType(message)) {
        case IPv6_TYPE:
...
static inline uint16_t
messageHandler_GetWldrLastReceived(const uint8_t *message)
{
    const uint8_t * icmp_ptr;
    switch (messageHandler_GetIPPacketType(message)) {
        case IPv6_TYPE:
...
static inline uint16_t
messageHandler_GetWldrLabel(const uint8_t *message)
{
    switch (messageHandler_GetIPPacketType(message)) {
        case IPv6_TYPE:
...
static inline void
messageHandler_SetWldrLabel(uint8_t *message, uint16_t label)
{
...
static inline void
messageHandler_ResetWldrLabel(uint8_t *message)
{
    messageHandler_SetWldrLabel(message, 0);
}

static inline bool
messageHandler_HasWldr(const uint8_t *message)
{
    if(messageHandler_IsTCP(message)){
...
static inline uint8_t
messageHandler_GetProbePacketType(const uint8_t *message)
{
    const uint8_t * icmp_ptr;
    switch (messageHandler_GetIPPacketType(message)) {
        case IPv6_TYPE:
...
static inline uint32_t
messageHandler_GetPathLabel(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return 0;
...
static inline void
messageHandler_SetPathLabel(uint8_t *message, uint32_t new_path_label)
{
    if(!messageHandler_IsTCP(message))
        return;
    ...
    messageHandler_UpdateTCPCheckSum(message, (uint16_t *) &old_path_label, (uint16_t *) &new_path_label, 2);
}


static inline void
messageHandler_UpdatePathLabel(uint8_t *message, uint8_t outFace)
{
    if(!messageHandler_IsTCP(message))
        return;
    ...
    uint32_t pl_new_32bit = (uint32_t) ((((pl_old_8bit << 1) | (pl_old_8bit >> 7)) ^ outFace) << 24UL);

    hicn_data_set_path_label((hicn_header_t *) message, pl_new_32bit);

    messageHandler_UpdateTCPCheckSum(message, (uint16_t *) &pl_old_32bit, (uint16_t *) &pl_new_32bit, 2);
}

static inline void
messageHandler_ResetPathLabel(uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return;
...
static inline uint16_t
messageHandler_GetInterestLifetime(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return 0;
...
static inline bool
messageHandler_HasInterestLifetime(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return false;

    if(messageHandler_GetInterestLifetime(message) == 0)
        return false;
    return true;
}

static inline uint32_t
messageHandler_GetContentExpiryTime(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return 0;

    uint32_t expirationTime;
    int res = hicn_data_get_expiry_time((hicn_header_t *) message, &expirationTime);
    if(res < 0)
        return 0;
...
static inline bool
messageHandler_HasContentExpiryTime(const uint8_t *message)
{
    if(!messageHandler_IsTCP(message))
        return 0;
    ...
    if(expirationTime == NEVER_EXPIRE)
        return false;
...
static inline void *
messageHandler_GetSource(const uint8_t *message){
...
static inline void *
messageHandler_GetDestination(const uint8_t *message){
...
static inline void
messageHandler_SetSource_IPv6(uint8_t *message, struct in6_addr *address)
{
...
static inline void
messageHandler_SetDestination_IPv6(uint8_t *message, struct in6_addr *address)
{
    if(messageHandler_IsTCP(message)){
...
static inline void
messageHandler_SetSource_IPv4(uint8_t *message, uint32_t *address)
{
    //update tcp checksum
    uint16_t *old_src = (uint16_t *) messageHandler_GetSource(message);
    if(messageHandler_IsTCP(message)){
    ...
    //update IPv4 cheksum
    //the IPv4 checksum is not part of the psudo header for TCP checksum calculation
    //we can update them separetelly
...
static inline void
messageHandler_SetDestination_IPv4(uint8_t *message, uint32_t *address)
{
    uint16_t *old_dst = (uint16_t *) messageHandler_GetDestination(message);
    if(messageHandler_IsTCP(message)){
...
static inline void
messageHandler_SetWldrNotification(uint8_t *notification, uint8_t * original, uint16_t expected, uint16_t received)
{
    ...
    switch (messageHandler_GetIPPacketType(original)) {
        case IPv6_TYPE:{
            *h = (hicn_header_t) {
            ...
            messageHandler_SetSource_IPv6(notification, (struct in6_addr *) messageHandler_GetDestination(original));
            messageHandler_SetDestination_IPv6(notification, (struct in6_addr *) messageHandler_GetSource(original));
            break;
        }
        case IPv4_TYPE:{
            break;
...
static inline void
messageHandler_SetProbePacket(uint8_t *message, uint8_t probeType, struct in6_addr *src, struct in6_addr *dst)
{
