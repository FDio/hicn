...
static bool
connectionTable_ConnectionIdEquals(const void *keyA, const void *keyB)
{
...
static int
connectionTable_ConnectionIdCompare(const void *keyA, const void *keyB)
{
    unsigned idA = *((unsigned  *) keyA);
    unsigned idB = *((unsigned  *) keyB);
    if (idA < idB) {
...
static bool
connectionTable_AddressPairEquals(const void *keyA, const void *keyB)
{
...
static HashCodeType
connectionTable_ConnectionIdHashCode(const void *keyA)
{
    unsigned idA = *((unsigned  *) keyA);
...
static HashCodeType
connectionTable_AddressPairHashCode(const void *keyA)
{
    const AddressPair *pairA = (const AddressPair *) keyA;
...
static void
connectionTable_ConnectionIdDestroyer(void **dataPtr)
{
...
static void
connectionTable_ConnectionDestroyer(void **dataPtr)
{
...
connectionTable_Create()
{
    ...
    ConnectionTable *conntable = parcMemory_AllocateAndClear(sizeof(ConnectionTable));
    parcAssertNotNull(conntable, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ConnectionTable));

    conntable->storageTableById = parcHashCodeTable_Create_Size(connectionTable_ConnectionIdEquals,
                                                                connectionTable_ConnectionIdHashCode,
                                                                connectionTable_ConnectionIdDestroyer,
                                                                connectionTable_ConnectionDestroyer,
                                                                initialSize);

    // no key or data destroyer, this is an index into storageByid.
    conntable->indexByAddressPair = parcHashCodeTable_Create_Size(connectionTable_AddressPairEquals,
                                                                  connectionTable_AddressPairHashCode,
                                                                  NULL,
                                                                  NULL,
                                                                  initialSize);

    conntable->listById = parcTreeRedBlack_Create(connectionTable_ConnectionIdCompare,
                                                  NULL,  // key free
                                                  NULL,  // key copy
                                                  NULL,  // value equals
                                                  NULL,  // value free
...
void
connectionTable_Destroy(ConnectionTable **conntablePtr)
{
    parcAssertNotNull(conntablePtr, "Parameter must be non-null double pointer");
    parcAssertNotNull(*conntablePtr, "Parameter must dereference to non-null pointer");
 ...
 */
void
connectionTable_Add(ConnectionTable *table, Connection *connection)
{
    ...
    parcAssertNotNull(connectionIdKey, "parcMemory_Allocate(%zu) returned NULL", sizeof(unsigned));
    *connectionIdKey = connection_GetConnectionId(connection);
    ...
    if (parcHashCodeTable_Add(table->storageTableById, connectionIdKey, connection)) {
        parcHashCodeTable_Add(table->indexByAddressPair, (void *) connection_GetAddressPair(connection), connection);
        parcTreeRedBlack_Insert(table->listById, connectionIdKey, connection);
    } else {
         parcTrapUnexpectedState("Could not add connection id %u -- is it a duplicate?", *connectionIdKey);
    }
}
 ...
 */
void
connectionTable_Remove(ConnectionTable *table, const Connection *connection)
{
    parcAssertNotNull(table, "Parameter table must be non-null");
    parcAssertNotNull(connection, "Parameter connection must be non-null");
    ...
    parcHashCodeTable_Del(table->indexByAddressPair, connection_GetAddressPair(connection));
    parcHashCodeTable_Del(table->storageTableById, &connid);
}

void
connectionTable_RemoveById(ConnectionTable *table, unsigned id)
{
    parcAssertNotNull(table, "Parameter table must be non-null");
...
const Connection *
connectionTable_FindByAddressPair(ConnectionTable *table, const AddressPair *pair)
{
    parcAssertNotNull(table, "Parameter table must be non-null");
...
const Connection *
connectionTable_FindById(ConnectionTable *table, unsigned id)
{
    parcAssertNotNull(table, "Parameter table must be non-null");
...
ConnectionList *
connectionTable_GetEntries(const ConnectionTable *table)
{
    parcAssertNotNull(table, "Parameter table must be non-null");
