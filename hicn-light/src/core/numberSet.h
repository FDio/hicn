 ...
 * The reference count is increased by one.  It must be released with NumberSet_Release().
 ...
 * @param [in,out] setPtr A pointer to a NumberSet.  Will be NULL'd after release.
 */
 ...
 */
NumberSet *numberSet_Subtract(const NumberSet *minuend, const NumberSet *subtrahend);
 ...
 *
 * The following equivalence relations on non-null `NumberSet` instances are maintained:
 *
 *  * It is reflexive: for any non-null reference value x, `NumberSet_Equals(x, x)`
 *      must return true.
 *
