...
numberSet_Create()
{
    ...
    parcAssertNotNull(set, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(NumberSet));
    set->arrayOfNumbers = parcMemory_AllocateAndClear(sizeof(Number) * 16);
    parcAssertNotNull((set->arrayOfNumbers), "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Number) * 16);
    set->length = 0;
    set->limit = 16;
    set->refcount = 1;
    return set;
...
NumberSet *
numberSet_Acquire(const NumberSet *original)
{
...
void
numberSet_Release(NumberSet **setPtr)
{
    ...
    parcAssertTrue(set->refcount > 0, "Invalid state: calling destroy on an object with 0 reference count");
    set->refcount--;
 ...
 */
static void
numberSet_AddNoChecks(NumberSet *set, Number number)
{
...
bool
numberSet_Add(NumberSet *set, Number number)
{
...
size_t
numberSet_Length(const NumberSet *set)
{
    parcAssertNotNull(set, "Parameter set must be non-null");
    return set->length;
}
...
Number
numberSet_GetItem(const NumberSet *set, size_t ordinalIndex)
{
    parcAssertNotNull(set, "Parameter set must be non-null");
    parcAssertTrue(ordinalIndex < set->length, "Limit beyond end of set, length %zu got %zu", set->length, ordinalIndex);

    return set->arrayOfNumbers[ordinalIndex];
}

bool
numberSet_Contains(const NumberSet *set, Number number)
{
    parcAssertNotNull(set, "Parameter set must be non-null");
...
void
numberSet_AddSet(NumberSet *destinationSet, const NumberSet *setToAdd)
{
    parcAssertNotNull(destinationSet, "Parameter destinationSet must be non-null");
...
NumberSet *
numberSet_Subtract(const NumberSet *minuend, const NumberSet *subtrahend)
{
...
bool
numberSet_Equals(const NumberSet *a, const NumberSet *b)
{
    if (a == NULL && b == NULL) {
        return true;
    }
...
void
numberSet_Remove(NumberSet *set, Number number)
{
    parcAssertNotNull(set, "Parameter set must be non-null");
    for (size_t i = 0; i < set->length; i++) {
        if (set->arrayOfNumbers[i] == number) {
...
static void
numberSet_Expand(NumberSet *set)
{
