...
Message *
message_Acquire(const Message *message)
{
...
Message *
message_CreateFromEventBuffer(PARCEventBuffer *data,
                                         size_t dataLength,
                                         unsigned ingressConnectionId,
                                         Ticks receiveTime,
                                         Logger *logger)
{
    ...
    parcAssertNotNull(message, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Message));
    ...
    parcAssertNotNull(message->messageHead, "parcMemory_AllocateAndClear(%zu) returned NULL", dataLength);
    ...
    message->name = name_CreateFromPacket(message->messageHead, message->packetType);
...
Message *
message_CreateFromByteArray(unsigned connid, uint8_t *pckt, MessagePacketType type,
...
void
message_Release(Message **messagePtr)
{
    parcAssertNotNull(messagePtr, "Parameter must be non-null double pointer");
    parcAssertNotNull(*messagePtr, "Parameter must dereference to non-null pointer");

    Message *message = *messagePtr;
    parcAssertTrue(message->refcount > 0, "Invalid state: message_Release called on message with 0 references %p", (void *) message);

    message->refcount--;
    if (message->refcount == 0) {
        if (logger_IsLoggable(message->logger, LoggerFacility_Message, PARCLogLevel_Debug)) {
            logger_Log(message->logger, LoggerFacility_Message, PARCLogLevel_Debug, __func__,
                              "Message %p destroyed",
                              (void *) message);
        }
...
bool
message_Write(PARCEventQueue *parcEventQueue, const Message *message)
{
    ...
    return parcEventQueue_Write(parcEventQueue, message->messageHead, message_Length(message));
}


size_t
message_Length(const Message *message)
{
...
bool
message_HasWldr(const Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
bool
message_IsWldrNotification(const Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
void
message_ResetWldrLabel(Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
unsigned
message_GetWldrLabel(const Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
unsigned
message_GetWldrExpectedLabel(const Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
unsigned
message_GetWldrLastReceived(const Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
void
message_SetWldrLabel(Message *message, uint16_t label)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
Message *
message_CreateWldrNotification(Message *original, uint16_t expected, uint16_t lastReceived)
{
    ...
    message->length = messageHandler_GetICMPPacketSize(messageHandler_GetIPPacketType(original->messageHead));
    message->messageHead = parcMemory_AllocateAndClear(message->length);
    parcAssertNotNull(message->messageHead, "parcMemory_AllocateAndClear returned NULL");

    message->packetType = MessagePacketType_WldrNotification;
    message->name = NULL; //nobody will use the name in a notification packet, so we can simply set it to NULL

    //set notification stuff.
    messageHandler_SetWldrNotification(message->messageHead, original->messageHead, expected, lastReceived);
...
void
message_SetIngressConnectionId(Message *message, unsigned conn)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
Ticks
message_GetReceiveTime(const Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
uint32_t
message_GetPathLabel(const Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
void
message_SetPathLabel(Message *message, uint32_t label)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
void
message_UpdatePathLabel(Message *message, uint8_t outFace)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
void
message_ResetPathLabel(Message *message)
{
    parcAssertNotNull(message, "Parameter must be non-null");
...
MessagePacketType
message_GetType(const Message *message)
{
...
Name *
message_GetName(const Message *message)
{
    parcAssertNotNull(message, "Parameter message must be non-null");
    return message->name;
}
...
bool
message_HasInterestLifetime(const Message *message)
{
    parcAssertNotNull(message, "Parameter message must be non-null");
...
uint64_t
message_GetInterestLifetimeTicks(const Message *message)
{
    parcAssertNotNull(message, "Parameter message must be non-null");
...
bool
message_HasContentExpiryTime(const Message *message)
{
    parcAssertNotNull(message, "Parameter message must be non-null");
...
uint64_t
message_GetContentExpiryTimeTicks(const Message *message)
{
    parcAssertNotNull(message, "Parameter message must be non-null");
...
const uint8_t *
message_FixedHeader(const Message *message)
{
    parcAssertNotNull(message, "Parameter message must be non-null");
