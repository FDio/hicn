...
//assumption: the IPv6 address is the name, the TCP segment number is the ICN segment
...
{
    parcAssertTrue(*name->refCountPtr > 0, "Illegal State: Trying to increment a 0 refcount!");
    (*name->refCountPtr)++;
}
...
{
    parcAssertTrue(*name->refCountPtr > 0, "Illegal State: Trying to decrement a 0 refcount!");
    (*name->refCountPtr)--;
}

static uint32_t
_computeHash(Name *name)
{
...
name_CreateFromPacket(const uint8_t *packet, MessagePacketType type)
{
    ...
    parcAssertNotNull(name, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Name));

    if(messageHandler_GetIPPacketType(packet) == IPv6_TYPE){
        if(type == MessagePacketType_Interest){
            name->content_name =
                nameBitvector_CreateFromIn6Addr((struct in6_addr *) messageHandler_GetDestination(packet), 128);
        }else if (type == MessagePacketType_ContentObject){
            name->content_name =
                nameBitvector_CreateFromIn6Addr((struct in6_addr *) messageHandler_GetSource(packet), 128);
        }else {
        ...
        if(type == MessagePacketType_Interest){
            name->content_name =
                nameBitvector_CreateFromInAddr(*((uint32_t *) messageHandler_GetDestination(packet)), 32);
        }else if (type == MessagePacketType_ContentObject){
            name->content_name =
                nameBitvector_CreateFromInAddr(*((uint32_t *) messageHandler_GetSource(packet)), 32);
        }else {
            parcMemory_Deallocate((void **) &name);
            return NULL;
        }
    ...
    parcAssertNotNull(name->refCountPtr, "parcMemory_Allocate(%zu) returned NULL", sizeof(unsigned));
    *name->refCountPtr = 1;
    return name;
}

Name *
name_CreateFromAddress(address_type addressType, union commandAddr addr, uint8_t len)
{
    Name * name = parcMemory_AllocateAndClear(sizeof(Name));
    parcAssertNotNull(name, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Name));
...
name_Acquire(const Name *original)
{
    ...
    parcAssertNotNull(copy, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(Name));
...
name_Equals(const Name *a, const Name *b)
{
    ...
    if((nameBitvector_Equals(a->content_name, b->content_name) && a->segment == b->segment))
        return true;
    return false;
...
bool
name_StartsWith(const Name *name, const Name *prefix)
{
...
name_ToString(const Name *name)
{
    ...
    sprintf(output, "name: %s seq: %u",addressToString(packetAddr),name->segment);
...
void
name_setLen(const Name *name, uint8_t len)
{
