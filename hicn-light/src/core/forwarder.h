...
// needs to be after the definition of Forwarder
 ...
 */
void forwarder_SetupAllListeners(Forwarder *forwarder, uint16_t port, const char *localPath);
...
void forwarder_ReceiveCommand(Forwarder *forwarder, command_id command, struct iovec *message, unsigned ingressId);
 ...
 */
bool forwarder_AddOrUpdateRoute(Forwarder *forwarder, add_route_command *control, unsigned ifidx);

/**
 * @function forwarder_RemoveRoute
 * @abstract Removes a route from all the message processors
 */
bool forwarder_RemoveRoute(Forwarder *forwarder, remove_route_command *control, unsigned ifidx);

/**
 * Removes a connection id from all routes
 */
void forwarder_RemoveConnectionIdFromRoutes(Forwarder *forwarder, unsigned connectionId);
 ...
 */
void forwarder_SetContentObjectStoreSize(Forwarder *forwarder, size_t maximumContentStoreSize);
...
void forwarder_SetStrategy(Forwarder *forwarder, Name *prefix, strategy_type strategy);
 ...
 */
void forwarder_onConnectionRemoved(Forwarder * forwarder, const Connection * conn);
 ...
 */
void forwarder_ProcessMapMe(Forwarder * forwarder, uint8_t * msgBuffer, unsigned conn_id);
