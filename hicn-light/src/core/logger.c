...
const char *
logger_FacilityString(LoggerFacility facility)
{
...
static void
_allocateLoggers(Logger *logger, PARCLogReporter *reporter)
{
    parcTrapUnexpectedStateIf(logger->reporter != NULL, "Trying to allocate a reporter when the previous one is not null");
    ...
    for (int i = 0; i < LoggerFacility_END; i++) {
        logger->loggerArray[i] = parcLog_Create(hostname, logger_FacilityString(i), "forwarder", logger->reporter);
        parcLog_SetLevel(logger->loggerArray[i], PARCLogLevel_Error);
    }
}

static void
_releaseLoggers(Logger *logger)
{
    for (int i = 0; i < LoggerFacility_END; i++) {
...
parcObject_ExtendPARCObject(Logger, _destroyer, NULL, NULL, NULL, NULL, NULL, NULL);
...
Logger *
logger_Create(PARCLogReporter *reporter, const PARCClock *clock)
{
...
void
logger_SetReporter(Logger *logger, PARCLogReporter *reporter)
{
...
void
logger_SetClock(Logger *logger, PARCClock *clock)
{
    parcAssertNotNull(logger, "Parameter logger must be non-null");
...
static void
_assertInvariants(const Logger *logger, LoggerFacility facility)
{

    parcAssertNotNull(logger, "Parameter logger must be non-null");
    parcTrapOutOfBoundsIf(facility >= LoggerFacility_END, "Invalid facility %d", facility);

}

void
logger_SetLogLevel(Logger *logger, LoggerFacility facility, PARCLogLevel minimumLevel)
{
...
bool
logger_IsLoggable(const Logger *logger, LoggerFacility facility, PARCLogLevel level)
{
    _assertInvariants(logger, facility);
    PARCLog *log = logger->loggerArray[facility];
