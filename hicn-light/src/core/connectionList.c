...
static void
connectionList_ArrayDestroyer(void **voidPtr)
{
...
connectionList_Create()
{
    ...
    parcAssertNotNull(list, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ConnectionList));
    list->listOfConnections = parcArrayList_Create(connectionList_ArrayDestroyer);
    return list;
}

void
connectionList_Destroy(ConnectionList **listPtr)
{
...
void
connectionList_Append(ConnectionList *list, Connection *entry)
{
...
size_t
connectionList_Length(const ConnectionList *list)
{
    parcAssertNotNull(list, "Parameter list must be non-null");
...
Connection *
connectionList_Get(ConnectionList *list, size_t index)
{
    parcAssertNotNull(list, "Parameter list must be non-null");
    Connection *original = (Connection *) parcArrayList_Get(list->listOfConnections, index);
    return original;
}
