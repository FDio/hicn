 ...
 *
 * @constant listener is the object created by <code>forwarder_NewBind()</code> that received the client connection
 * @constant client_socket is the client socket
 * @constant user_data is the user_data passed to <code>forwarder_NewBind()</code>
...
void dispatcher_DestroyListener(Dispatcher *dispatcher, PARCEventSocket **listenerPtr);
 ...
 * @constant user_data is the user_data passed to <code>Forwarder_CreateEvent()</code>
 */
 ...
 * @discussion
 *
 *   When created, the timer is idle and you need to call <code>forwarder_StartTimer()</code>
 *
 * @param isPeriodic means the timer will fire repeatidly, otherwise it is a one-shot and
 *         needs to be set again with <code>dispatcher_StartTimer()</code>
 */
PARCEventTimer *dispatcher_CreateTimer(Dispatcher *dispatcher, bool isPeriodic, PARCEvent_Callback *callback, void *userData);
 ...
 */
void dispatcher_StartTimer(Dispatcher *dispatcher, PARCEventTimer *timerEvent, struct timeval *delay);
 ...
 */
void dispatcher_DestroyTimerEvent(Dispatcher *dispatcher, PARCEventTimer **eventPtr);
 ...
 *
 *   When created, the event is idle and you need to call <code>forwarder_StartNetworkEvent()</code>
 *
 ...
 */
PARCEvent *dispatcher_CreateNetworkEvent(Dispatcher *dispatcher, bool isPersistent, PARCEvent_Callback *callback, void *userData, int fd);
...
void dispatcher_DestroyNetworkEvent(Dispatcher *dispatcher, PARCEvent **eventPtr);
 ...
 *
 *   When created, the signal trap is idle and you need to call <code>forwarder_StartSignalEvent()</code>
 *
 * @param signal is the system signal to monitor (e.g. SIGINT).
 * @return <#return#>
 */
PARCEventSignal *dispatcher_CreateSignalEvent(Dispatcher *dispatcher, PARCEventSignal_Callback *callback, void *userData, int signal);

void dispatcher_DestroySignalEvent(Dispatcher *dispatcher, PARCEventSignal **eventPtr);

void dispatcher_StartSignalEvent(Dispatcher *dispatcher, PARCEventSignal *event);
 ...
 */
PARCEventQueue *dispatcher_CreateStreamBufferFromSocket(Dispatcher *dispatcher, SocketType fd);
 ...
 */
PARCEventQueue *dispatcher_StreamBufferConnect(Dispatcher *dispatcher, const AddressPair *pair);
#endif // dispatcher_h
