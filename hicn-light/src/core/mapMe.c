...
#define LOG(mapme, log_level, fmt, ...) \
    do { \
        ...
        if (logger_IsLoggable(logger, LOG_FACILITY, log_level)) { \
            logger_Log(logger, LOG_FACILITY, log_level, __func__, \
                              fmt, ##__VA_ARGS__); \
        } \
   } while(0)

#define WARN(mapme, fmt, ...) LOG(mapme, PARCLogLevel_Warning, fmt, ##__VA_ARGS__)
#define ERROR(mapme, fmt, ...) LOG(mapme, PARCLogLevel_Error, fmt, ##__VA_ARGS__)
#define INFO(mapme, fmt, ...) LOG(mapme, PARCLogLevel_Info, fmt, ##__VA_ARGS__)
#define DEBUG(mapme, fmt, ...) LOG(mapme, PARCLogLevel_Debug, fmt, ##__VA_ARGS__)
...
void
mapMeTFIB_Release(MapMeTFIB ** tfibPtr)
{
 ...
 */
static void
mapMe_CreateTFIB(FibEntry * fibEntry)
{
    MapMeTFIB * tfib;
...
static const PARCEventTimer *
mapMeTFIB_Get(const MapMeTFIB * tfib, unsigned conn_id)
{
...
static void
mapMeTFIB_Put(MapMeTFIB * tfib, unsigned conn_id, const PARCEventTimer * timer)
{
...
static void
mapMeTFIB_Remove(MapMeTFIB * tfib, unsigned conn_id)
{
...
static PARCIterator *
mapMeTFIB_CreateKeyIterator(const MapMeTFIB * tfib)
{
...
static Message *
mapMe_createMessage(const MapMe * mapme, const Name * name, mapme_params_t * params)
{
    Ticks now = forwarder_GetTicks(mapme->forwarder);
    Logger * logger = logger_Acquire(forwarder_GetLogger(mapme->forwarder));

    INFO(mapme, "[MAP-Me] CreateMessage type=%d seq=%d", params->type, params->seq);

    size_t size = (params->protocol == IPPROTO_IPV6) ? HICN_MAPME_V6_HDRLEN : HICN_MAPME_V4_HDRLEN;
    ...
    return message_CreateFromByteArray(NO_INGRESS, icmp_pkt, MessagePacketType_Interest, now, logger);
...
static Message *
mapMe_createAckMessage(const MapMe * mapme, const uint8_t * msgBuffer, const mapme_params_t * params)
{
    Ticks now = forwarder_GetTicks(mapme->forwarder);
    Logger * logger = logger_Acquire(forwarder_GetLogger(mapme->forwarder));
    ...
    return message_CreateFromByteArray(NO_INGRESS, icmp_pkt, MessagePacketType_ContentObject, now, logger);
}
...
static bool
mapMe_setFacePending(const MapMe * mapme, const Name * name, FibEntry *
        fibEntry, unsigned conn_id, bool send, bool is_first, uint32_t
        num_retx);

static void
mapMe_setFacePendingCallback(int fd, PARCEventType which_event, void *data)
{
    ...
    mapMe_setFacePending(args->mapme, args->name, args->fibEntry,
            args->conn_id, args->send, args->is_first, args->num_retx);
}
 ...
 * interest header.
 */
static hicn_mapme_type_t
mapMe_getTypeFromHeuristic(const MapMe * mapme, FibEntry * fibEntry)
{
...
static bool
mapMe_setFacePending(const MapMe * mapme, const Name * name, FibEntry *
        fibEntry, unsigned conn_id, bool send, bool is_first, uint32_t num_retx)
{
    int rc;

    INFO(mapme, "[MAP-Me] SetFacePending connection=%d prefix=XX retx=%d", conn_id, num_retx);
    ...
    if (is_first || send) {
        ...
        const Connection * conn = connectionTable_FindById((ConnectionTable *)table, conn_id);
        if (conn) {
        ...
        if (num_retx < MAX_RETX) {
            ...
            struct setFacePendingArgs * args = malloc(sizeof(struct setFacePendingArgs));
            if (!args)
                goto ERR_MALLOC;
            args->mapme = mapme;
            args->name = name;
            ...
            struct timeval timeout = { mapme->retx / 1000, (mapme->retx % 1000) * 1000 };
 ...
 */
static bool
mapMe_hasLocalNextHops(const MapMe * mapme, const FibEntry * fibEntry)
{
 ...
 */
void
mapMe_onConnectionAdded(const MapMe * mapme, const Connection * conn_added)
{
    /* bool ret; */
    ...
    for (size_t i = 0; i < fibEntryList_Length(fiblist); i++) {
        ...
        INFO(mapme, "[MAP-Me] sending IU/IN for name %s on connection %d", name_str, conn_added_id);
        free(name_str);
 ...
 */
static bool
mapMe_onSpecialInterest(const MapMe * mapme, const uint8_t * msgBuffer,
        unsigned conn_in_id, hicn_prefix_t * prefix, mapme_params_t * params)
{
    const ConnectionTable * table = forwarder_GetConnectionTable(mapme->forwarder);
    /* The cast is needed since connectionTable_FindById miss the
     * const qualifier for the first parameter */
    const Connection * conn_in = connectionTable_FindById((ConnectionTable *)table, conn_in_id);
    ...
    char * name_str = name_ToString(name);
    INFO(mapme, "[MAP-Me] Ack'ed Special Interest on connection %d - prefix=%s type=XX seq=%d", conn_in_id, name_str, seq);
    free(name_str);
    ...
    if (!fibEntry) {
        INFO(mapme, "[MAP-Me]   - Re-creating FIB entry with next hop on connection %d", conn_in_id);
        /*
        ...
        /* Create TFIB associated to FIB entry */
        INFO(mapme, "[MAP-Me]   - Creating TFIB entry with default sequence number");
        mapMe_CreateTFIB(fibEntry);
    }

    fibSeq = TFIB(fibEntry)->seq;
    if (seq > fibSeq) {
        INFO(mapme, "[MAP-Me]   - Higher sequence number than FIB %d, updating seq and next hops", fibSeq);
	    ...
	    while (parcIterator_HasNext(iterator)) {
                ...
                INFO(mapme, "[MAP-Me]   - Re-sending IU to pending connection %d", conn_id);
		mapMe_setFacePending(mapme, fibEntry_GetPrefix(fibEntry), fibEntry, conn_id, false, false, 0);
	    }
        ...
        PARCEventTimer * oldTimer = (PARCEventTimer *)mapMeTFIB_Get(TFIB(fibEntry), conn_in_id);
        if (oldTimer) {
        ...
        for (size_t k = 0; k < numberSet_Length(nexthops); k++) {
            ...
            INFO(mapme, "[MAP-Me]   - Sending IU on current next hop connection %d", conn_id);
            mapMe_setFacePending(mapme, fibEntry_GetPrefix(fibEntry), fibEntry, conn_id, send, false, 0);
            complete = false;
        }
         ...
         */
        if (complete)
            INFO(mapme, "[MAP-Me]   - Update completed !");
        ...
        INFO(mapme, "[MAP-Me]   - Adding multipath next hop on connection %d", conn_in_id);
        fibEntry_AddNexthopByConnectionId(fibEntry, conn_in_id);
         ...
         * arborescence.
         */
        INFO(mapme, "[MAP-Me]   - Update interest %d -> %d sent backwards on connection %d", seq, fibSeq, conn_in_id);
        mapMe_setFacePending(mapme, fibEntry_GetPrefix(fibEntry), fibEntry, conn_in_id, send, false, 0);
    }
...
void
mapMe_onSpecialInterestAck(const MapMe * mapme, const uint8_t * msgBuffer, unsigned conn_in_id, hicn_prefix_t * prefix, mapme_params_t * params)
{
    INFO(mapme, "[MAP-Me] Receive IU/IN Ack on connection %d", conn_in_id);

    const Name * name = name_CreateFromPacket(msgBuffer, MessagePacketType_Interest);

    FIB * fib = forwarder_getFib(mapme->forwarder);
    FibEntry * fibEntry = fIB_Contains(fib, name);
    parcAssertNotNull(fibEntry, "No corresponding FIB entry for name contained in IU Ack");
    ...
    if (seq != INVALID_SEQ) {
        ...
        if (seq < fibSeq) {
            INFO(mapme, "[MAP-Me]   - Ignored special interest Ack with seq=%u, expected %u", seq, fibSeq);
            return;
        }
    }
    ...
    PARCEventTimer * timer = (PARCEventTimer *)mapMeTFIB_Get(TFIB(fibEntry), conn_in_id);
    if (!timer) {
        INFO(mapme, "[MAP-Me]   - Ignored ACK for prefix not having the Connection in TFIB entry. Possible duplicate ?");
        return;
    }
    ...
    INFO(mapme, "[MAP-Me]   - Removing TFIB entry for ack on connection %d", conn_in_id);
