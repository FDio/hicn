 ...
 */
static void connectionManager_MessengerCallback(MessengerRecipient *recipient, Missive *missive);

/**
 * Event callback
 *
 * This is our main run loop to process our queue of messages.  It is scheduled
 * in {@link connectionManager_MessengerCallback} when the queue becomes non-empty.
 *
 ...
 *
 */
static void connectionManager_ProcessQueue(int fd, PARCEventType which_event, void *connManagerVoidPtr);

static void connectionManager_ProcessClosedMissive(ConnectionManager *connManager, const Missive *missive);

// ========================================================
// Public API

ConnectionManager *
connectionManager_Create(Forwarder *forwarder)
{
    ConnectionManager *connManager = parcMemory_AllocateAndClear(sizeof(ConnectionManager));
    parcAssertNotNull(connManager, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(ConnectionManager));
    ...
    PARCEventScheduler *base = dispatcher_GetEventScheduler(forwarder_GetDispatcher(forwarder));
    connManager->timerEvent = parcEventTimer_Create(base, 0, connectionManager_ProcessQueue, connManager);

    connManager->messengerRecipient =
        messengerRecipient_Create(connManager, connectionManager_MessengerCallback);
    messenger_Register(messenger, connManager->messengerRecipient);
    return connManager;
}

void
connectionManager_Destroy(ConnectionManager **managerPtr)
{
...
static void
connectionManager_MessengerCallback(MessengerRecipient *recipient, Missive *missive)
{
    ConnectionManager *connManager = messengerRecipient_GetRecipientContext(recipient);
    ...
    if (missiveDeque_Size(connManager->missiveQueue) == 1) {
        // When it becomes non-empty, schedule {@link connectionManager_ProcessQueue}
...
static void
connectionManager_ProcessQueue(int fd, PARCEventType which_event, void *connManagerVoidPtr)
{
    ConnectionManager *connManager = (ConnectionManager *) connManagerVoidPtr;

    Missive *missive;
    while ((missive = missiveDeque_RemoveFirst(connManager->missiveQueue)) != NULL) {
        switch (missive_GetType(missive)) {
            ...
            case MissiveType_ConnectionDestroyed:
                ...
                break;
            default:
                parcTrapUnexpectedState("Missive %p of unknown type: %d", (void *) missive, missive_GetType(missive));
        }
...
static void
connectionManager_ProcessClosedMissive(ConnectionManager *connManager, const Missive *missive)
{
    logger_Log(connManager->logger, LoggerFacility_Core, PARCLogLevel_Debug, __func__,
    ...
    const Connection *conn = connectionTable_FindById(table, missive_GetConnectionId(missive));
    ...
    if (conn) {
        ...
        // remove from FIB
        forwarder_RemoveConnectionIdFromRoutes(connManager->forwarder, missive_GetConnectionId(missive));
    }
}
