...
NameBitvector *
nameBitvector_CreateFromInAddr(uint32_t s_addr, uint8_t len)
{

    NameBitvector * bitvector = parcMemory_AllocateAndClear(sizeof(NameBitvector));
    parcAssertNotNull(bitvector, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(NameBitvector));
...
NameBitvector *
nameBitvector_CreateFromAddress(const Address *prefix, uint8_t len)
{
...
nameBitvector_Copy(const NameBitvector *original)
{
    ...
    parcAssertNotNull(copy, "parcMemory_AllocateAndClear(%zu) returned NULL", sizeof(NameBitvector));
...
nameBitvector_Destroy(NameBitvector **bitvectorPtr)
{
    ...
    parcAssertNotNull(*bitvectorPtr, "Parameter must dereference to non-null pointer");
...
bool
nameBitvector_StartsWith(const NameBitvector *name, const NameBitvector *prefix)
{
    ...
    if (prefix->len > BLOCK_SIZE)
        return (name->bits[1] == prefix->bits[1]) && ((name->bits[0] ^ prefix->bits[0]) >> (BLOCK_SIZE - (prefix->len -BLOCK_SIZE)) == 0);

    return ((name->bits[1] ^ prefix->bits[1]) >> (BLOCK_SIZE - prefix->len) == 0);
}

bool
nameBitvector_testBit(const NameBitvector *name, uint8_t pos)
{
    if (pos == WIDTH)
        pos = 127;
...
uint8_t
nameBitvector_firstDiff(const NameBitvector *a, const NameBitvector *b)
{
    ...
    //res is computed over the bitvector which is composed by 128 bit all the times
    //however the prefixes may be diffrent just because the have different lengths
    //example:
    //prefix 1: 0::/30
    //prefix 2: 0::/20
    //at this point of the function res would be 0 since both the bitvectors are composed by 0s
    //but the function will return 127-20, which is the position at which the two prefix are
    //different, since prefix 2 has only 20 bits
...
int
nameBitvector_ToIPAddress(const NameBitvector *name, ip_address_t * ip_address)
{
...
Address *
nameBitvector_ToAddress(const NameBitvector *name)
{
    if(name->IPversion == IPv4_TYPE){
        struct sockaddr_in addr;
        ...
        for(int i = 0; i < 8; i++) {
             addr.sin6_addr.s6_addr[i] = (uint8_t)((name->bits[1] >> 8*(7 - i)) & 0xFF);
        }

        int x = 0;
        for (int i=8; i<16; ++i){
            addr.sin6_addr.s6_addr[i] = (uint8_t)((name->bits[0] >> 8*(7 - x)) & 0xFF);
            x++;
        }
