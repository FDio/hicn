...
NameBitvector * nameBitvector_CreateFromIn6Addr(struct in6_addr *addr, uint8_t len);

NameBitvector * nameBitvector_CreateFromAddress(const Address *prefix, uint8_t len);
...
bool nameBitvector_StartsWith(const NameBitvector *name, const NameBitvector *prefix);
...
int nameBitvector_ToIPAddress(const NameBitvector *name, ip_address_t * ip_address);
