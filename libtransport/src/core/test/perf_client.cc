...
class PerfClient : interface::Portal::ConsumerCallback, public PacketManager<Interest> {
 public:
  Client(uint16_t window)
      : portal_(), signals_(portal_.getIoService(), SIGINT), window_(window), in_flight_interests_(0), name_("b001::1") {
  ...
  void onContentObject(Interest::Ptr &&interest,
	...
	while (in_flight_interests_ < window_) {
		auto interest = getPacket();
		portal_.sendInterest();
		in_flight_interests_++;
	}
  }
