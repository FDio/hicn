...
void RTCLossDetectionAndRecovery::setState(std::shared_ptr<RTCState> state) {
  state_ = state;
  scheduleSentinelTimer(SENTINEL_TIMER_INTERVAL);
  sentinel_timer_interval_ = SENTINEL_TIMER_INTERVAL;
};
