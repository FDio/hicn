...
int TLSProducerSocket::writeOld(BIO *b, const char *buf, int num) {
  ...
  if ((SSL_in_before(socket->ssl_) || SSL_in_init(socket->ssl_)) &&
      socket->first_) {
    //! socket->tls_chunks corresponds to is_last
    socket->tls_chunks--;
    ...
    socket->parent_->ProducerSocket::produce(
        socket->name_, (const uint8_t *)buf, num, socket->tls_chunks == 0,
        socket->produced_segments_);
    socket->parent_->setSocketOption(GeneralTransportOptions::MAKE_MANIFEST,
                                     making_manifest);
    socket->first_ = false;
  } else {
    ...
    socket->async_thread_.add([socket = socket, a]() {
      ...
      socket->produced_segments_ += socket->ProducerSocket::produce(
          socket->name_, std::move(mbuf), socket->tls_chunks == 0,
          socket->produced_segments_);
...
int TLSProducerSocket::async_accept() {
  if (!async_thread_.stopped()) {
    async_thread_.add([this]() { this->TLSProducerSocket::accept(); });
  } else {
    throw errors::RuntimeException(
...
uint32_t TLSProducerSocket::produce(Name content_name,
  ...
  tls_chunks = to_call_oncontentproduced_ =
  ...
  produced_segments_ = start_offset;
  ...
  if (!encryption_thread_.stopped()) {
    encryption_thread_.add([this, content_name, buffer = buf,
      ...
      if (start_offset != NULL) {
        TLSProducerSocket::produce(content_name, buffer, size, is_last,
                                   *start_offset);
      } else {
        TLSProducerSocket::produce(content_name, buffer, size, is_last, 0);
      }
    });
  }
}
...
void TLSProducerSocket::asyncProduce(Name content_name,
  ...
  if (!encryption_thread_.stopped()) {
    ...
    encryption_thread_.add([this, content_name, a, is_last, offset,
                            last_segment]() {
      auto buf = std::unique_ptr<utils::MemBuf>(a);
      if (last_segment != NULL) {
        *last_segment = &produced_segments_;
      }
      TLSProducerSocket::produce(content_name, std::move(buf), is_last, offset);
    });
  }
}
