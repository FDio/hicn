...
int ConsumerSocket::rescheduleOnIOService(int socket_option_key,
  ...
  if (transport_protocol_->isRunning() && std::this_thread::get_id() != thread_id_) { 
    std::mutex mtx;
...
int ConsumerSocket::setSocketOption(
  ...
  return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ReadCallback *socket_option_value) -> int {
...
int ConsumerSocket::setSocketOption(int socket_option_key,
  ...
  //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
  return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, std::nullptr_t socket_option_value) -> int {
    switch (socket_option_key) {
...
int ConsumerSocket::setSocketOption(
  ...
  //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
  return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ConsumerContentObjectCallback socket_option_value) -> int {
    switch (socket_option_key) {
      case ConsumerCallbacksOptions::CONTENT_OBJECT_INPUT:
...
int ConsumerSocket::setSocketOption(
  ...
  //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
  return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ConsumerContentObjectVerificationCallback socket_option_value) -> int {
    switch (socket_option_key) {
      case ConsumerCallbacksOptions::CONTENT_OBJECT_TO_VERIFY:
...
int ConsumerSocket::setSocketOption(
  ...
  //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
  return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ConsumerInterestCallback socket_option_value) -> int {
    switch (socket_option_key) {
      case ConsumerCallbacksOptions::INTEREST_RETRANSMISSION:
...
int ConsumerSocket::setSocketOption(
  ...
  //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
  return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ConsumerManifestCallback socket_option_value) -> int {
    switch (socket_option_key) {
...
int ConsumerSocket::setSocketOption(
  ...
  //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
  return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ConsumerTimerCallback socket_option_value) -> int {
    switch (socket_option_key) {
