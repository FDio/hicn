...
ProducerSocket::ProducerSocket(asio::io_service &io_service)
    : io_service_(io_service),
      ...
      on_interest_input_(VOID_HANDLER_PTR),
      on_interest_dropped_input_buffer_(VOID_HANDLER_PTR),
      on_interest_inserted_input_buffer_(VOID_HANDLER_PTR),
      on_interest_satisfied_output_buffer_(VOID_HANDLER_PTR),
      on_interest_process_(VOID_HANDLER_PTR),
      on_new_segment_(VOID_HANDLER_PTR),
      on_content_object_to_sign_(VOID_HANDLER_PTR),
      on_content_object_in_output_buffer_(VOID_HANDLER_PTR),
      on_content_object_output_(VOID_HANDLER_PTR),
      on_content_object_evicted_from_output_buffer_(VOID_HANDLER_PTR),
      on_content_produced_(VOID_HANDLER_PTR) {}
...
void ProducerSocket::passContentObjectToCallbacks(
  ...
  if (content_object) {
    if (on_new_segment_ != VOID_HANDLER_PTR) {
      io_service_.dispatch([this, content_object](){on_new_segment_(*this, *content_object);});
    }

    if (on_content_object_to_sign_ != VOID_HANDLER_PTR) {
      io_service_.dispatch([this, content_object](){on_content_object_to_sign_(*this, *content_object);});
    }

    if (on_content_object_in_output_buffer_ != VOID_HANDLER_PTR) {
      io_service_.dispatch([this, content_object](){on_content_object_in_output_buffer_(*this, *content_object);});
    }

    output_buffer_.insert(content_object);

    if (on_content_object_output_ != VOID_HANDLER_PTR) {
      io_service_.dispatch([this, content_object](){on_content_object_output_(*this, *content_object);});
    }
...
uint32_t ProducerSocket::produce(Name content_name,
  ...
  //Copy the atomic variable to ensure always the same value during the a production
  ...
  if (on_content_produced_ != VOID_HANDLER_PTR) {
    io_service_.dispatch([this, buffer_size](){on_content_produced_(*this, std::make_error_code(std::errc(0)),
                         buffer_size);});
  }
...
void ProducerSocket::onInterest(Interest &interest) {
  if (on_interest_input_ != VOID_HANDLER_PTR) {
  ...
  if (content_object) {
    if (on_interest_satisfied_output_buffer_ != VOID_HANDLER_PTR) {
    ...
    if (on_interest_process_ != VOID_HANDLER_PTR) {
      on_interest_process_(*this, interest);
    }
  }
}

//If the thread calling lambda_func is not the same of io_service, this function reschedule the function on it
...
int ProducerSocket::rescheduleOnIOService(int socket_option_key,
  ...
  if (listening_thread_.joinable() && std::this_thread::get_id() != listening_thread_.get_id()) { 
    std::mutex mtx;
    /* Condition variable for the wait */
    std::condition_variable cv;
    std::unique_lock<std::mutex> lck(mtx);
    bool done = false;
    io_service_.dispatch([this, &socket_option_key, &socket_option_value, &mtx, &cv, &result, &done, &func]() {
      std::unique_lock<std::mutex> lck(mtx);
	    done = true;
...
//If the thread calling lambda_func is not the same of io_service, this function reschedule the function on it
template<typename Lambda, typename arg2>
int ProducerSocket::rescheduleOnIOServiceWithReference(int socket_option_key,
                                                      arg2 &socket_option_value,
                                                      Lambda lambda_func) {
  //To enforce type check
  std::function<int(int, arg2 &)> func = lambda_func;
  int result = SOCKET_OPTION_SET;
  if (listening_thread_.joinable() && std::this_thread::get_id() != this->listening_thread_.get_id()) { 
    std::mutex mtx;
    /* Condition variable for the wait */
    std::condition_variable cv;
    std::unique_lock<std::mutex> lck(mtx);
...
int ProducerSocket::setSocketOption(int socket_option_key,
  ...
  int ProducerSocket::setSocketOption(int socket_option_key,
    ...
    //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
    return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ProducerContentObjectCallback socket_option_value) -> int {
      switch (socket_option_key) {
      case ProducerCallbacksOptions::INTEREST_INPUT:
        if (socket_option_value == VOID_HANDLER_PTR) {
          on_interest_input_ = VOID_HANDLER_PTR;
          break;
        }
  ...
  int ProducerSocket::setSocketOption(
    ...
    //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
    return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ProducerInterestCallback socket_option_value) -> int {
      switch (socket_option_key) {
        case ProducerCallbacksOptions::INTEREST_INPUT:
  ...
  int ProducerSocket::setSocketOption(
    ...
    //Reschedule the function on the io_service to avoid race condition in case setSocketOption is called while the io_service is running.
    return rescheduleOnIOService(socket_option_key, socket_option_value, [this](int socket_option_key, ProducerContentCallback socket_option_value) -> int {
      switch (socket_option_key) {
