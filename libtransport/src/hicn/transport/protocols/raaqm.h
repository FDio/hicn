...
class RaaqmTransportProtocol 
  : public TransportProtocol, public BaseReassembly, public CWindowProtocol {
 ...
 protected:
  ...
  static constexpr uint32_t buffer_size = 1 << interface::default_values::log_2_default_buffer_size;
  static constexpr uint16_t mask = buffer_size - 1;
  using PathTable = std::unordered_map<uint32_t, std::unique_ptr<RaaqmDataPath>>;
 ...
 private:
  ...
  void onContentSegment(Interest::Ptr &&interest, ContentObject::Ptr &&content_object);
  ...
  void returnContentToApplication() override;
