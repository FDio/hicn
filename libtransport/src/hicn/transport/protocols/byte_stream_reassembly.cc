...
void ByteStreamReassembly::reassemble(
  ...
  if (TRANSPORT_EXPECT_TRUE(manifest != nullptr)) {
    TRANSPORT_LOGI("inserting manifest  %u", manifest->getName().getSuffix());
...
void ByteStreamReassembly::assembleContent() {
  ...
  TRANSPORT_LOGI("Looking for content object %u", index_);
  while (it != received_packets_.end()) {
    // Check if valid packet
    if (it->second) {
      TRANSPORT_LOGI("Reassemblying segment %u", index_);
      copyContent(*it->second);
    } else {
      TRANSPORT_LOGI("Ignored manifest %u", index_);
    }

    received_packets_.erase(it);
    index_ = index_manager_->getNextReassemblySegment();
    TRANSPORT_LOGI("new index: %u", index_);
