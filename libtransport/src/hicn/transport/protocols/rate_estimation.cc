	...
	namespace protocol {
		...
		void *Timer(void *data) {
			...
			while (estimator->is_running_) {
				std::this_thread::sleep_for(std::chrono::microseconds((uint64_t)(KV * dat_rtt)));
		...
		void InterRttEstimator::onWindowIncrease(double win_current) {
			...
			auto delay = std::chrono::duration_cast<Microseconds>(end - this->begin_batch_).count();
		...
		void ALaTcpEstimator::onDownloadFinished() {
			TimePoint end = std::chrono::steady_clock::now();
			auto delay = std::chrono::duration_cast<Microseconds>(end - this->start_time_).count();
		...
		void SimpleEstimator::onDownloadFinished() {
			...
			if (!this->estimated_) {
				...
				if (this->number_of_packets_ >=
					...
					delay = std::chrono::duration_cast<Microseconds>(end - this->begin_batch_).count();
					// Assuming all packets carry max_packet_size_ bytes of data
					// (8*max_packet_size_ bits); 1000000 factor to convert us to seconds
					if (this->estimation_) {
						this->estimation_ =
