...
void RTCTransportProtocol::retransmit() {
  ...
  while (it != interestRetransmissions_.end()) {
    ...
    //XXX
    /*auto pathStats = pathTable_.find(producerPathLabel_);

    if(pathStats == pathTable_.end()){
      rtx_time = sent_time;
    }else{
      if(it->second == 0){
      //first rtx: wait the max inter arrival gap
        rtx_time = sent_time + ceil(pathStats->second->getInterArrivalGap());
      } else{
      //not first rtx: wait min rtt
        rtx_time = sent_time + ceil(pathStats->second->getMinRtt());
      }
    }
    */
    ...
    if (first_rtx) {
      ...
      uint64_t now = std::chrono::duration_cast<std::chrono::milliseconds>(
                         std::chrono::steady_clock::now().time_since_epoch())
                         .count();
      if ((now - inflightInterests_[pkt].transmissionTime) > 20) {  //TODO copia da qui
...
void RTCTransportProtocol::onContentObject(
  ...
  if(segmentNumber == probe_seq_number_){
    if(payload_size == HICN_NACK_HEADER_SIZE){
      ...
      std::cout << "WE GOT A DATA PACKET AS A REPLY FOR A PROBE" << std::endl;
