...
void RTCTransportProtocol::checkRound() {
  uint32_t duration = (uint32_t) std::chrono::duration_cast<std::chrono::milliseconds>(
...
void RTCTransportProtocol::updateStats(uint32_t round_duration) {
  ...
  uint32_t BDP =
      (uint32_t) ceil((estimatedBw_ * (double)((double)minRtt_ /
                (double)HICN_MILLI_IN_A_SEC) *
...
void RTCTransportProtocol::computeMaxWindow(uint32_t productionRate,
  ...
  if (BDPWin != 0) {
    maxCWin_ = (uint32_t) ceil((double)BDPWin + ((double)BDPWin / 10.0));  // BDP + 10%
  } else {
...
void RTCTransportProtocol::updateWindow() {
  ...
  if (currentCWin_ < maxCWin_ * 0.7) {
    currentCWin_ = min(maxCWin_, (uint32_t) (currentCWin_ * HICN_WIN_INCREASE_FACTOR));
  } else if (currentCWin_ > maxCWin_) {
    currentCWin_ = max((uint32_t) (currentCWin_ * HICN_WIN_DECREASE_FACTOR), HICN_MIN_CWIN);
  }
}
...
void RTCTransportProtocol::decreaseWindow() {
  ...
  if (gotFutureNack_ == 1)
    currentCWin_ =
        min((currentCWin_ - 1), (uint32_t) ceil((double)maxCWin_ * 0.66));  // 2/3
  else
    currentCWin_--;
...
void RTCTransportProtocol::increaseWindow() {
  ...
  if (currentCWin_ < ((double)maxCWin_ * 0.5)) {
    ...
    currentCWin_ = min(
        maxCWin_, (uint32_t) ceil(currentCWin_ + (1.0 / (double)currentCWin_)));  // linear
  }
}
