...
void RTCTransportProtocol::newRound() {
  round_timer_->expires_after(std::chrono::milliseconds(HICN_ROUND_LEN));
...
void RTCTransportProtocol::probeRtt() {
  probe_timer_->expires_after(std::chrono::milliseconds(1000));
...
void RTCTransportProtocol::sentinelTimer() {
  ...
  sentinel_timer_->expires_after(std::chrono::milliseconds(wait));
...
void RTCTransportProtocol::checkRtx() {
  ...
  rtx_timer_->expires_after(std::chrono::milliseconds(wait));
