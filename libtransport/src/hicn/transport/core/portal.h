...
using PendingInterestHashTable = std::unordered_map<uint32_t, PendingInterest::Ptr>;
...
class Portal {
 ...
 public:
  ...
  TRANSPORT_ALWAYS_INLINE bool interestIsPending(const Name &name) {
    auto it = pending_interest_hash_table_.find(name.getHash32());
  ...
  TRANSPORT_ALWAYS_INLINE void sendInterest(
    ...
    uint32_t hash = interest->getName().getHash32() + interest->getName().getSuffix();
    // Send it
    ...
    pending_interest->setOnContentObjectCallback(std::move(on_content_object_callback));
    pending_interest->setOnTimeoutCallback(std::move(on_interest_timeout_callback));
    pending_interest->startCountdown(std::bind(&Portal<ForwarderInt>::timerHandler, this, std::placeholders::_1, hash));
    pending_interest_hash_table_.emplace(std::make_pair(hash, std::move(pending_interest)));
  }
 ...
 private:
  ...
  TRANSPORT_ALWAYS_INLINE void processContentObject(
    ...
    uint32_t hash = content_object->getName().getHash32() + content_object->getName().getSuffix();
    ...
    if (it != pending_interest_hash_table_.end()) {
      ...
      if (TRANSPORT_EXPECT_TRUE(!interest_ptr->isReceived())) {
        ...
        TRANSPORT_LOGW("Content %s already received! Dropping it.", content_object->getName().toString().c_str());
      }
