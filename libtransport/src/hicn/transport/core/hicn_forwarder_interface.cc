...
std::unique_ptr<RouteToSelfCommand> createCommandRoute(std::unique_ptr<sockaddr> &&addr, uint8_t prefix_length) {
...
HicnForwarderInterface::~HicnForwarderInterface() {
  ...
  send((uint8_t *)command, sizeof(DeleteSelfConnectionCommand), [command](){ delete command; });
}
...
void HicnForwarderInterface::registerRoute(Prefix &prefix) {
  auto command = createCommandRoute(prefix.toSockaddr(), prefix.getPrefixLength()).release();
  send((uint8_t *)command, sizeof(RouteToSelfCommand), [command](){ delete command; });
}
