...
void VPPForwarderInterface::consumerConnection() {
  ...
  std::memcpy(inet_address_.address.v4.as_u8, output.src4->as_u8, IPV4_ADDR_LEN);

  inet6_address_.family = AF_INET6;
  std::memcpy(inet6_address_.address.v6.as_u8, output.src6->as_u8, IPV6_ADDR_LEN);
}
...
void VPPForwarderInterface::registerRoute(Prefix &prefix) {
  ...
  if (face_id_ == uint32_t(~0)) {
    ...
    if (addr.family == AF_INET6) {
      inet6_address_.address.as_u64[0] = output.prod_addr->as_u64[0];
      inet6_address_.address.as_u64[1] = output.prod_addr->as_u64[1];
    } else {
      // The ipv4 is written in the last 4 bytes of the ipv6 address, so we need
      // to copy from the byte 12
      inet_address_.address.as_u64[0] = output.prod_addr->as_u64[0];
      inet_address_.address.as_u64[1] = output.prod_addr->as_u64[1];
    }
